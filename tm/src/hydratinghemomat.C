/*
    ...
    2004 Zbynek Hora
    22.9.2004 16:00 added IST_Humidity to TransportMaterial->giveIPValue(), taken here via HeMoTKMaterial
*/
...
// creates the hydration model instance if necessary, sets the mixture type
{
 ...
 else if (hydration) _error("setMixture: Can't setup undefined hydrationModel.");
}
...
*/
   ...
   if (vech->giveSize()>=2) vech->at(2) = inverse_sorption_isotherm(vec.at(2)); // compute relative humidity
   else {
    vech->resize(2);
   ...
   if (teplotaOut && (gp->giveNumber()==1) && giveStatus(gp)) {
    ...
    if (aux.isEmpty()) {aux.resize(1); aux.zero();}
    aux.times(1./give('d'));
...
HydratingHeMoMaterial :: giveCharacteristicValue (MatResponseMode rmode, GaussPoint* gp, TimeStep* atTime)
{
 ...
 if ((rmode >= Capacity_ww) && (rmode <= Capacity_wh)) { // standard HeMoTK values
  ...
  if (castAt && (atTime->giveTime()<castAt)) answer *= PRECAST_CAPACITY_COEFF; // ~Zero capacity before cast
 }
 else if ((rmode >= IntSource_ww) && (rmode <= IntSource_wh)) { // Internal source values
  if (!hydrationLHS) answer=0;
   ...
   if ((rmode==IntSource_ww) || (rmode==IntSource_hw)) answer *= give_dphi_dw(vec->at(2));
...
// printf ("IP %d::giveIPValue, IST %d", giveNumber(), type);
 if (type == IST_HydrationDegree) {
  ...
  //else answer.at(1) = 0;
  return 1;
 } else return HeMoTKMaterial::giveIPValue (answer, aGaussPoint, type, atTime);
}

InternalStateValueType
HydratingHeMoMaterial::giveIPValueType (InternalStateType type)
{
 if (type == IST_HydrationDegree) return ISVT_SCALAR;
 else return HeMoTKMaterial::giveIPValueType (type);
}

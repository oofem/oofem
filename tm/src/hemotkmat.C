/* $Header: /home/cvs/bp/oofem/tm/src/hemotkmat.C,v 1.2 2003/05/19 13:04:10 bp Exp $ */
/*
...
{
 if ((mode == _2dHeMo) || (mode == _3dHeMo)) return 1;
 return 0;
}
   ...
   //  25.9.2001
{
 ...
 if (s.isEmpty()) _error ("matcond1d: undefined state vector");
 ...
 else _error ("Unknown MatResponseMode");
  ...
  //  25.9.2001
{
 ...
 //  t = Tm->ip[ipp].av[1];
 s = status->giveTempStateVector();
 if (s.isEmpty()) _error ("matcond2d: undefined state vector");
 ...
 d.resize(2,2);
  d.at(1,1) = k;   d.at(1,2) = 0.0;
 d.at(2,1) = 0.0; d.at(2,2) = k;
}
  ...
  //  25.9.2001
{
 ...
 //  t = Tm->ip[ipp].av[1];
 s = status->giveTempStateVector();
 if (s.isEmpty()) _error ("matcond3d: undefined state vector");
 ...
 d.resize(3,3);
  d.at(1,1) = k;   d.at(1,2) = 0.0;  d.at(1,3) = 0.0;
 d.at(2,1) = 0.0; d.at(2,2) = k;    d.at(2,3) = 0.0;
 d.at(3,1) = 0.0; d.at(3,2) = 0.0;  d.at(3,3) = 0.0;
}
...
double HeMoTKMaterial::computeCapacityCoeff (MatResponseMode mode, GaussPoint* gp, TimeStep* atTime)
{
 if (mode == Capacity_ww)
  ...
  s = status->giveTempStateVector();
  if (s.isEmpty()) _error ("computeCapacityCoeff: undefined state vector");
  w = s.at(2);
  t = s.at(1);
...
HeMoTKMaterial::giveHumidity(GaussPoint* gp, ValueModeType mode)
{
 ...
 if (tempState.giveSize()<2) _error("giveHumidity: undefined moisture status!");
  ...
  // IST_Humidity overriden to use inverse_sorption_isotherm
{
  if (type == IST_Humidity) {
    ...
    return 1;
  } else return TransportMaterial::giveIPValue (answer, aGaussPoint, type, atTime);
}

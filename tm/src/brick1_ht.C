/* $Header: /home/cvs/bp/oofem/tm/src/brick1_ht.C,v 1.2 2003/04/23 14:22:15 bp Exp $ */
/*
...
// evaluated at aGaussPoint.
{
  ...
  for (int i=1; i<=8; i++) answer.at(1,i) = n.at(i);
  return ;
}
...
Brick1_ht :: computeNmatrixAt (FloatMatrix& answer, FloatArray* coords)
{
 if (emode==HeatTransferEM) this->computeNSubMatrixAt (answer, coords);
  ...
  answer.resize(2,16);
  for (i=1; i<=2; i++)
   for (j=1;j<=8;j++) answer.at(i,(j-1)*2+i) = n.at(1,j);
 }
 ...
 if (emode == HeatTransferEM) {
   ...
   for (i=1; i<=2; i++) {
     ...
     if (subAnswer.isNotEmpty()) {
       if (answer.isEmpty()) {answer.resize(16); answer.zero();}

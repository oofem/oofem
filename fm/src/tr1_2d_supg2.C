...
{
  if (ut == EID_MomentumBalance) return 2;
  else if (ut == EID_ConservationEquation) return 1;
...
TR1_2D_SUPG2 :: computeDiffusionDerivativeTerm_MB (FloatMatrix& answer, MatResponseMode mode, TimeStep* atTime)
{
  ...
  _b.at(2,1) = 0.;   _b.at(2,2)=c[0]; _b.at(2,3)= 0.;   _b.at(2,4)=c[1]; _b.at(2,5)=0.;   _b.at(2,6)=c[2];
  _b.at(3,1) = c[0]; _b.at(3,2)=b[0]; _b.at(3,3)= c[1]; _b.at(3,4)=b[1]; _b.at(3,5)=c[2]; _b.at(3,6)=b[2];
...
{
  TR1_2D_SUPG :: computeAdvectionTerm_MC (answer, atTime);
}
...
{
  TR1_2D_SUPG :: computeAdvectionDerivativeTerm_MC (answer, atTime);
}
...
TR1_2D_SUPG2::giveIPValue (FloatArray& answer, GaussPoint* aGaussPoint, InternalStateType type, TimeStep* atTime)
{
 if (type == IST_VOFFraction) {
   ...
   return 1;
 } else return SUPGElement::giveIPValue (answer, aGaussPoint, type, atTime);
}
...
contextIOResultType TR1_2D_SUPG2 :: saveContext (FILE* stream, void *obj)
//
...
contextIOResultType TR1_2D_SUPG2 :: restoreContext (FILE* stream, void *obj)
//

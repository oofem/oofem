                               ...
                               double F)
{
  ...
  for (it=bcDofMans.begin(); it != bcDofMans.end(); ++it) {
    ...
    for (k=1; k<=neighborElemList->giveSize(); k++) {
      if (neighborElemList->at(k) == i) continue;
...
FastMarchingMethod::updateTrialValue (FloatArray &dmanValues, int id, double F)
{
  ...
  for (i=1; i<=neighborElemList->giveSize(); i++) {
    ...
    if (ie->giveGeometryType()==EGT_triangle_1) {
      for (j=1; j<=3; j++) if (ie->giveDofManagerNumber(j) == id) _ind = j;
      ...
      if ((dmanRecords.at(ai-1).status == FMM_Status_KNOWN) && 
        ...
        b = cc->distance(ac);
        // compute fi angle 
        cb = *bc; cb.substract(cc); cb.normalize();
        ca = *ac; ca.substract(cc); ca.normalize();
	...
	if (nroot == 0) reg_upd_flag = false;
	else if (nroot == 1) t=r1;
        else if (r1 >= 0.0) t=r1;
        else if (r2 >= 0.0) t=r2;
	else reg_upd_flag = false;
	...
	if (!reg_upd_flag) {
	  if (F > 0.) _h = min (b*F+at, a*F+bt);
	  else _h=max(b*F+at, a*F+bt);
...
{
  int answer;
  if (dmanTrialQueue.empty()) return 0;

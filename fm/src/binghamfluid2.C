...
BinghamFluidMaterial2 :: giveCharacteristicValue (MatResponseMode mode,
                                                 ...
                                                 TimeStep* atTime)
{
  if (mode == MRM_Density) return this->give('d');
...
#else
      if (temp_tau<tau_c) return mu_inf;
      else return mu_0;
#endif
      //return this->mu_0;
  }
  else return FluidDynamicMaterial::giveCharacteristicValue(mode,gp,atTime);
}
...
//
{
 if ((aProperty == Viscosity)) return mu_0;
 else if ((aProperty == YieldStress)) return tau_0;
 else return FluidDynamicMaterial::give(aProperty);
}
...
BinghamFluidMaterial2 :: computeDevStrainMagnitude (MaterialMode mmode, const FloatArray& epsd)
{
  ...
  } else _error ("computeDevStrainMagnitude: unsuported material mode");
  return sqrt(_val);
}
...
BinghamFluidMaterial2 :: computeDeviatoricStrain (FloatArray& answer, const FloatArray& eps, MaterialMode mmode)
{
  ...
  } else _error ("computeDeviatoricStrain: unsuported material mode");
}
...
FluidDynamicMaterialStatus (n,d,g)
{
  ...
  if (mmode == _2dFlow) _size = 3;
  else if (mmode == _2dAxiFlow) _size = 4;
  else _error ("BinghamFluidMaterial2Status: unsupportted material mode");

  deviatoricStrainVector.resize(_size); deviatoricStrainVector.zero();
  deviatoricStressVector.resize(_size); deviatoricStressVector.zero();

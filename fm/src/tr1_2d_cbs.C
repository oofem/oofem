...
int 
TR1_2D_CBS :: computeGlobalCoordinates (FloatArray& answer, const FloatArray& lcoords)
{
 double l1,l2,l3;
...
int
TR1_2D_CBS :: computeLocalCoordinates (FloatArray& answer, const FloatArray& coords)
{
...
TR1_2D_CBS :: computeLEPLICVolumeFraction(const FloatArray& n, const double p, LEPlic* matInterface, bool updFlag) 
{
  ...
  if (answer > 1.000000001) return 1.0;
  else return answer;
}
...
//</RESTRICTED_SECTION>
   if (type == IST_Density) {
     ...
     return 1;
   } else return CBSElement::giveIPValue (answer, aGaussPoint, type, atTime);
}
...
void
TR1_2D_CBS :: ZZNodalRecoveryMI_ComputeEstimatedInterpolationMtrx  
...
//</RESTRICTED_SECTION>
    ...
    } else return CBSElement::giveInternalStateAtNode (answer, type, mode, node, atTime);
}

...
// This function translates this request to numerical method language
{
 ...
 if (chc == EID_ConservationEquation) {// pressures
   ...
   switch (mode) {
   case VM_Incremental:
     if (deltaAuxVelocity.isNotEmpty()) return deltaAuxVelocity.at(eq); else return 0.;
   case VM_Total:
...
// This function translates this request to numerical method language
{
  ...
  if (chc == ReynoldsNumber) {
    if (equationScalingFlag) return this->Re;
    else return 1.0;
  }
  else if (chc == Theta_1) return this->theta[0];
  else if (chc == Theta_2) return this->theta[1];
...
CBS :: giveNextStep ()
{
  ...
  if (currentStep != NULL) totalTime = currentStep->giveTime() + dt;
...
CBS :: solveYourselfAt (TimeStep* tStep) 
{
  ...
  if (initFlag) {
    ...
    if (consistentMassFlag) {
      ...
      if (mss==NULL) _error ("solveYourselfAt: sparse matrix creation failed");
...
//</RESTRICTED_SECTION>
  ...
  for (i=1; i<=presneq_prescribed; i++) prescribedTractionPressure.at(i) /= nodalPrescribedTractionPressureConnectivity.at(i);
  ...
  if (consistentMassFlag) {
    ...
    for (i=1; i<=momneq; i++) velocityVector->at(i) = prevVelocityVector->at(i) + deltaAuxVelocity.at(i)+deltaT*rhs.at(i)/mm.at(i);
  }
...
#ifdef TIME_REPORT
    oofem_timeval tstart;  ::getUtime(tstart);
#endif
    materialInterface->updatePosition(this->giveCurrentStep());
#ifdef TIME_REPORT
    oofem_timeval ut;  ::getRelativeUtime (ut, tstart);
...
contextIOResultType 
CBS :: saveContext (FILE* stream, void *obj)
// 
...
//
{
 ...
 if (stream==NULL) {
  ...
  if ((iores = PressureField.saveContext(stream)) != CIO_OK) THROW_CIOERR(iores);
  if ((iores = VelocityField.saveContext(stream)) != CIO_OK) THROW_CIOERR(iores);
  if ((iores = prescribedTractionPressure.storeYourself(stream)) != CIO_OK) THROW_CIOERR(iores);
...
contextIOResultType 
CBS :: restoreContext (FILE* stream, void *obj)
// 
...
//
{
 ...
 if ((iores = PressureField.restoreContext(stream)) != CIO_OK) THROW_CIOERR(iores);
 if ((iores = VelocityField.restoreContext(stream)) != CIO_OK) THROW_CIOERR(iores);
 if ((iores = prescribedTractionPressure.restoreYourself(stream)) != CIO_OK) THROW_CIOERR(iores);
...
// if success returns nonzero
 ...
 if (equationScalingFlag) {
   ...
   for (i=1; i<= nbc; i++) {
     ...
     else _error ("checkConsistency: unknown bc/ic type\n");
   }
...
CBS::printDofOutputAt (FILE* stream, Dof* iDof, TimeStep* atTime) 
{
  ...
  else _error ("printDofOutputAt: unsupported dof type");
...
//
  ...
  if ((id == EID_MomentumBalance) || (id == EID_AuxMomentumBalance)) 
    ...
    return numberOfConservationEqs;
  else _error ("giveNumberOfEquations: unknown equation id");
  return 0;
}
...
//
  ...
  if ((id == EID_MomentumBalance) || (id == EID_AuxMomentumBalance)) 
    ...
    return numberOfPrescribedConservationEqs;
  else _error ("giveNumberOfPrescribedEquations: unknown equation id");
  return 0;
}
...
//
  ...
  if ((id == EID_MomentumBalance) || (id == EID_AuxMomentumBalance)) 
    ...
    return numberOfConservationEqs;
  else _error ("giveNumberOfDomainEquations: unknown equation id");
  return 0;
}
...
//
  ...
  if ((id == EID_MomentumBalance) || (id == EID_AuxMomentumBalance)) 
    ...
    return numberOfPrescribedConservationEqs;
  else _error ("giveNumberOfPrescribedDomainEquations: unknown equation id");
  return 0;
}

double CBS :: giveVariableScale (VarScaleType varID)
{
  if (varID == VST_Length) return this->lscale;
  else if (varID == VST_Velocity) return this->uscale;
  else if (varID == VST_Density) return this->dscale;
  else if (varID == VST_Time) return (lscale/uscale);
  else if (varID == VST_Pressure) return (dscale*uscale*uscale);
  else if (varID == VST_Force) return (uscale*uscale/lscale);
  else if (varID == VST_Viscosity) return 1.0;
  else _error ("giveVariableScale: unknown variable type");
  return 0.0;

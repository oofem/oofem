...
LEPlicElementInterface::isBoundary ()
{
  ...
  if ((fvi > 0.) && (fvi <= 1.0)) {
    ...
    if ((fvi > 0.) && (fvi < 1.0)) return true;
      currCell.at(1) = this->giveElement()->giveNumber(); contable->giveElementNeighbourList(neighborList, currCell);
...
LEPlic::doLagrangianPhase(TimeStep* atTime)
{
  ...
  for (i=1; i<=ndofman; i++) {
    ...
    if (vfield == NULL) _error ("doLagrangianPhase: Velocity field not available");
    ...
    for (ci=1; ci<=nsd; ci++) x2.at(ci)=x.at(ci)+dt*v_tn1.at(ci);
#else
    // pure explicit version
    dman->giveUnknownVector(v_t, velocityMask, EID_MomentumBalance, VM_Total, atTime);
    for (ci=1; ci<=nsd; ci++) x2.at(ci)=x.at(ci)+dt*v_t.at(ci);
#endif
...
LEPlic::doCellDLS (FloatArray& fvgrad, int ie, bool coord_upd, bool vof_temp_flag)
{
  ...
  if ((interface = (LEPlicElementInterface*) (domain->giveElement(ie)->giveInterface(LEPlicElementInterfaceType)))) {
    if (vof_temp_flag) fvi = interface->giveTempVolumeFraction(); else fvi = interface->giveVolumeFraction();
    if ((fvi > 0.) && (fvi <= 1.0)) {
      // potentially boundary cell 

      if ((fvi > 0.) && (fvi < 1.0)) isBoundaryCell = true;
      ...
      */
      ...
      for (i=1; i<=nneighbr; i++) {
        ineighbr = neighborList.at(i);
        if (ineighbr == ie) continue; // skip itself
        if ((ineghbrInterface = 
             (LEPlicElementInterface*) (domain->giveElement(ineighbr)->giveInterface(LEPlicElementInterfaceType)))) {
          if (vof_temp_flag) fvk=ineghbrInterface->giveTempVolumeFraction(); else fvk = ineghbrInterface->giveVolumeFraction();
          if (fvk < 1.0) isBoundaryCell = true;
...
LEPlic::findCellLineConstant (double &p, FloatArray& fvgrad, int ie, bool coord_upd, bool temp_vof_flag)
{
  ...
  */
  ...
  if (temp_vof_flag) target_vof= interface->giveTempVolumeFraction(); else target_vof= interface->giveVolumeFraction();
  ...
  */
  ...
  if (temp_vof_flag) fvi = interface->giveTempVolumeFraction(); else fvi = interface->giveVolumeFraction();
  if ((fvi > LEPLIC_ZERO_VOF) && (fvi < 1.0)) {
    ...
    for (ivert=1; ivert<=nelemnodes; ivert++) {
      ...
      if ( ((ivof-target_vof) >= 0.) && (ivof < upper_vof)) {
        upper_vof = ivof; upper_p = pp;
      } else if (((target_vof-ivof) >= 0.) && (ivof>lower_vof)) {
        lower_vof = ivof; lower_p = pp;
      }
    }

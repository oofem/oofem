...
DynCompRow::DynCompRow(const DynCompRow &S) : base_(S.base_)
{
 ...
 if (S.rows_) {
  ...
  for (i=0; i<S.nRows; i++) this->rows_[i]= new FloatArray(*S.rows_[i]);
 } else this->rows_ = NULL;

 if (S.colind_) {
  this->colind_ = new IntArray*[S.nRows];
  for (i=0; i<S.nRows; i++) this->colind_[i]= new IntArray(*S.colind_[i]);
 } else this->colind_ = NULL;
...
DynCompRow& DynCompRow::operator=(const DynCompRow &C)  
{
 ...
 if (C.rows_) {
  ...
  for (i=0; i<C.nRows; i++) this->rows_[i]=new FloatArray(*C.rows_[i]);
 } else this->rows_ = NULL;
 ...
 if (C.colind_) {
  ...
  for (i=0; i<C.nRows; i++) this->colind_[i]=new IntArray(*C.colind_[i]);
 } else this->colind_ = NULL;
...
*/
 ...
 colind_ = (IntArray**) new IntArray*[neq];
 for (j=0; j<neq; j++) colind_[j] = new IntArray();
...
double& DynCompRow::at (int i, int j) 
{
 ...
 // increment version
 this->version++;
 if ((colIndx= this->giveColIndx (i-1, j-1))) return rows_[i-1]->at(colIndx);
...
double DynCompRow::operator()(int i, int j)  const
{
 ...
 int colIndx;
 if ((colIndx= this->giveColIndx (i, j))) return rows_[i]->at(colIndx);
 if (i < nRows && j < nColumns) return 0.0;
 else {
...
int DynCompRow::giveColIndx (int row, int col) const
{
 ...
 if (right == 0) return 0;
 if (this->colind_[row]->at(right) == col) return right;
 ...
 if (middleVal == col) return middle;
 return 0;
}
  ...
  for (i=1; i< nRows; i++) { // loop  over rows
    ...
    while (curr <= end) {
      if ((fabs(w.at(curr)) < drop_tol * inorm) && ( iw.at(curr)!= i)) {
        ...
        if (curr != end) irw(iw.at(curr)) = curr;
        end --;
      } else {
        curr ++;
      }
    ...
    for (kk=1; kk<= count; kk++) {
      ...
      if (indx==0) OOFEM_ERROR ("DynCompRow::ILUPYourself : internal error");
      previndx = iw.at(indx);
      ...
      if (irw(iw.at(indx)) != 0) irw(iw.at(indx)) = indx;
      kkend--;
    ...
    /*
       ...
       */  
    if ((icount-count) != 1) OOFEM_ERROR4 ("DynCompRow::ILUPYourself : %d - row errorr (%d,%d)\n", i, icount, count);

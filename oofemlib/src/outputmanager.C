/* $Header: /home/cvs/bp/oofem/oofemlib/src/outputmanager.C,v 1.10.4.1 2004/04/05 15:19:43 bp Exp $ */
/*
...
OutputManager :: initializeFrom (InputRecord* ir)
{
 ...
 if (!ir->hasField (IFT_OutputManager_name, "outputmanager")) OOFEM_ERROR ("OutputManager::instanciateFrom: bad record");
...
OutputManager::doDofManOutput (FILE* file, TimeStep* tStep) 
{
 ...
 if (dofman_all_out_flag   && dofman_except.isEmpty())
  for (i=1; i<= ndofman; i++) {
   ...
   if (domain->giveDofManager(i)->giveParallelMode() == DofManager_null) continue;
#endif
   domain->giveDofManager(i)->printOutputAt(file, tStep);
  } else 
  for (i=1; i<= ndofman; i++) 
   if (_testDofManOutput (i)) domain->giveDofManager(i)->printOutputAt(file, tStep);
...
OutputManager :: doElementOutput (FILE* file, TimeStep* tStep) 
{
 ...
 for (i=1; i<= nelem; i++) {
  ...
  if (domain->giveElement(i)->giveParallelMode() == Element_remote) continue;
#endif 
  
  domain -> giveElement(i)->printOutputAt(file, tStep);
 } else 
  for (i=1; i<= nelem; i++) 
   if (_testElementOutput (i)) domain->giveElement(i)->printOutputAt(file, tStep);
...
OutputManager ::_testDofManOutput (int number) 
{
...
#ifdef __PARALLEL_MODE
 // test for null dof in parallel mode
 if (domain->giveDofManager(number)->giveParallelMode() == DofManager_null) return 0;
#endif

 // test all_select flag on
 if(dofman_all_out_flag) selected  = 1;
 else {
  ...
  for (dofmanOutIter = dofman_out.begin(); dofmanOutIter!=dofman_out.end(); ++dofmanOutIter) {
   if ((*dofmanOutIter).test (number)) {selected  = 1; break;}
  }
 }
 ...
 for (dofmanExceptIter=list2->begin(); dofmanExceptIter!=list2->end(); ++dofmanExceptIter) {
  // test if excluded
  if ((*dofmanExceptIter).test (number)) return 0;
 }
...
OutputManager ::_testElementOutput (int number) 
{
...
#ifdef __PARALLEL_MODE
 // test for remote element in parallel mode
 if (domain->giveElement(number)->giveParallelMode() == Element_remote) return 0;
#endif 


 // test all_select flag on
 if (element_all_out_flag) selected  = 1;
 else {
  ...
  for (elemOutIter=element_out.begin(); elemOutIter!=element_out.end(); ++elemOutIter) {
   if ((*elemOutIter).test (number)) {selected  = 1; break;}
  }
 }
 // if not selected exit
 if (!selected) return 0;
 ...
 for (elemExceptIter=element_except.begin(); elemExceptIter!=element_except.end(); ++elemExceptIter) {
  // test if excluded
  if ((*elemExceptIter).test (number)) return 0;
 }
...
{
 if (tstep_all_out_flag) return 1;
 
 if (tstep_step_out) 
  if (((tStep->giveNumber()-domain->giveEngngModel()->giveNumberOfFirstStep()) % tstep_step_out) == 0) return 1;
  
 dynaList<Range>::iterator tstepsIter;
 for (tstepsIter = tsteps_out.begin(); tstepsIter!=tsteps_out.end(); ++tstepsIter) {
  // test if INCLUDED
  if ((*tstepsIter).test (tStep->giveNumber())) return 1;
 }
...
OutputManager :: testDofManOutput (int num, TimeStep* tStep) 
{
 if (testTimeStepOutput (tStep))
  if (_testDofManOutput (num)) return 1;
 return 0;
}
...
{
 if (testTimeStepOutput (tStep))
  if (_testElementOutput (num)) return 1;
 return 0;
}

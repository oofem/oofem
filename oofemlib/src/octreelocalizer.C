...
oofemOctantRec::oofemOctantRec (OctreeSpatialLocalizer* loc, oofemOctantRec* parent, FloatArray& origin, double size)
{
 ...
 for (i=0; i<=1; i++)
  for (j=0; j<=1; j++)
   for (k=0; k<=1; k++) this->child[i][j][k] = NULL;
...
oofemOctantRec::~oofemOctantRec ()
{
 ...
 for (i=0; i<=1; i++)
  for (j=0; j<=1; j++)
   for (k=0; k<=1; k++) if (this->child[i][j][k]) delete this->child[i][j][k];
...
oofemOctantRec::containsPoint (const FloatArray& coords)
{
 ...
 for (i=1; i<=coords.giveSize(); i++) {
  if (localizer->giveOctreeMaskValue(i)) {
   if (coords.at(i) < this->origin.at(i)) return 0;
...
oofemOctantRec::giveChildContainingPoint (oofemOctantRec** child, const FloatArray& coords)
{
 ...
 if (this->containsPoint (coords)) {
  if (this->isTerminalOctant()) {*child = NULL; return -1;}
  for (i=1; i<=coords.giveSize(); i++) {
   if (localizer->giveOctreeMaskValue(i) && (coords.at(i) > (this->origin.at(i) + this->size/2.))) ind.at(i) = 1;
   else ind.at(i) = 0;
  }
...
oofemOctantRec::divideLocally (int level, const IntArray& octantMask) 
{
 ...
 if (newLevel > 0) {
  ...
  for (i=0; i<=octantMask.at(1); i++)
   for (j=0; j<=octantMask.at(2); j++)
    for (k=0; k<=octantMask.at(3); k++) {
     if (this->child[i][j][k]) result &= this->child[i][j][k]->divideLocally (newLevel, octantMask); 
    }
 }
 return result;
}
...
oofemOctantRec::testBoundingBox (const FloatArray& coords, double radius)
{
 ...
 for (i=1; i<4; i++) cellCenter.at(i) += this->size/2.0;
 for (i=1, dist = 0.0; i<=size; i++) 
  if (localizer->giveOctreeMaskValue(i)) dist += (cellCenter.at(i)-coords.at(i))*(cellCenter.at(i)-coords.at(i));
 dist = sqrt (dist);
 if (dist > (cellRadius+radius)) return BBS_OUTSIDECELL;
...
*/
  ...
  int inBounds = 1;
  for (i=1; i <= size; i++) {
   if (localizer->giveOctreeMaskValue(i) && (fabs(cellCenter.at(i)-coords.at(i)) > (this->size/2.+radius))) inBounds = 0;
  }
  if (inBounds) return BBS_CONTAINSCELL;
 }
...
OctreeSpatialLocalizer::buildOctreeDataStructure ()
{
 ...
 for (i=1; i<=nnode; i++) {
  ...
  if ((dman->giveClassID()==NodeClass) || (dman->giveClassID()==RigidArmNodeClass)) {
   ...
   if (init) {
    init = 0;
    for (j=1; j<=coords->giveSize(); j++) minc.at(j) = maxc.at(j) = coords->at(j);
   } else {
    for (j=1; j<=coords->giveSize(); j++) {
     if (coords->at(j) < minc.at(j)) minc.at(j) = coords->at(j);
     if (coords->at(j) > maxc.at(j)) maxc.at(j) = coords->at(j);
    }
   }
  }
 } // end loop over nodes
 // determine root size
 rootSize = 0.0;
 for (i=1; i<=3; i++) rootSize = 1.000001 * max (rootSize, maxc.at(i)-minc.at(i));
 // check for degenerated domain
 resolutionLimit = min (1.e-3, rootSize/1.e6);
 for (i=1; i<=3; i++) 
  if ((maxc.at(i)-minc.at(i)) > resolutionLimit) this->octreeMask.at(i) = 1; 
  else this->octreeMask.at(i) = 0;
...
OctreeSpatialLocalizer::initElementIPDataStructure ()
{
 ...
 if (this->elementIPListsInitialized) return 1;
...
OctreeSpatialLocalizer::insertNodeIntoOctree (oofemOctantRec* rootCell, int nodeNum, const FloatArray& coords)
{
 ...
 if ((nCellItems > OCTREE_MAX_NODES_LIMIT) && (cellDepth <= OCTREE_MAX_DEPTH)) {
  ...
  result = currCell->giveChildContainingPoint (&currCell, coords);
  if (result != 1) _error ("insertNodeIntoOctree: internal error - octree inconsistency");
 }
...
OctreeSpatialLocalizer::giveElementContainingPoint (const FloatArray& coords, const IntArray* regionList)
{
 ...
 currCell = this->findTerminalContaining (rootCell, coords);
 if(currCell == NULL)currCell = rootCell;
...
/* HUHU CHEATING */
	 ...
	 if(ielemptr -> giveParallelMode() == Element_remote)continue;
#endif

   interface = (SpatialLocalizerInterface*) ielemptr->giveInterface(SpatialLocalizerInterfaceType);
   if (interface) {
    if (regionList && (regionList->findFirstIndexOf (ielemptr->giveRegionNumber())==0)) continue;
    if (interface->SpatialLocalizerI_BBoxContainsPoint (coords) == 0) continue;
    if (interface->SpatialLocalizerI_containsPoint (coords)) return ielemptr;
   }
   ...
   for (j=0; j<=octreeMask.at(2); j++)
    for (k=0; k<=octreeMask.at(3); k++) {
     if (cell->giveChild(i,j,k)) {
      if (scannedChild == cell->giveChild(i,j,k)) continue;
...
/* HUHU CHEATING */
    ...
    } else _error ("giveElementCloseToPoint: no interface");
   }
   if(minDist == 0.0 || currCell == rootCell)return answer;
  }
 }
 ...
 if (BBStatus == oofemOctantRec::BBS_INSIDECELL) return answer;
  ...
  if(startCell == NULL)startCell = rootCell;
  ...
  for (cellListIt=cellList.begin(); cellListIt != cellList.end(); ++cellListIt) {
   if (currCell == (*cellListIt)) continue;
...
/* HUHU CHEATING */
    ...
    } else _error ("giveElementCloseToPointWithinOctant: no interface");
   }
...
/* HUHU CHEATING */
 ...
 // construct bounding box and test its position within currCell
 BBStatus = currCell->testBoundingBox (bborigin,minDist);
 if (BBStatus == oofemOctantRec::BBS_INSIDECELL) return nearestGp;
 else if (BBStatus == oofemOctantRec::BBS_CONTAINSCELL) {
...
*/
 } else _error ("giveClosestIP: octree inconsistency found");
 return NULL;
}
...
/* HUHU CHEATING */
    ...
    for (j=0 ; j < iRule->getNumberOfIntegrationPoints() ; j++) {
     ...
     } else _error ("giveClosestIPWithinOctant: computeGlobalCoordinates failed");
    }
   }
 ...
 if (currentCell->isTerminalOctant()) {
  ...
  if (!elementList->empty()) 
   for (pos=elementList->begin(); pos !=  elementList->end(); ++pos) {
    ...
    if (elemSet.find(*pos) != elemSet.end()) continue;
    // ask for element
    ielem = domain->giveElement(*pos);
...
*/
    ...
    for (j=0 ; j < iRule->getNumberOfIntegrationPoints() ; j++) {
     if (ielem->computeGlobalCoordinates (jGpCoords, *(iRule->getIntegrationPoint(j)->giveCoordinates()))) {
      currDist = coords.distance(jGpCoords);
      // multiple insertion are handled by STL set implementation
      if (currDist <= radius) elemSet.insert (*pos);
     } else _error ("giveElementsWithIPWithinBox: computeGlobalCoordinates failed");
    }
   }
...
OctreeSpatialLocalizer::giveMaxTreeDepthFrom (oofemOctantRec* root, int& maxDepth)
{
 ...
 for (i=0; i<=octreeMask.at(1); i++)
  for (j=0; j<=octreeMask.at(2); j++)
   for (k=0; k<=octreeMask.at(3); k++) 
    if (root->giveChild(i,j,k)) this->giveMaxTreeDepthFrom (root->giveChild(i,j,k), maxDepth);

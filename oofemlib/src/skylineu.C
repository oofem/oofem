...
int
SkylineUnsym :: buildInternalStructure (EngngModel* eModel, int di, EquationID  ut) 
{
...
//
  ...
  // Enlarge the rowcolumns
  for (i=1; i<=n-1; i++) this->giveRowColumn(i) -> growTo(firstIndex.at(i));
...
SkylineUnsym::GiveCopy () const 
{
 ...
 for (i=1; i<=size; i++) {
  ...
  if (rowColumni) newRowColumns[i-1] = rowColumni->GiveCopy();
  else newRowColumns[i-1] = NULL;
 }
...
{
 int nelem = 0;
 for (int i=1; i<=size; i++) nelem += this->giveRowColumn(i)->giveSize();

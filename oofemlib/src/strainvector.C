/* $Header: /home/cvs/bp/oofem/oofemlib/src/strainvector.C,v 1.1.4.1 2004/04/05 15:19:44 bp Exp $ */
/*
...
StrainVector::computePrincipalValues (FloatArray& answer) const
{
  ...
  for (int i=1; i<=size; i++) if (fabs(this->at(i)) > 1.e-20) nonzeroFlag = 1;
  if (nonzeroFlag==0) {
   answer.zero(); return;
  }
  ...
  D= dst*dst + this->at(3)*this->at(3);
    ...
    if (D < 0.) OOFEM_ERROR("StrainVector::computePrincipalValues: Imaginar roots ");
    D = sqrt(D);
...
StrainVector::computePrincipalValDir (FloatArray& answer, FloatMatrix& dir) const
{
 ...
 if (myMode == _1dMat) {
  ...
  for (i=1; i<=size; i++) if (fabs(this->at(i)) > 1.e-20) nonzeroFlag = 1;
  if (nonzeroFlag==0) {

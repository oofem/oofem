/* $Header: /home/cvs/bp/oofem/oofemlib/src/Attic/petscordering.C,v 1.1.2.1 2004/04/05 15:19:43 bp Exp $ */
/*
...
PetscOrdering_Base :: isLocal (DofManager* dman) {
  ...
  if (dman->giveParallelMode() == DofManager_local) return true;
  if (dman->giveParallelMode() == DofManager_shared) {
    ...
    int minrank = myrank;
    for (int j=1; j<=n; j++) minrank = min (minrank, plist->at(j));
...
{
  if (dman->giveParallelMode() == DofManager_shared) return true;
  else return false;
}
...
*/
      ...
      for (j=1; j<=ndofs; j++) {
        if (dman->giveDof(j)->isPrimaryDof()) {
          if (et == et_standard) {
            ...
            if (dman->giveDof(j)->givePrescribedEquationNumber()) l_neqs++;
	  }
        }
      }       
    }
  ...
  int offset = 0;
  for (j=0; j<myrank; j++) offset+=leqs[j];
  // count global number of eqs
  for (g_neqs=0, j=0; j<emodel->giveNumberOfProcesses(); j++) g_neqs+=leqs[j]; 

  // send numbered shared ones
  if (et == et_standard) locGlobMap.resize(emodel->giveNumberOfEquations(ut));
  else locGlobMap.resize(emodel->giveNumberOfPrescribedEquations(ut));
  ...
  for (i=1; i<=ndofman; i++) {
    ...
    if (isShared(d->giveDofManager(i))) {
      ...
      if (minrank == myrank) { // count to send
        for (j=1; j<=psize; j++) sizeToSend(plist->at(j))+=(1+n); // ndofs+dofman number
      } else {
        nrecToReceive(minrank)++;
        for (j=1; j<=psize; j++) sizeToRecv(plist->at(j))+=(1+n); // ndofs+dofman number
      }
    }
  }
  ...
  */
	  ...
	  //fprintf (stderr, "[%d] Sending localShared node %d to proc %d\n", myrank, dman->giveGlobalNumber(), p);
          ...
          for (k=1; k<=ndofs; k++) {
            if (dman->giveDof(k)->isPrimaryDof()) {
              ...
              if (eq) buffs[p]->packInt(locGlobMap.at(eq));
            }
          }
        }
      }
  ...
  // send buffers
  for (p=0; p<nproc; p++) if (p != myrank) buffs[p]->iSend(p, 999);
...
****/
  ...
  for (p=0; p<nproc; p++) if (p != myrank) rbuffs[p]->iRecv (p, 999);

  IntArray finished(nproc);
  int fin = 1; finished.at(emodel->giveRank()+1) = 1;
  do {
    for (p=0; p<nproc; p++) {
...
PetscNatural2GlobalOrdering::map2New (IntArray& answer, const IntArray& src, int baseOffset) 
{
  ...
  for (i=1; i<=n; i++) {
    if ((indx=src.at(i))) answer.at(i)=locGlobMap.at(indx)+baseOffset;
    else answer.at(i)=(-1)+baseOffset;
  }
}
...
PetscNatural2GlobalOrdering::map2Old (IntArray& answer, const IntArray& src, int baseOffset)
{
  ...
  answer.resize(n);
  for (i=1; i<=n; i++) answer.at(i)=giveOldEq(src.at(i))+offset;
} 
...
PetscNatural2LocalOrdering::init (EngngModel* emodel, EquationID ut, int di,  EquationType et)
{
  ...
  if (et == et_standard) n2l.resize(emodel->giveNumberOfEquations(ut));
  else n2l.resize(emodel->giveNumberOfPrescribedEquations(ut));
...
PetscNatural2LocalOrdering::map2New (IntArray& answer, const IntArray& src, int baseOffset)
{
  ...
  answer.resize(n);
  for (i=1; i<=n; i++) 
    if ((indx=src.at(i))) answer.at(i)=n2l.at(indx)-baseOffset;
    else answer.at(i)=0-baseOffset;
...
PetscNatural2LocalOrdering::map2Old (IntArray& answer, const IntArray& src, int baseOffset)
{
  int i, n=src.giveSize();
  answer.resize(n);
  for (i=1; i<=n; i++) answer.at(i)=giveOldEq(src.at(i))-baseOffset;
}

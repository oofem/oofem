...
Polygon::computeVolume() const
{
  ...
  it.giveNext(p); x1=p.coords(0); y1=p.coords(1);
  it.giveNext(p); x2=p.coords(0); y2=p.coords(1);
  while (it.giveNext(p)) {
    x3=p.coords(0); y3=p.coords(1);
...
Polygon::draw (oofegGraphicContext& gc, bool filled, int layer)
{
  ...
  if (filled) {
    ...
    while (it.giveNext(p)) {
      xc+=p.coords(0); yc+=p.coords(1);        
      count++;
    }
    xc/=count; yc/=count;

    EASValsSetFillStyle(FILL_SOLID);
    r[0].x = xc; r[0].y=yc; r[0].z=0.0;
    Polygon::PolygonEdgeIterator it2(this);
    Vertex p1,p2;
    while (it2.giveNext(p1,p2)) {
      r[1].x=p1.coords(0); r[1].y=p1.coords(1); r[1].z=0.0;
      r[2].x=p2.coords(0); r[2].y=p2.coords(1); r[2].z=0.0;
    ...
    while (it.giveNext(p1,p2)) {
      p[0].x=p1.coords(0); p[0].y=p1.coords(1); p[0].z=0.0;
      p[1].x=p2.coords(0); p[1].y=p2.coords(1); p[1].z=0.0;
...
#endif
    if ((fabs(d1/min(plength,qlength)) >= GT_EPS) && (fabs(d2/min(plength,qlength)) >=GT_EPS) && (sgn(d1)*sgn(d2) > 0.)) return 0; // lines are parallel with nonzero distance
...
#endif
    if ((qt1<0.)&&(qt2<0.)) return 0; // lines do not have common part
    if ((qt1>1.)&&(qt2>1.)) return 0;    // lines do not have common part
    ...
    if ((qt1<-GT_EPS) && (qt2>(1.+GT_EPS))) { // p1,p2 inside q1,q2
      ...
      *alpha2 = qt2;
      ...
      if ((pt1>=GT_EPS) && (pt1<=(1.-GT_EPS))) { // p1 inside
        *alpha1 = pt1; *alpha2=qt1;
        return 11;
      } else if ((pt2>=GT_EPS) && (pt2<=(1.-GT_EPS))) { // p2 inside
        *alpha1 = pt2; *alpha2=qt1;
        return 21;
      ...
      double pt2 = (qtx*(p2->x-q1->x)+qty*(p2->y-q1->y))/qlength;
      if ((pt1>=GT_EPS) && (pt1<=(1.-GT_EPS))) { // p1 inside
        *alpha1 = pt1; *alpha2=qt2;
        return 12;
      } else if ((pt2>=GT_EPS) && (pt2<=(1.-GT_EPS))) { // p2 inside
        *alpha1 = pt2; *alpha2=qt2;
        return 22;
      } else if ((pt1 <=GT_EPS || (1.-pt1) <= GT_EPS) && ((pt2 <0.) || (pt2>1.0))) {// p1 inside
        *alpha1 = pt1; *alpha2=qt2;
        return 12;
...
Graph::clip(Polygon& result, const Polygon& a, const Polygon& b)
{
  ...
  do {
    ...
    if(auxs->status != NS_Intersection) {
      ...
      if (testCollapsedEdge (p1,p2)) continue;
      ...
      do {
        ...
        if(auxc->status != NS_Intersection) {
          ...
          if (testCollapsedEdge (q1,q2)) continue;
          ...
          if (ret == 0) { /*linear are parallel, but do not coincide*/
            ...
            */
            ...
            if (vertex2IntersectionVertex(p1,q1,q2)) {
              ...
              p1->neighbor = is; is->neighbor = p1; 
              insert(is, auxc, next_node(auxc->next));
              if (alpha1 < alpha2) is->entry=-1;
            ...
            if (vertex2IntersectionVertex(p2,q1,q2)) {
              ...
              p2->neighbor = ic; ic->neighbor = p2;
              insert(ic, auxc, next_node(auxc->next));
              if (alpha1 > alpha2) ic->entry=-1;
            ...
            */
            if (ret == 0) { // no intersection
              ...
              if ((p1->status==NS_IntersectionVertex) || (p2->status==NS_IntersectionVertex) ||
                ...
                if (p1->status==NS_IntersectionVertex) {if ((_p1=belongs(p1, q1,q2))) c1++;}
                if (p2->status==NS_IntersectionVertex) {if ((_p2=belongs(p2, q1,q2))) c1++;}
                if (q1->status==NS_IntersectionVertex) {if ((_q1=belongs(q1, p1,p2))) c2++;}
                if (q2->status==NS_IntersectionVertex) {if ((_q2=belongs(q2, p1,p2))) c2++;}
                
                if ((c1 == 1)||(c2==1)) { skip=true;
                } else if ((c1 > 1)||(c2>1)) {
  ...
  auxs=s; 
  do {
    if(auxs->entry!=-1) identical=false;
    if(auxs->status==NS_Intersection)  {
...
{
  node *aux=p;
  while(aux && (aux->status==NS_Intersection))  aux=aux->prev;
  return aux;
}
...
{
  node *aux=p;
  while(aux && (aux->status==NS_Intersection)) aux=aux->next;
  return aux;
}
...
{
  if (n->status == NS_Vertex) return false;
  else if (n->neighbor->status == NS_Intersection) { //Intersection or intersection vertex
    if ((next_node(n->neighbor) == v2) && (prev_node(n->neighbor) == v1)) return true;
    if ((prev_node(n->neighbor) == v2) && (next_node(n->neighbor) == v1)) return true;
  } else { // Vertex to vertex
    if (n->neighbor == v1 || n->neighbor == v2) return true;
  }
  return false;
}
                  ...
                  node *neighbor, nodeStatus st, int entry, int visited, double alpha)
{
  ...
  nw->next = next;
  nw->prev = prev;
  if(prev) nw->prev->next = nw;
  if(next) nw->next->prev = nw;
...
Graph::intersectionExist (node* p1, node* p2, node* q1, node* q2)
{
  ...
  while (aux != q2) {
    if (aux->status==NS_Intersection) 
      if ((aux->neighbor->prev ==p1)&&(aux->neighbor->next==p2)) return true;
    aux=aux->next;
  }
  return false;
}
...
{
  if (dist(p1->x, p1->y, p2->x, p2->y) <= GT_EPS) return true;
  else return false;
}
...
Graph::vertex2IntersectionVertex (node*v, node*l1, node*l2)
{
  ...
  if (v->status == NS_IntersectionVertex) {
    if ((v->neighbor->status == NS_IntersectionVertex)) {
      if ((v->neighbor == l1)||(v->neighbor ==l2)) return 0; 
      else {
...
Graph::testNewIntersectionVertexEdgeCollapse (node*v, node*l1, node *l2)
{
  ...
  if (l1->status==NS_IntersectionVertex && l1->neighbor->status==NS_Intersection) {
    ...
    if (vp->neighbor == l1)  vp_par = 0.;
    else if (vp->neighbor == l2) vp_par = 1.;
    else vp_par=vp->neighbor->alpha;

    if (v->neighbor == l1) v_par = 0.;
    else if (v->neighbor == l2) v_par = 1.;
    else v_par = v->neighbor->alpha;

    if (v_par < vp_par) v->neighbor->entry=-1;
    else vp->neighbor->entry=-1;

  } else if (belongs(next_node(v->next), l1,l2)) {
    node *vn=next_node(v->next);
    v->entry = -1;

    if (vn->neighbor == l1)  vn_par = 0.;
    else if (vn->neighbor == l2) vn_par = 1.;
    else vn_par=vn->neighbor->alpha;

    if (v->neighbor == l1) v_par = 0.;
    else if (v->neighbor == l2) v_par = 1.;
    else v_par = v->neighbor->alpha;

    if (v_par < vn_par) v->neighbor->entry=-1;
    else vn->neighbor->entry=-1;

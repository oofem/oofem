...
class Vertex 
{
 public:
  FloatArray coords;
  Vertex (double x=0, double y=0): coords(2) {coords(0)=x; coords(1)=y;}
  Vertex (double c[2]): coords(2) {coords(0)=c[0]; coords(1)=c[1];}
  Vertex (const Vertex&src) {coords=src.coords;}
  Vertex& operator= (const Vertex& src) {coords=src.coords; return *this;}
  void setCoords (double x, double y) {coords(0)=x; coords(1)=y;}
...
class Polygon 
{
 ...
 public:
  class PolygonEdgeIterator
	...
	if (iter == p->vertices.end()) {last=true;} else {curr=(*iter); ptr=p; last=false;}}
  ...
  class PolygonVertexIterator
    {
      ...
      std::list<Vertex>::const_iterator iter;
    public:
      PolygonVertexIterator (const Polygon* p): iter() {iter=p->vertices.begin(); ptr=p;}
      void init (const Polygon* p) {iter=p->vertices.begin(); ptr=p;}
      int giveNext (Vertex& p1) {if (iter==ptr->vertices.end()) return 0; else p1=(*iter); ++iter; return 1;}
      int giveNext (const Vertex** p1) {if (iter==ptr->vertices.end()) return 0; else *p1=iter.operator->(); ++iter; return 1;}

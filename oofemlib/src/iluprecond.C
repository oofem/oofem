/* $Header: /home/cvs/bp/oofem/oofemlib/src/iluprecond.C,v 1.3.4.1 2004/04/05 15:19:43 bp Exp $ */
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*             ********   ***                                 SparseLib++    */
/*          *******  **  ***       ***      ***               v. 1.5c        */
/*           *****      ***     ******** ********                            */
...
CompCol_ILUPreconditioner::init (const SparseMtrx& A)
{
 ...
 if (A.giveType() == SMT_CompCol) this->initialize (*((CompCol*)&A));
 else if (A.giveType() == SMT_DynCompCol) this->initialize (*((DynCompCol*)&A));
 else {
...
CompCol_ILUPreconditioner::qsortRowPartition (IntArray& src, FloatArray& val, int l, int r)
{
 ...
 for (;;) {
  ...
  if (i >= j) break;
  swap = src(i); src(i) = src(j); src(j) = swap;
  dswap= val(i); val(i) = val(j); val(j) = dswap;
 }
 swap = src(i); src(i) = src(r); src(r) = swap;

/* $Header: /home/cvs/bp/oofem/oofemlib/src/Attic/communicator.C,v 1.1.2.1 2004/04/05 15:19:43 bp Exp $ */
/*
...
{
 int i, result=1;
 for  (i=0; i<size; i++) result &= this->giveProcessCommunicator(i)->initExchange (tag);
...
{
 int i, result=1;
 for  (i=0; i<size; i++) result &= this->giveProcessCommunicator(i)->initSend (tag);
...
{
 int i, result=1;
 for  (i=0; i<size; i++) result &= this->giveProcessCommunicator(i)->initReceive (tag);
...
{
 int i;
 for  (i=0; i<size; i++) this->giveProcessCommunicator(i)->clearBuffers();
}

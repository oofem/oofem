 ...
 */
 ...
 for (i=0; i<neq; i++) this->nz_ += columns[i].size();
...
int SymCompCol::assemble (const IntArray& rloc, const IntArray& cloc, const FloatMatrix& mat)
{
 ...
 for (i=1 ; i<= dim1; i++) { 
  ...
  if (ii) for (j=1 ; j<= dim2; j++) {
   jj = cloc.at(j);
   if (jj && (ii>=jj)) this->at(ii,jj) += mat.at(i,j);
  }
 }
...
{
 for (int t=0; t<nz_; t++) val_(t)= 0.0;
...
double& SymCompCol::at (int i, int j) 
{
 ...
 for (int t=colptr_(jj-1); t<colptr_(jj); t++)
   if (rowind_(t) == (ii-1)) return val_(t);

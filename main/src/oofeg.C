...
static char* OOFEG_layer_names[]={"GEOMETRY LAYER", "DEFORMED GEOMETRY", "NODE ANNOTATION", "VARPLOT CONTOURS", "CRACK PATTERNS", "IC-BC ANNOTATIONS", "NATURAL_BC", "SPARSE PROFILE LAYER", "DEBUG LAYER"};
...
//
  ...
  if (!inputFileFlag) ::giveInputDataFileName(inputFileName, MAX_FILENAME_LENGTH);
  OOFEMTXTDataReader dr(inputFileName);

  // extract job name
  char *pos=inputFileName, *baseNamePos = pos;
  int length = 0;
  if(inputFileName != NULL)length = (int)strlen(inputFileName);
  while(length--){
    if(*pos++ == '/')baseNamePos = pos;
  }
 ...
 //gc.setSmoother (new ZZNodalRecoveryModel(problem));
  ...
  if (oofeg_box_setup) oofeg_display_message(OOFEG_VERSION);
  updateISA(gc);
 ...
 oofeg_add_palette("< View >", parent_pane, &view_palette);
 {
  ...
  oofeg_add_palette("< Color scale >", view_palette,&color_scale_palette);
  {
   ...
   oofeg_add_palette("< Setup Grey Scale Colors>", color_scale_palette,&grey_scale_setup_palette);
   {
    ...
    ac = 0;
    XtSetArg(al[ac], XtNlabel, " grey_min (>=0.0)"); ac++;
    ...
    greyscale_min = ESIAddButton("greyscale_min_val", "", 
                   ...
                   al, ac, NULL, NULL); 
    ac = 0;
    XtSetArg(al[ac], XtNfromHoriz,greyscale_min); ac++;
    XtSetArg(al[ac], XtNlabel, " grey_max (<=1.0)"); ac++;
    ...
    ac = 0;
    XtSetArg(al[ac], XtNfromHoriz, greyscale_max); ac++;
   ...
   oofeg_add_palette("< Setup Color Scale >", color_scale_palette, &color_scale_setup_palette);
   {
    ...
    ac = 0;
    XtSetArg(al[ac], XtNlabel, " scale_min"); ac++;
    ...
    scale_min = ESIAddButton("scale_min_val", "", 
                 ...
                 al, ac, NULL, NULL); 
    ac = 0;
    XtSetArg(al[ac], XtNfromHoriz,scale_min); ac++;
    XtSetArg(al[ac], XtNlabel, " scale_max"); ac++;
    ...
    ac = 0;
    XtSetArg(al[ac], XtNfromHoriz, scale_max); ac++;
    scale_setup_ok = ESIAddButton("precinput_xyz_ok", " OK ", 
 ...
 oofeg_add_palette("< Animate/Loop steps >", activeStep_palette, &animate_setup_palette);
   {
    ...
    ac = 0;
    XtSetArg(al[ac], XtNlabel, "starting step"); ac++;
    ...
    start_step = ESIAddButton("start_step", "", 
                             ...
                             al, ac, NULL, NULL); 
    ac = 0;
    XtSetArg(al[ac], XtNfromHoriz,start_step); ac++;
    XtSetArg(al[ac], XtNlabel, " end step"); ac++;
    ...
    ac = 0;
    XtSetArg(al[ac], XtNfromHoriz, end_step); ac++;
...
void OOFEGSimpleCmd( char* buf)
{
 ...
 int istep, pstep, iversion = 0;
  ...
  if (strtod (remain,NULL) < 0 ) gc[0].setDefScale (1.0);
...
void  defAutoScale(Widget wid, XtPointer cl, XtPointer cd)
{
 ...
 for (id = 1; id <= gc[OOFEG_DEFORMED_GEOMETRY_LAYER].getActiveProblem()->giveNumberOfDomains(); id++) {
  ...
  if (id==1) for (j = 1; j<=3; j++) maxcoords[j-1] = mincoords[j-1] = domain->giveNode(1)->giveCoordinate(j);
    
  for (i = 1; i<= nnodes; i++) {
   dman = domain->giveDofManager(i);
   if ((dman->giveClassID() == NodeClass) || (dman->giveClassID() == RigidArmNodeClass)) {
    if (init) {
     for (j = 1; j<=3; j++) maxcoords[j-1] = mincoords[j-1] = domain->giveNode(i)->giveCoordinate(j);
     init = 0;
    }
...
void stresscompPlot (Widget w, XtPointer ptr, XtPointer call_data)
{
 ...
 ac = *((int*) ptr);
 if (s) gc[OOFEG_VARPLOT_PATTERN_LAYER].setInternalStateType (IST_CylindricalStressTensor);
 else gc[OOFEG_VARPLOT_PATTERN_LAYER].setInternalStateType (IST_StressTensor);
 ...
 if (gc[OOFEG_VARPLOT_PATTERN_LAYER].getActiveStep() != -1) drawData(gc[OOFEG_VARPLOT_PATTERN_LAYER]);
...
void straincompPlot (Widget w, XtPointer ptr, XtPointer call_data)
{
 ...
 ac = *((int*) ptr);
 if (s) gc[OOFEG_VARPLOT_PATTERN_LAYER].setInternalStateType (IST_CylindricalStrainTensor);
 else gc[OOFEG_VARPLOT_PATTERN_LAYER].setInternalStateType (IST_StrainTensor);
 gc[OOFEG_VARPLOT_PATTERN_LAYER].setIntVarIndx (ac);
 gc[OOFEG_VARPLOT_PATTERN_LAYER].setPlotMode (OGC_scalarPlot);
...
void errorcompPlot (Widget w, XtPointer ptr, XtPointer call_data)
{
 ...
 if (ac == 1) gc[OOFEG_VARPLOT_PATTERN_LAYER].setInternalStateType (IST_ErrorIndicatorLevel);
 else if (ac == 2)  gc[OOFEG_VARPLOT_PATTERN_LAYER].setInternalStateType (IST_InternalStressError);
 else if (ac == 3)  gc[OOFEG_VARPLOT_PATTERN_LAYER].setInternalStateType (IST_PrimaryUnknownError); 
 else { deleteLayerGraphics(OOFEG_VARPLOT_PATTERN_LAYER); return;}
...
{ 
 ...
 ac = *((int*) ptr);
 if (ac == 1) gc[OOFEG_VARPLOT_PATTERN_LAYER].setInternalStateMode (ISM_recovered);
 else if (ac == 2) gc[OOFEG_VARPLOT_PATTERN_LAYER].setInternalStateMode (ISM_local);
...
void varPlot (Widget w, XtPointer ptr, XtPointer call_data) 
{
  ...
  if (mode == damageLevel) {
    ...
    if (gc[OOFEG_CRACK_PATTERN_LAYER].getActiveStep() != -1) drawData(gc[OOFEG_CRACK_PATTERN_LAYER]);
...
#define COLOR_SCALE_NUM_LABELS 11
 ...
 if (fabs(max - min) < 1.e-20 ) max += 1.e-20;
 //ColorSetupFringeTableByMinMax(ft,min,max);
 deleteLayerGraphics(OOFEG_VARPLOT_PATTERN_LAYER);
...
pass_setgreyscale_command(Widget w, XtPointer ptr, XtPointer call_data)
{
 ...
 if (fabs(min) < 0.0) min = 0.0;
 if ((max  <0.0) || (max > 1.0)) max = 1.0;
...
void drawData (oofegGraphicContext& gc)
{
 ...
 if (gc.giveIntVarPlotMode() == OGC_unknown) return;
...
updateISA (oofegGraphicContext* context)
{
  ...
  if (ct!=NULL) time = ct->giveTime(); else time = 0.0;
  if (GetISASize() > 2) {
...
view_toggleStatus_on_off(EView *v_p, caddr_t data, WCRec *p)
{
  ...
  if (status==NO)EVShowStatus(v_p, YES);
  else EVShowStatus(v_p, NO);
}
  ...
  for (int i=0; i<OOFEG_LAST_LAYER; i++) {
    if (gc[i].isActive()) {
      if (gc[i].giveIntVarPlotMode() != OGC_unknown) {
        if (i==OOFEG_VARPLOT_PATTERN_LAYER) setupData (gc[i]); // setup data only for varplots
        drawData (gc[i]);
      }

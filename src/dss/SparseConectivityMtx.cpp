...
SparseConectivityMtxII::SparseConectivityMtxII(const SparseMatrixF& sm,Ordering* node_order,char block_size)
{
	...
	for (ULONG j=0; j<sm.neq; j++)
	{
		...
		if (bj != old_bj)
		{
			if (this_columnJ) this_columnJ->SetIndexesTo(pmask,0);
			this_columnJ = ColumnsIndexes[bj];
			old_bj = bj;
		}
...
IntArrayList* SparseConectivityMtxII::GetOrder_Cuthill_McKee2()
{
	...
	do
	{
		if (last_level_count == 0)
		{
			long r=-1,cl,min = n+1;
			for (long i=0; i<n; i++)
				if ((p_node_level[i]==0) && (cl=ColumnLength(i))<min) {min = cl;r=i;}
		...
		for (long ilr = last_level_start; ilr<next_level_start; ilr++)
		{
			...
			for (long idx = al.Count-1; idx>=0; idx--)
			{
				...
				if (p_node_level[neig]!=0) continue;
...
Ordering* SparseConectivityMtxII::GetPermutationAndPattern(Ordering::Type ord,IntArrayList* fixed)
{
	...
	else if (ord == Ordering::ReverseCuthillMcKee)
	{
		...
		Write(MT.MeasureClock(start));Write("...");
		GenerateFillInPresorted(order);
		order->cm = new SparseConectivityMtxII(*this,order);
	}        
...
MD_Qqraph::~MD_Qqraph()
{
	if (degrees) {delete [] degrees;degrees=NULL;}
	if (elements) {delete elements;elements = NULL;}
	if (amd_w) {delete [] amd_w;amd_w = NULL;}

	for (long i=0; i<n; i++)
	{
		//if (A[i]) {delete A[i];A[i] = NULL;}
		if (E[i]) {delete E[i];E[i] = NULL;}
		if (L[i]) {
			delete L[i];L[i] = NULL;
		}
		if (I[i]) {delete I[i];I[i] = NULL;}
	}
	//delete [] A; A = NULL;
	delete [] E; E = NULL;
	delete [] L; L = NULL;
...
void MD_Qqraph::ComputeAMD_w_Le_Lp(long i)
{
	...
	{
		long e = el[ie];
		if (amd_w[e]<0) amd_w[e] = L[e]->Count;
		amd_w[e]-=I[i]->Count;
...
long MD_Qqraph::ApproximateMinumumDegree(long i,IntArrayList& Lp)
{
	...
	long* pe = Eli.Items;
	{
		...
		for(idx=0; idx<Lp.Count; idx++){long j = Lp[idx]; if (pPIdx[j]==0) Lpc++;}
...
void MD_Qqraph::Eliminate(long p)
{
	...
	//A[p] = NULL;
	if (E[p]) {delete E[p];E[p] = NULL;}
	if (I[p]) {delete I[p];I[p] = NULL;}
}

bool MD_Qqraph::IsIndistinguishable(long i,long j)
{
	if (A[i]->Count!=A[j]->Count) return false;
	if (E[i]->Count!=E[j]->Count) return false;
...
void MD_Qqraph::SupervariablesDetection(IntArrayList& Lp)
{
	...
	for (idx = ht.occupied_buckets.Count-1; idx>=0; idx--)
	{
		...
		if (al.Count>=2)
		{
			...
			{
				long i=al[ix];
				if (hash_parents[ix]<0) continue;
				for (long jx=ix+1; jx<al.Count; jx++)
				{
					if (hash_parents[jx]<0) continue;
					long j=al[jx];
			...
			for (ix=0; ix<al.Count; ix++)
			{
				if (hash_parents[ix]<0)
				{
					...
					variables.Remove(j);
					if (I[j]) {delete I[j];I[j] = NULL;}
					//if (A[j]) {delete A[j];A[j] = NULL;}
					//A[j] = NULL;
					if (E[j]) {delete E[j];E[j] = NULL;}
				}
			}
		}
	}
...
IntArrayList* MD_Qqraph::GenerateMD(bool approximate_degree,IntArrayList* fixed )
{
	...
	for (long k=0; k<n; k++)
	{
		I[k] = new IntArrayList(4);I[k]->Add(k);
	...
	long* pLIdx = permutation,*pPIdx = pivot_pattern,*vl = vlasts, *vn = vnexts;
	{
		...
		else
		{
			while (no_elements<n)
			{
				...
				if (no_fixed && (no_elements==n-no_fixed))
				{       // we can now unblock the fixed nodes
					delete [] pfixed;pfixed = NULL;
					no_fixed = 0;
				}

...
NumericalMethod *PFEM :: giveNumericalMethod(TimeStep *atTime)
{
    if ( nMethod ) {
        return nMethod;
    }
...
PFEM :: giveNextStep()
{
    ...
    for ( i = 1; i <= domain->giveNumberOfElements(); i++ ) {
        dt = min( dt, ( ( PFEMElement * ) domain->giveElement(i) )->computeCriticalTimeStep(previousStep) );
    }

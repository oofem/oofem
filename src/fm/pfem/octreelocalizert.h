...
#define TEMPLATED_OCTREE_MAX_DEPTH 15
	...
	public:
		...
		CellPtrType giveChild(int xi, int yi, int zi){
			if ( ( xi >= 0 ) && ( xi < 2 ) && ( yi >= 0 ) && ( yi < 2 ) && ( zi >= 0 ) && ( zi < 2 ) ) {
				...
				OOFEM_ERROR4("OctantRecT::giveChild invalid child index (%d,%d,%d)", xi, yi, zi);
			}
	...
	public:
		InsertTriangleBasedOnCircumcircle(Domain *d){domain = d;}
	...
	public:
		...
		OctreeSpatialLocalizerT (int n, Domain *d) {
			...
			domain = d;}
		/// Destructor
		~OctreeSpatialLocalizerT() { if ( rootCell ) { delete rootCell; } }
		...
		classType                giveClassID() const { return OctreeSpatialLocalizer2Class; }

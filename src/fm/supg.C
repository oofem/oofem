...

{
  ...
  if (nMethod==NULL) _error ("giveNumericalMethod: linear solver creation failed");
  return nMethod;
}
...
SUPG :: initializeFrom (InputRecord* ir)
{
 ...
 if (__val) stopmaxiter = true; else stopmaxiter=false;
 ...
 if (val) renumberFlag=true; else renumberFlag=false;
 ...
 if (val==1) {
   ...
   fm->registerField (VelocityPressureField, FT_Velocity);
...
// sets this->numberOfEquations and this->numberOfPrescribedEquations and returns this value
 ...
 for ( i=1; i <= nnodes ; i++) {
   ...
   for (j=1;j<= ndofs; j++) {
     ...
     type  =  jDof->giveDofID();
     if ((type == V_u) || (type == V_v) || (type == V_w)) jDof->askNewEquationNumber(currStep);
   }
 }
 // and then pressures
 for ( i=1; i <= nnodes ; i++) {
...
SUPG :: giveNextStep ()
{
  ...
  if (materialInterface) dt = min(dt, materialInterface->computeCriticalTimeStep (previousStep));
  // dt *= 0.6;
...
SUPG :: solveYourselfAt (TimeStep* tStep) 
{
  ...
  double _absErrResid, err, avn=0.0, aivn=0.0, rnorm, deltaT = tStep->giveTimeIncrement();
  DofManager *node;
  Dof  *iDof;
  DofIDItem type;
  ...
  do {
    ...
    for (i=1; i<=neq;i++) {
      if(fabs(__rhs.at(i)-rhs.at(i)) > __absErr) __absErr=fabs(__rhs.at(i)-rhs.at(i));
      if(fabs((__rhs.at(i)-rhs.at(i))/rhs.at(i)) > __relErr) __relErr=fabs((__rhs.at(i)-rhs.at(i))/rhs.at(i));
    }
...
SUPG :: applyIC (TimeStep* stepWhenIcApply)
{
 ...
 if (materialInterface) this->updateElementsForNewInterfacePosition (stepWhenIcApply);
 for (j = 1; j <= nelem ; j++ ) {
...
SUPG::giveUnknownDictHashIndx (EquationID type, ValueModeType mode, TimeStep* stepN)
{
  ...
  else _error ("giveUnknownDictHashIndx: unsupported solution step");
  return 0;
}

...
{
 if (rank != 0) _error ("FETICommunicator: bad rank number, expected rank 0 for master");
}
...
FETICommunicator :: setUpCommunicationMaps (EngngModel* pm)
{
 ...
 j = 0;
 for (i=1; i<= nnodes; i++) {
  if (domain->giveDofManager (i)->giveParallelMode () == DofManager_shared) j ++;
 }

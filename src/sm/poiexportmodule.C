...
POIExportModule::exportPrimVarAs (UnknownType valID, FILE* stream, TimeStep* tStep)
{
  ...
  if (valID == DisplacementVector) type = ISVT_VECTOR;
  else if (valID == FluxVector) {
    type = ISVT_SCALAR;
  } else OOFEM_ERROR("POIExportModule::exportPrimVarAs: unsupported UnknownType");
  ...
  for (PoiIter = POIList.begin(); PoiIter!=POIList.end(); ++PoiIter) {
    ...
    if (source) {
      ...
      fprintf (stream,"%10d ", (*PoiIter).id);
      if (pv.giveSize()) for (j=1; j<= pv.giveSize() ; j++) fprintf (stream, " %15e ",pv.at(j));
      fprintf (stream, "\n");
    } else {

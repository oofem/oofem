...
AdaptiveNonLinearStatic :: solveYourselfAt(TimeStep *tStep)
{
    ...
    if ( strategy == NoRemeshing_RS ) {
        ...
        if ( result == MesherInterface :: MI_OK ) {
            ...
            OOFEM_ERROR("solveYourselfAt: MesherInterface::createMesh failed");
        }
    }
}
...
AdaptiveNonLinearStatic :: initializeAdaptiveFrom(EngngModel *sourceProblem)
{
    ...
    if ( dynamic_cast< AdaptiveNonLinearStatic * >(sourceProblem) ) {
        OOFEM_ERROR("AdaptiveNonLinearStatic :: initializeAdaptiveFrom - source problem must also be AdaptiveNonlinearStatic.");
    }
    ...
    for ( ielem = 1; ielem <= nelem; ielem++ ) {
        result &= this->giveDomain(1)->giveElementGeometry(ielem)->adaptiveMap( sourceProblem->giveDomain(1),
    ...
    for ( ielem = 1; ielem <= nelem; ielem++ ) {
        result &= this->giveDomain(1)->giveElementGeometry(ielem)->adaptiveUpdate(currentStep);
    }

    // finish mapping process
    for ( ielem = 1; ielem <= nelem; ielem++ ) {
        result &= this->giveDomain(1)->giveElementGeometry(ielem)->adaptiveFinish(currentStep);
    }
    ...
    if ( equilibrateMappedConfigurationFlag ) {
        ...
        if ( initFlag ) {
            ...
            if ( stiffnessMatrix == NULL ) {
                OOFEM_ERROR("proceedStep: sparse matrix creation failed");
            }

            if ( nonlocalStiffnessFlag ) {
                if ( !stiffnessMatrix->isAsymmetric() ) {
                    OOFEM_ERROR("proceedStep: stiffnessMatrix does not support asymmetric storage");
                }
            }
...
AdaptiveNonLinearStatic :: adaptiveRemap(Domain *dNew)
{
    ...
    for ( ielem = 1; ielem <= nelem; ielem++ ) {
        ...
        result &= this->giveDomain(2)->giveElementGeometry(ielem)->adaptiveMap( this->giveDomain(1), this->giveCurrentStep() );
    }
    ...
    for ( ielem = 1; ielem <= nelem; ielem++ ) {
        ...
        result &= this->giveDomain(1)->giveElementGeometry(ielem)->adaptiveUpdate( this->giveCurrentStep() );
    }
    ...
    for ( ielem = 1; ielem <= nelem; ielem++ ) {
        ...
        result &= this->giveDomain(1)->giveElementGeometry(ielem)->adaptiveFinish( this->giveCurrentStep() );
    }
    ...
    for ( int imstep = 1; imstep < mStepNum; imstep++ ) {
        ...
        if ( hasfixed ) {
            ...
            if ( ir->hasField(_IFT_AdaptiveNonLinearStatic_ddm) ) {
                OOFEM_ERROR("assembleInitialLoadVector: fixload recovery not supported for direct displacement control");
            }
            ...
            if ( mode == ( int ) nls_directControl ) { // and only load control
                ...
                OOFEM_ERROR("assembleInitialLoadVector: fixload recovery not supported");
            }
        }
 ...
 * OOFEM_ERROR ("assembleCurrentTotalLoadVector: fixload recovery not supported for direct displacement control");
 * int _val = 0;
 ...
 * loadVectorOfPrescribed.add(_incrementalLoadVectorOfPrescribed);
 * } else {
 * OOFEM_ERROR ("assembleCurrentTotalLoadVector: totalload recovery not supported");
 * }
 *
 *
 * }

...
ZZErrorEstimator::estimateError (EE_ErrorMode mode, TimeStep* tStep)
{
 ...
 if (mode == temporaryEM)type = IST_StressTensorTemp; // _error ("estimateError: temporaryEM mode not supported");
 ...
 for (ielem=1; ielem<=nelems; ielem++) {
  ...
  if (this->skipRegion(this->domain->giveElement(ielem)->giveRegionNumber())) continue;
...
ZZErrorEstimator::giveElementError (EE_ErrorType type, Element* elem, TimeStep* tStep)
{
 ...
 if (this->skipRegion(elem->giveRegionNumber())) return 0.0;
 
 this->estimateError (equilibratedEM, tStep);

#ifdef ZZErrorEstimator_ElementResultCashed
 if (type == internalStressET) return this->eNorms.at(elem->giveNumber());
#else
 ZZErrorEstimatorInterface* interface;
...
ZZErrorEstimator::giveValue (EE_ValueType type, TimeStep* tStep) 
{
 ...
 else if (type == globalNormEEV) return this->globalSNorm;
 else return 0.0;
}
...
ZZErrorEstimator :: initializeFrom (InputRecord* ir)
{
 ...
 if (n==1)  this->normType = EnergyNorm;
 else this->normType = L2Norm; // default

 n = 0;
 IR_GIVE_OPTIONAL_FIELD (ir, n, IFT_ZZErrorEstimator_recoverytype, "recoverytype"); // Macro
 if (n==1)  this->nodalRecoveryType = SPRRecovery;
 else this->nodalRecoveryType = ZZRecovery; // default
                                     ...
                                     TimeStep* tStep)
{
 ...
 for (i=1; i<=elem->giveNumberOfNodes(); i++) {
  ...
  for (j=1; j<=size; j++) nodalRecoveredStreses.at(i,j) = recoveredStress->at(j);
 }
...
ZZRemeshingCriteria::estimateMeshDensities (TimeStep* tStep)
{
 ...
 for (i=0; i<nnode; i++) dofManInitFlag[i]=0;
 ...
 } else _error ("estimateMeshDensities: unknown mode");
 ...
 for (i = 1; i <= nelem; i ++) {
  ...
  if (this->ee->skipRegion(ielem->giveRegionNumber())) continue;
  ...
  if (fabs(iratio) < 1.e-3) continue;
  if (iratio > 1.0)this->remeshingStrategy = RemeshingFromPreviousState_RS;
  if (iratio < 0.5) iratio = 0.5;
//  if (iratio > 5.0)iratio = 5.0;
 ...
 for (i=0; i<nnode; i++) 
  if (dofManInitFlag[i]==0) this->nodalDensities.at(i+1) = this->giveDofManDensity (i+1);
 // remember time stamp
 stateCounter = tStep->giveSolutionStateCounter();
 return 1;
}

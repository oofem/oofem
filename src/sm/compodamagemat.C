...
IRResultType CompoDamageMat :: initializeFrom(InputRecord *ir)
{
    ...
    if ( this->inputTension.giveSize() != 12 ) {
        OOFEM_ERROR("instanciateFrom: need 12 components for tension in pairs f0 Gf for all 6 directions");
    }

    if ( this->inputCompression.giveSize() != 12 ) {
        OOFEM_ERROR("instanciateFrom: need 12 components for compression in pairs f0 Gf for all 6 directions");
    }

    for ( i = 1; i <= 12; i += 2 ) {
        if ( this->inputTension.at(i) < 0.0 ) {
            OOFEM_ERROR("instanciateFrom: negative f0 detected for tension");
        }

        if ( this->inputCompression.at(i) > 0.0 ) {
            OOFEM_ERROR("instanciateFrom: positive f0 detected for compression");
        }
    }
...
void CompoDamageMat :: giveRealStressVector(FloatArray &answer, GaussPoint *gp, const FloatArray &totalStrain, TimeStep *tStep)
{
    ...
    ElementGeometry *elementGeometry = gp->giveElementGeometry();
    ...
    {
        if ( !elementGeometry->giveLocalCoordinateSystem(elementCs) ) { //lcs undefined
    ...
    { //applies only for 1D, strain vectors are already in local c.s.
        if ( st->elemCharLength.at(1) == 0. ) {
            FloatArray normal(0);
            st->elemCharLength.at(1) = gp->giveElementGeometry()->giveCharacteristicLenght(gp, normal); //truss length
            this->checkSnapBack(gp, mMode);
        }
    ...
    for ( int i = 1; i <= i_max; i++ ) {
        ...
        if ( ( fabs( tempStressVectorL.at(i) ) > fabs( ( * inputFGf ).at(2 * i - 1) ) ) && ( st->strainAtMaxStress.at(i + s) == 0. ) && ( st->Iteration > this->afterIter ) ) { //damage initiated now, can be replaced for more advanced initiation criteria, e.g. Hill's maximum combination of stresses
            ...
            if ( Gf_tmp < 0. ) {
                OOFEM_WARNING("Too large initiation trial stress in element %d, component %d, |%f| < |%f|=f_t, negative remaining Gf=%f, treated as a snap-back", gp->giveElementGeometry()->giveNumber(), s == 0 ? i : -i, st->initDamageStress.at(i + s), tempStressVectorL.at(i), Gf_tmp);
            ...
            if ( fabs( st->maxStrainAtZeroStress.at(i + s) ) < fabs( st->strainAtMaxStress.at(i + s) ) && st->hasSnapBack.at(i) == 0 ) {
                OOFEM_WARNING( "Snap-back occured in element %d, component %d, |elastic strain=%f| > |fracturing strain %f|", gp->giveElementGeometry()->giveNumber(), s == 0 ? i : -i, st->strainAtMaxStress.at(i + s), st->maxStrainAtZeroStress.at(i + s) );
                st->hasSnapBack.at(i) = 1;
            }
        }
...
{
    FloatMatrix t(3, 3);
    StructuralElement *element = static_cast< StructuralElement * >( gp->giveElementGeometry() );
    MaterialMode mMode = gp->giveMaterialMode();
...
void CompoDamageMat :: giveCharLength(CompoDamageMatStatus *status, GaussPoint *gp, FloatMatrix &elementCs)
{
    ...
    for ( int i = 1; i <= 3; i++ ) {
        ...
        status->elemCharLength.at(i) = gp->giveElementGeometry()->giveCharacteristicLenght(gp, crackPlaneNormal);
    }
}
            ...
            for ( int i = 1; i <= 6; i++ ) {
                ...
                if ( elem_h > 2 * l_ch ) {
                    if ( this->allowSnapBack.contains(i + 6 * j) ) {
                        OOFEM_LOG_INFO("Allowed snapback of 3D element %d GP %d Gf(%d)=%f, would need Gf>%f\n", gp->giveElementGeometry()->giveNumber(), gp->giveNumber(), j == 0 ? i : -i, Gf, ft * ft * elem_h / 2 / modulus);
                    } else {
                        OOFEM_ERROR("Decrease size of 3D element %d or increase Gf(%d)=%f to Gf>%f, possible snap-back problems", gp->giveElementGeometry()->giveNumber(), j == 0 ? i : -i, Gf, ft * ft * elem_h / 2 / modulus);
                    }
                }
            }
        ...
        case _1dMat:
            ...
            if ( elem_h > 2 * l_ch ) {
                ...
                if ( this->allowSnapBack.contains(1 + 6 * j) ) {
                    OOFEM_LOG_INFO("Allowed snapback of 1D element %d GP %d Gf(%d)=%f, would need Gf>%f\n", gp->giveElementGeometry()->giveNumber(), gp->giveNumber(), j == 0 ? 1 : -1, Gf, ft * ft * elem_h / 2 / modulus);
                } else {
                    OOFEM_ERROR("Decrease size of 1D element %d or increase Gf(%d)=%f to Gf>%f, possible snap-back problems", gp->giveElementGeometry()->giveNumber(), j == 0 ? 1 : -1, Gf, ft * ft * elem_h / 2 / modulus);
                }
            }
...
void CompoDamageMatStatus :: printOutputAt(FILE *file, TimeStep *tStep)
{
    ...
    fprintf( file, " Cross section num %d", gp->giveElementGeometry()->giveCrossSection()->giveNumber() );

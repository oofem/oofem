...
ConcreteDPM2Status :: saveContext(DataStream *stream, ContextMode mode, void *obj)
{
    ...
    if ( !stream->write(& deltaLambda, 1) ) {
        THROW_CIOERR(CIO_IOERR);
    }

    if ( !stream->write(& dFDKappa, 1) ) {
        THROW_CIOERR(CIO_IOERR);
    }
    ...
    if ( !stream->write(& rateFactor, 1) ) {
        THROW_CIOERR(CIO_IOERR);
    }

    if ( !stream->write(& rateStrain, 1) ) {
        THROW_CIOERR(CIO_IOERR);
    }
...
ConcreteDPM2Status :: restoreContext(DataStream *stream, ContextMode mode, void *obj)
{
    ...
    if ( !stream->read(& deltaLambda, 1) ) {
        THROW_CIOERR(CIO_IOERR);
    }

    if ( !stream->read(& dFDKappa, 1) ) {
        THROW_CIOERR(CIO_IOERR);
    }
    ...
    if ( !stream->read(& rateFactor, 1) ) {
        THROW_CIOERR(CIO_IOERR);
    }

    if ( !stream->read(& rateStrain, 1) ) {
        THROW_CIOERR(CIO_IOERR);
    }

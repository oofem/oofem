...
//
{
 ...
 answer.resize(1);
 if (tempKappa <= 1.e-12) answer.at(1) = 0.0;
 else answer.at(1) = (this->computeBondForce (tempKappa)/tempKappa) * slip;
...
//
{
 //MaterialMode mode  = gp -> giveMaterialMode ();
  ...
  if (mmode == _1dInterface) return ind;
...
{ 
 MaterialMode mode = gp-> giveMaterialMode ();
 if (mode == _1dInterface) {answer = strainVector; return; }

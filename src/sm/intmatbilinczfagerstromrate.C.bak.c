                                                     ...
                                                     const FloatMatrix &F, TimeStep *tStep)
{
    ...
    if (oldDamage < 0.99) {
		...
		if (Qt+Qn_M>0) {
			beta = (pow(Qt,2)/(pow(Qt,2) + pow(gamma*Qn_M,2)));
			S = pow(gamma,2)*sigf*(beta*this->GIIc + (1-beta)*this->GIc)/(pow(gamma*Qn_M,2) + pow(Qt,2));
		}
...
//		}
        ...
        if (loadFun/sigf < 0.0000001) {
            ...
            for( int iter = 1; fabs(loadFunDyn)/sigf > errorTol; iter++) {
                ...
                R.at(1) = dJElastic.at(1) - (dJ.at(1) - S*dAlpha*M.at(1));		// R(1:2) = dJtn_e(1:2) - (dJtn_v(1:2) - S*dalpha*M(1:2))
                R.at(2) = dJElastic.at(2) - (dJ.at(2) - S*dAlpha*M.at(2));
                ...
                Smat.at(1,1) = 1.0 + dAlpha*S*2*Kstiff.at(1,1)/(pow(gamma,2)*sigf);		// S_mat(1:2,1:2) = eye3(1:2,1:2)+ dalpha*S*dMdJtn_e(1:2,1:2)
                Smat.at(2,2) = 1.0 + dAlpha*S*2*Kstiff.at(2,2)/(pow(gamma,2)*sigf);				
                ...
                Smat.at(1,4) = S*M.at(1);			// S_mat(1:2,3) = S*M(1:2)
                Smat.at(2,4) = S*M.at(2);
...
{
    if ( this->kn0 < 0.0 ) {
        OOFEM_ERROR2("IntMatBilinearCZFagerstromRate :: initializeFrom - stiffness kn0 is negative (%.2e)", this->kn0);
    } else if ( this->ks0 < 0.0 ) {
        OOFEM_ERROR2("IntMatBilinearCZFagerstromRate :: initializeFrom - stiffness ks0 is negative (%.2e)", this->ks0);
    } else if ( this->GIc < 0.0 ) {
        OOFEM_ERROR2("IntMatBilinearCZFagerstromRate :: initializeFrom - GIc is negative (%.2e)", this->GIc);
    } else if ( this->GIIc < 0.0 ) {
        OOFEM_ERROR2("IntMatBilinearCZFagerstromRate :: initializeFrom - GIIc is negative (%.2e)", this->GIIc);
    } else if ( this->gamma < 0.0  ) { 
        OOFEM_ERROR2("IntMatBilinearCZFagerstromRate :: initializeFrom - gamma (%.2e) is below zero which is unphysical" ,
            this->gamma);
    } 
    return 1;
}

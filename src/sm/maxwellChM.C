...
MaxwellChainMaterial :: updateEmjuModuluses(GaussPoint *gp, double atTime)
{
    ...
    if ( fabs(atTime - EmjuValTime) > TIME_DIFF ) {
        ...
        if ( atTime < 0 ) {
            this->computeCharCoeficients(EmjuVal, gp, 1.e-3);
        } else {
            this->computeCharCoeficients(EmjuVal, gp, atTime);
        }
...
MaxwellChainMaterial :: giveRelaxationTime(int i)
{
    ...
    if ( !relaxationTimes.isNotEmpty() ) {
        this->computeRelaxationTimes();
    }
...
MaxwellChainMaterialStatus :: letHiddenStressVectorBe(int i, FloatArray *newVector)
{
    ...
    if ( hiddenStreses [ i - 1 ] ) {
        delete hiddenStreses [ i - 1 ];
    }
...
//
{
    ...
    for ( i = 0; i < nMaxwellUnits; i++ ) {
        if ( hiddenStreses [ i ] == NULL ) {
            hiddenStreses [ i ] = new FloatArray(0);
        }

        if ( ( iores = hiddenStreses [ i ]->storeYourself(stream, mode) ) != CIO_OK ) {
            THROW_CIOERR(iores);
        }
    }
...
//
{
    ...
    for ( i = 0; i < nMaxwellUnits; i++ ) {
        ...
        if ( ( iores = hiddenStreses [ i ]->restoreYourself(stream, mode) ) != CIO_OK ) {
            THROW_CIOERR(iores);
        }
    }

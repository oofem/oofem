...
MaxwellChainMaterial :: giveEModulus(GaussPoint *gp, TimeStep *atTime)
{
     ...
     *
     * It is stored as "Eval" for further expected requests from other gaussPoints that correspond to the same material.
     *
    ...
    this->updateEmuModuli(gp, relMatAge + ( atTime->giveTime() - 0.5 * atTime->giveTimeIncrement() ) / timeFactor);
    for ( mu = 1; mu <= nUnits; mu++ ) {
        ...
        Emu      = this->giveEmuModulus(mu); // previously updated by updateEmuModuli
...
MaxwellChainMaterial :: updateYourself(GaussPoint *gp, TimeStep *tNow)
{
    ...
    this->updateEmuModuli(gp, relMatAge + ( tNow->giveTime() - 0.5 * tNow->giveTimeIncrement() ) / timeFactor);

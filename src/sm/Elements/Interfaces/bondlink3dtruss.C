...
#include "domain.h"
#include "../sm/Elements/Interfaces/bondlink3dtruss.h"
...
namespace oofem {
REGISTER_Element(BondLink3dTruss);

BondLink3dTruss::BondLink3dTruss(int n, Domain *aDomain) : StructuralElement(n, aDomain)
{
    numberOfDofMans = 2;
}
...
double
BondLink3dTruss::computeVolumeAround(GaussPoint *aGaussPoint)
{
...
void
BondLink3dTruss::computeBmatrixAt(GaussPoint *aGaussPoint, FloatMatrix &answer, int li, int ui)
...
void BondLink3dTruss::computeGaussPoints()
...
bool
BondLink3dTruss::computeGtoLRotationMatrix(FloatMatrix &answer)
{
    FloatMatrix lcs;
    int i, j;
...
int
BondLink3dTruss::giveLocalCoordinateSystem(FloatMatrix &answer)
{
    if ( geometryFlag == 0 ) {
        computeGeometryProperties();
    }
...
double
BondLink3dTruss::giveBondLength()
{
...
double
BondLink3dTruss::giveBondDiameter()
{
...
void
BondLink3dTruss::giveDofManDofIDMask(int inode, IntArray &answer) const
{
    answer = { D_u, D_v, D_w };
}

void
BondLink3dTruss::initializeFrom(const std::shared_ptr<InputRecord> &ir, int priority)
{
    // first call parent
    StructuralElement::initializeFrom(ir, priority);

    IR_GIVE_FIELD(ir, this->bondLength, _IFT_BondLink3dTruss_length);

    IR_GIVE_FIELD(ir, this->bondDiameter, _IFT_BondLink3dTruss_diameter);

    IR_GIVE_FIELD(ir, this->directionVector, _IFT_BondLink3dTruss_dirvector);
}


int
BondLink3dTruss::computeGlobalCoordinates(Coordinates &answer, const FloatArray &lcoords)
{
    if ( geometryFlag == 0 ) {
        computeGeometryProperties();
    }
...
void
BondLink3dTruss::computeGeometryProperties()
{
    ...
    for ( int i = 1; i <= 3; i++ ) {
        //nodeA and nodeB should be at same point
...
void
BondLink3dTruss::saveContext(DataStream &stream, ContextMode mode)
{
...
void
BondLink3dTruss::restoreContext(DataStream &stream, ContextMode mode)
{
...
void
BondLink3dTruss::giveInternalForcesVector(FloatArray &answer,
...
void
BondLink3dTruss::computeConstitutiveMatrixAt(FloatMatrix &answer, MatResponseMode rMode, GaussPoint *gp, TimeStep *tStep)
{
    answer = static_cast< StructuralInterfaceCrossSection * >( this->giveCrossSection() )->give3dStiffnessMatrix_Eng(rMode, gp, tStep);
}

void
BondLink3dTruss::computeStressVector(FloatArray &answer, const FloatArray &strain, GaussPoint *gp, TimeStep *tStep)
{
...
void
BondLink3dTruss::drawYourself(oofegGraphicContext &gc, TimeStep *tStep)
{
...
void BondLink3dTruss::drawRawGeometry(oofegGraphicContext &gc, TimeStep *tStep)
{
    GraphicObj *go;
...
void BondLink3dTruss::drawDeformedGeometry(oofegGraphicContext &gc, TimeStep *tStep, UnknownType type)
{
    GraphicObj *go;

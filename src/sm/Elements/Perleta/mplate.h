...
#define _IFT_MPlate_Name "mplate"
 ...
 * Element is taken from an article FINITE ELEMENT STIFFNESS MATRICES FOR ANALYSIS OF PLATE BENDING
 * Stiffness matrix of the element is built from 4 different matrices
 * Unfortunately there is an error somewhere in these matrices, so this element is not used
 ...
 */
class MPlate : public StructuralElement
{
protected:
...
protected:
    ...
    virtual void computeKa(FloatMatrix &answer);
    virtual void computeKb(FloatMatrix &answer);
    virtual void computeKc(FloatMatrix &answer);
    virtual void computeKd(FloatMatrix &answer);
...
public:
    ...
    virtual const char *giveClassName() const { return "MPlate"; }
    virtual const char *giveInputRecordName() const { return _IFT_MPlate_Name; }

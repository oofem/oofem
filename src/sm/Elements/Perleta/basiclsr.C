...
#include "../sm/Elements/PlaneStress/basiclsr.h"
...
namespace oofem {
REGISTER_Element(BasicLSR);

FEI2dQuadLin BasicLSR :: interp(1, 2);

BasicLSR :: BasicLSR(int n, Domain *aDomain) : StructuralElement(n, aDomain)
{
    numberOfDofMans = 4;
...
FEInterpolation *BasicLSR :: giveInterpolation() const { return & interp; }

void
BasicLSR :: computeBmatrixAt(GaussPoint *gp, FloatMatrix &answer, int li, int ui)
//
...
void BasicLSR :: computeGaussPoints()
...
void
BasicLSR :: computeNmatrixAt(const FloatArray &iLocCoord, FloatMatrix &answer)
...
void
BasicLSR :: crossSectioncharacteristics()
        //Computes E,ni,t
{
...
void
BasicLSR :: computeLocalStiffnessMatrix(FloatMatrix &answer, MatResponseMode rMode, TimeStep *tStep)
...
void
BasicLSR :: computeStiffnessMatrix(FloatMatrix &answer, MatResponseMode rMode, TimeStep *tStep)
// Returns the stiffness matrix of the receiver, expressed in the global
// axes. No integration over volume done, beam with constant material and crosssection
// parameters assumed.
{
...
void
BasicLSR :: computeClampedStiffnessMatrix(FloatMatrix &answer,
...
double
BasicLSR :: computeLength()
// Returns the edge lengths of the receiver.
{
    double  x12 , y12, x14, y14 ;
    Node *nodeA, *nodeB,*nodeD;

    if ( a+b == 0. ) {
        nodeA   = this->giveNode(1);
        nodeB   = this->giveNode(2);
        nodeD   = this->giveNode(4);
          x12   = nodeB->giveCoordinate(1) - nodeA->giveCoordinate(1);
          y12   = nodeB->giveCoordinate(2) - nodeA->giveCoordinate(2);
          x14   = nodeD->giveCoordinate(1) - nodeA->giveCoordinate(1);
          y14   = nodeD->giveCoordinate(2) - nodeA->giveCoordinate(2);
...
BasicLSR :: computeLCS()
{
    lcsMatrix.resize(2, 2); // Note! G -> L transformation matrix
...
bool
BasicLSR :: computeGtoLRotationMatrix(FloatMatrix &answer)
{
    this->computeLCS();
    answer.resize(8, 8);
    answer.zero();
...
void
BasicLSR :: giveDofManDofIDMask(int inode, IntArray &answer) const
{
    answer = {
        D_u, D_v
    };
...
void
BasicLSR :: updateLocalNumbering(EntityRenumberingFunctor &f)
{

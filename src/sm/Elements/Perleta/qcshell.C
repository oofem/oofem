...
#include "../sm/Elements/Perleta/qcshell.h"
...
namespace oofem {
REGISTER_Element(qcshell);

FEI2dQuadLin qcshell :: interp(1, 2);

qcshell :: qcshell(int n, Domain *aDomain) : StructuralElement(n, aDomain),
...
FEInterpolation *qcshell :: giveInterpolation() const { return & interp; }

void
qcshell :: computeBmatrixAt(GaussPoint *gp, FloatMatrix &answer, int li, int ui)
//
...
void qcshell :: computeGaussPoints()
...
void
qcshell :: crossSectioncharacteristics()
        //Computes E,ni,t
{
...
void
qcshell :: computeLocalStiffnessMatrix(FloatMatrix &answer, MatResponseMode rMode, TimeStep *tStep)
...
void
qcshell :: computeStiffnessMatrix(FloatMatrix &answer, MatResponseMode rMode, TimeStep *tStep)
// Returns the stiffness matrix of the receiver, expressed in the global
// axes. No integration over volume done, beam with constant material and crosssection
// parameters assumed.
{
...
void
qcshell :: computeClampedStiffnessMatrix(FloatMatrix &answer,
...
// membrane part of stiffness matrix:
     ...
     answer.at(1,1)= (2*b2*ni2+3*a2*ni-8*b2-3*a2)* tE24ni;     // (1,1)
     answer.at(1,2)= - tE8ni;                             // (1,2)
     answer.at(1,7)= -(2*b2*ni2-3*a2*ni-8*b2+3*a2)* tE24ni;    // (1,3)
     answer.at(1,8)=  -(3*ni-1)* tE8ni2;               // (1,4)
     answer.at(1,13)=  (2*b2*ni2-3*a2*ni+4*b2+3*a2)* tE24ni;   // (1,5)
     answer.at(1,14)= tE8ni;                              // (1,6)
     answer.at(1,19)= -(2*b2*ni2+3*a2*ni+4*b2-3*a2)* tE24ni;      // (1,7)
     answer.at(1,20)=  -answer.at(1,8);                // (1,8)

     answer.at(2,2)= (2*a2*ni2+3*b2*ni-3*b2-8*a2)* tE24ni;     // (2,2)
     answer.at(2,7)= -answer.at(1,8);                // (2,3)
     answer.at(2,8)= -(2*a2*ni2+3*b2*ni-3*b2+4*a2)* tE24ni;      // (2,4)
     answer.at(2,13)= tE8ni;                              // (2,5)
     answer.at(2,14)= (2*a2*ni2-3*b2*ni+3*b2+4*a2)* tE24ni;   // (2,6)
     answer.at(2,19)= answer.at(1,8);               // (2,7)
     answer.at(2,20)= -(2*a2*ni2-3*b2*ni+3*b2-8*a2)* tE24ni;    // (2,8)


     answer.at(7,7)= answer.at(1,1);     // (3,3)
     answer.at(7,8)= tE8ni;                              // (3,4)
     answer.at(7,13)=  answer.at(1,19);      // (3,5)
     answer.at(7,14)= answer.at(1,8);               // (3,6)
     answer.at(7,19)= answer.at(1,13);   // (3,7)
     answer.at(7,20)= - tE8ni;                             // (3,8)

     answer.at(8,8)= answer.at(2,2);     // (4,4)
     answer.at(8,13)= -answer.at(1,8);                // (4,5)
     answer.at(8,14)= answer.at(2,20);    // (4,6)
     answer.at(8,19)= - tE8ni;                             // (4,7)
     answer.at(8,20)= answer.at(2,14);   // (4,8)



     answer.at(13,13)= answer.at(1,1);     // (5,5)
     answer.at(13,14)= - tE8ni;                             // (5,6)
     answer.at(13,19)= answer.at(1,7);    // (5,7)
     answer.at(13,20)= answer.at(1,8);               // (5,8)

     answer.at(14,14)= answer.at(2,2);     // (6,6)
     answer.at(14,19)= -answer.at(1,8);                // (6,7)
     answer.at(14,20)= answer.at(2,8);      // (6,8)



     answer.at(19,19)= answer.at(1,1);     // (7,7)
     answer.at(19,20)=  tE8ni;                              // (7,8)

     answer.at(20,20)= answer.at(2,2);     // (8,8)
...
void
qcshell :: initializeFrom(InputRecord &ir)
{
    numberOfGaussPoints = 4;
...
bool
qcshell :: computeGtoLRotationMatrix(FloatMatrix &answer)
{
    this->computeLCS();
    answer.resize(24, 24);
    answer.zero();
...
int
qcshell :: computeLoadGToLRotationMtrx(FloatMatrix &answer)
...
void
qcshell :: giveDofManDofIDMask(int inode, IntArray &answer) const
{
    answer = {
...
void
qcshell :: updateLocalNumbering(EntityRenumberingFunctor &f)
{

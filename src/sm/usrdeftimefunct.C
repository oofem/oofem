   ...
   // tion between two points).
{
 ...
 if (err) _error ("at: parser syntax error");
 return result;
}
   ...
   // tion between two points).
{
 ...
 if (dfdtExpression[0] == '\0') {_error("derAt: derivative not provided"); return 0.;}
 char buff [UserDefinedLoadTimeFunction_MAX_EXPR_LENGTH+20];
 sprintf (buff, "t=%e;%s", time, dfdtExpression);
 result = myParser.eval (buff, err);
 if (err) _error ("derAt: parser syntax error");
 return result;
}
   ...
   // tion between two points).
{
 ...
 if (d2fdt2Expression[0] == '\0') {_error("derAt: derivative not provided"); return 0.;}

 char buff [UserDefinedLoadTimeFunction_MAX_EXPR_LENGTH+20];
 sprintf (buff, "t=%e;%s", time, d2fdt2Expression);
 result = myParser.eval (buff, err);
 if (err) _error ("accelAt: parser syntax error");
 return result;
}
...
UserDefinedLoadTimeFunction :: initializeFrom (InputRecord* ir)
{
 ...
 if (result != IRRT_OK) IR_IOERR (giveClassName(), __proc, IFT_UserDefinedLoadTimeFunction_ft, "f(t)", ir, result);

...
namespace oofem {
// constructor
DruckerPragerMat :: DruckerPragerMat(int n, Domain *d) : StructuralMaterial(n, d)
{
...
// destructor
DruckerPragerMat :: ~DruckerPragerMat()
{ }

// specifies whether a given material mode is supported by this model
int
DruckerPragerMat :: hasMaterialModeCapability(MaterialMode mode)
{
...
IRResultType
DruckerPragerMat :: initializeFrom(InputRecord *ir)
{
...
MaterialStatus *
DruckerPragerMat :: CreateStatus(GaussPoint *gp) const
{
    DruckerPragerMatStatus *status;
    status = new DruckerPragerMatStatus(1, this->giveDomain(), gp);
    return status;
}
...
void
DruckerPragerMat :: giveRealStressVector(FloatArray &answer,
...
void
DruckerPragerMat :: giveRealStressVectorComputedFromStrain(FloatArray &answer,
                                                   MatResponseForm form,
                                                   GaussPoint *gp,
                                                   const FloatArray &totalStrain,
                                                   TimeStep *atTime)
{
    DruckerPragerMatStatus *status = ( DruckerPragerMatStatus * ) this->giveStatus(gp);
    MaterialMode mode = gp->giveMaterialMode();
...
void DruckerPragerMat :: performPlasticityReturn(GaussPoint *gp, const FloatArray &totalStrain, MaterialMode mode)
{
...
double
DruckerPragerMat :: computeDamageParam(double tempKappa)
{
    double tempDam;
    if ( tempKappa > 0. ) {
...
double
DruckerPragerMat :: computeDamageParamPrime(double tempKappa)
{
    double tempDam;
    if ( tempKappa >= 0. ) {
...
double
DruckerPragerMat :: computeDamage(GaussPoint *gp,  TimeStep *atTime)
{
...
void DruckerPragerMat :: computeCumPlastStrain(double &tempKappa, GaussPoint *gp, TimeStep *atTime)
{
    DruckerPragerMatStatus *status = ( DruckerPragerMatStatus * ) this->giveStatus(gp);
...
void
DruckerPragerMat :: give3dMaterialStiffnessMatrix(FloatMatrix &answer, MatResponseForm form, MatResponseMode mode, GaussPoint *gp, TimeStep *atTime)
{
    ...
    if ( mMode == _3dMat ) {
        ...
        OOFEM_ERROR("DruckerPragerMat::give3dMaterialStiffnessMatrix : unknown material response mode");
    }
}
...
void
DruckerPragerMat :: give3dSSMaterialStiffnessMatrix(FloatMatrix &answer, MatResponseForm form,
...
void
DruckerPragerMat :: give1dStressStiffMtrx(FloatMatrix &answer, MatResponseForm form,
                                  MatResponseMode mode,
                                  GaussPoint *gp,
                                  TimeStep *atTime)
{
...
void
DruckerPragerMat :: givePlaneStrainStiffMtrx(FloatMatrix &answer, MatResponseForm form,
                                     MatResponseMode mode,
                                     GaussPoint *gp,
                                     TimeStep *atTime)
{
...
int
DruckerPragerMat :: giveIPValue(FloatArray &answer, GaussPoint *aGaussPoint, InternalStateType type, TimeStep *atTime)
{
    DruckerPragerMatStatus *status = ( DruckerPragerMatStatus * ) this->giveStatus(aGaussPoint);
...
DruckerPragerMat :: giveIPValueType(InternalStateType type)
{
    if ( type == IST_PlasticStrainTensor ) {
...
int
DruckerPragerMat :: giveIntVarCompFullIndx(IntArray &answer, InternalStateType type, MaterialMode mmode)
{
    if ( type == IST_PlasticStrainTensor ) {
...
int
DruckerPragerMat :: giveIPValueSize(InternalStateType type, GaussPoint *gp)
{
    if ( type == IST_PlasticStrainTensor ) {
        MaterialMode mode = gp->giveMaterialMode();
...
DruckerPragerMatStatus :: DruckerPragerMatStatus (int n, Domain *d, GaussPoint *g) :
...
DruckerPragerMatStatus :: ~DruckerPragerMatStatus ()
{ }

void
DruckerPragerMatStatus :: printOutputAt(FILE *file, TimeStep *tStep)
{
    //int i, n;
...
void DruckerPragerMatStatus :: initTempStatus()
{
...
void
DruckerPragerMatStatus :: updateYourself(TimeStep *atTime)
{
...
contextIOResultType
DruckerPragerMatStatus :: saveContext(DataStream *stream, ContextMode mode, void *obj)
{
...
contextIOResultType
DruckerPragerMatStatus :: restoreContext(DataStream *stream, ContextMode mode, void *obj)
//

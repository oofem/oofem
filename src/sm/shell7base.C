...
{
    FloatArray E;
    this->computeStrainVector(E, gp, stepN, genEps);    // Green-Lagrange strain vector (6 components)
...
Shell7Base :: computeThicknessMappingCoeffNew(GaussPoint *gp, FloatArray &answer){
    ...
    FloatArray M, dM1(3), dM2(3), dX1(3), dX2(3);  double gam, dg1, dg2;
...
Shell7Base :: computeMassMatrixNum(FloatMatrix &answer, TimeStep *tStep){
    ...
    for ( int layer = 1; layer <= numberOfLayers; layer++ ) {
        ...
        for ( int j = 1; j <= iRuleL->getNumberOfIntegrationPoints(); j++ ) {    
            ...
            FloatArray xbar, m; double gam=0.;
        ...
        // Assemble contributions into node-ordered dofs
        answer.resize(ndofs, ndofs); answer.zero();
...
Shell7Base :: computeLinearizedStiffness(GaussPoint *gp, Material *mat, TimeStep *tStep, 
    ...
    FloatArray cartStressVector, contravarStressVector, sectionalForces;
    FloatArray m(3), dm1(3), dm2(3);
    

    //A = L^iklj * (g_k x g_l) + S^ij*I

    // Stiffness and stress in cartesian system
    FloatMatrix D, Dcart, S;
    mat->giveCharacteristicMatrix(Dcart, FullForm, TangentStiffness, gp, tStep);                // L_ijkl - cartesian system (Voigt)
    this->computeStressVector(cartStressVector, genEps, gp, mat, tStep);                        // S_ij

    // Stiffness and stress in curvilinear system
    this->transInitialCartesianToInitialContravar(gp, Dcart, D);                                // L^ijkl - curvilinear system (Voigt)
    this->transInitialCartesianToInitialContravar(gp, cartStressVector, contravarStressVector); // S^ij 
...
*/
{
    ...
    */
    FloatArray lcoords = *gp->giveCoordinates();
    FloatArray N;  FloatMatrix dNdxi;
    this->giveInterpolation()->evalN(N,         lcoords, FEIElementGeometryWrapper(this));
    this->giveInterpolation()->evaldNdxi(dNdxi, lcoords, FEIElementGeometryWrapper(this));

    // Compute each corresponding sub matrix

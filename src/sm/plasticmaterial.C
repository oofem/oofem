/* $Header: /home/cvs/bp/oofem/sm/src/plasticmaterial.C,v 1.5.4.1 2004/04/05 15:19:47 bp Exp $ */
/*
  ...
  // completely formulated in Reduced stress-strain space
{
 ...
 break;
    ...
    for (i=1; i<= strSize; i++) plasticStrainVectorR.at(i) += helpVec2.at(i);
    for (i=strSize+1; i<= totSize; i++) strainSpaceHardeningVariables.at(i-strSize)+= helpVec2.at(i);
    // delete helpVec2;
    Gamma += dGamma;
 ...
 crossSection->giveReducedCharacteristicVector(helpVec, gp, fullStressVector);
  ...
  if (Gamma > 0.)  newState = PM_Yielding; // test if plastic loading occur
  // no plastic loading - check for unloading
  else if ((state == PM_Yielding) || (state == PM_Unloading)) newState = PM_Unloading; 
  else newState = PM_Elastic;
              ...
              FloatArray* gradientVectorR)
{
  ...
  for (i=1; i<=isize; i++) answer->at(i) = oldPlasticStrainVectorR.at(i)-plasticStrainVectorR->at(i) + Gamma*gradientVectorR->at(i);
  for (i=isize+1; i<=size; i++) answer->at(i) = oldStrainSpaceHardeningVariables.at(i-isize) - strainSpaceHardeningVariables->at(i-isize) + Gamma*gradientVectorR->at(i);
                      ...
                      const FloatArray& fullStressSpaceHardeningVars)
{
 ...
 helpInverse.resize (size,size);
  ...
  for (i=1; i<=isize; i++) {
  for (j=1; j<=isize; j++) helpInverse.at(i,j) = elasticModuliInverse.at(i,j) + Gamma * gradientMatrix.at(i,j);
  for (j=isize+1; j<=size; j++) helpInverse.at(i,j) = Gamma * gradientMatrix.at(i,j);
  }
  for (i=isize+1; i<=size; i++) {
  for (j=1; j<=isize; j++) helpInverse.at(i,j) = Gamma * gradientMatrix.at(i,j);
  for (j=isize+1; j<=size; j++) helpInverse.at(i,j) = hardeningModuliInverse.at(i-isize, j-isize) + Gamma * gradientMatrix.at(i,j);
  }
                         ...
                         TimeStep* atTime)
{
 ...
 FloatArray *stressSpaceHardeningVars;
  ...
  this->computeReducedElasticModuli(elasticModuli, gp, atTime);
  if (form == ReducedForm) { answer = elasticModuli; return ;}
  else {
...
PlasticMaterialStatus :: printOutputAt  (FILE *file, TimeStep* tStep)
{
 ...
 if ((state_flag == PM_Yielding) || (state_flag == PM_Unloading)) {
   if (state_flag == PM_Yielding) fprintf(file," Yielding, ");

/* $Header: /home/cvs/bp/oofem/sm/src/mmaleastsquareprojection.C,v 1.8.4.1 2004/04/05 15:19:47 bp Exp $ */
/*
...
//(Domain* dold, IntArray& varTypes, GaussPoint* gp, TimeStep* tStep) 
{
 ...
 if (egt == EGT_line_1) this->patchType = MMALSPPatchType_1dq;
 else if ((egt == EGT_triangle_1)||(egt == EGT_quad_1)) this->patchType = MMALSPPatchType_2dq;
 else {
 ...
 */
 ...
 while ((actualNumberOfPoints < minNumberOfPoints)&&(nite <= 2)) {
  ...
  for (i=1; i<=neighborList.giveSize(); i++) {
   if (this->stateFilter) {
    ...
    if (this->regionFilter && (element->giveRegionNumber() != region)) continue;
 ...
 for (int ielem=1; ielem<= patchList.giveSize(); ielem++) {
  ...
  for (i=0; i< nip; i++) {
   ...
   } else _error ("init: computeGlobalCoordinates failed");
  }
 }
 ...
 for (i=1; i<=minNumberOfPoints; i++) {
  ...
  swap=dist.at(i); dist.at(i)=dist.at(minDistIndx); dist.at(minDistIndx)=swap;
  srcgp=gpList[i-1]; gpList[i-1]=gpList[minDistIndx-1];gpList[minDistIndx-1] = srcgp;
...
{
 if (regType == MMALSPPatchType_2dq) return 6;
 else if (regType == MMALSPPatchType_1dq) return 3;
 else return 0;
}

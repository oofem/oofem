...
{
    if ( mStep == NULL ) {
        OOFEM_ERROR("giveNumericalMethod: undefined meta step");
    }
...
NonLinearDynamic :: updateAttributes(MetaStep *mStep)
{
    ...
    if ( deltaT < 0. ) {
        OOFEM_ERROR("updateAttributes: deltaT < 0");
    }
    ...
    if ( currentStep != NULL ) {
        ...
        if ( !this->giveMetaStep(mStepNum)->isStepValid(istep) ) {
            ...
            if ( mStepNum > nMetaSteps ) {
                OOFEM_ERROR("giveNextStep: no next step available, mStepNum=%d > nMetaSteps=%d", mStepNum, nMetaSteps);
            }
        }
    }
    ...
    if ( initFlag ) {
        ...
        if ( nonlocalStiffnessFlag ) {
            if ( !effectiveStiffnessMatrix->isAsymmetric() ) {
                OOFEM_ERROR("proceedStep: effectiveStiffnessMatrix does not support asymmetric storage");
            }
        }
                                                    ...
                                                    CharType type, TimeStep *tStep, Domain *domain)
{
    ...
    if ( type == EffectiveStiffnessMatrix ) {
		...
		element->giveElementEvaluator()->giveCharacteristicMatrix(answer, TangentStiffnessMatrix, tStep);
        answer.times(1 + this->delta * a1);

        element->giveElementEvaluator()->giveCharacteristicMatrix(charMtrx, MassMatrix, tStep);
...
NonLinearDynamic :: timesMtrx(FloatArray &vec, FloatArray &answer, CharType type, Domain *domain, TimeStep *tStep)
{
    ...
    for ( int i = 1; i <= nelem; i++ ) {
        ...
        if ( elementGeometry->giveParallelMode() == Element_remote ) {
            continue;
        }
...
NonLinearDynamic :: estimateMaxPackSize(IntArray &commMap, CommunicationBuffer &buff, int packUnpackType)
{
    ...
    if ( packUnpackType == ProblemCommMode__ELEMENT_CUT ) {
        ...
        for ( int i = 1; i <= mapSize; i++ ) {
            count += domain->giveElementGeometry( commMap.at(i) )->estimatePackSize(buff);
        }

...
Subdivision :: RS_Tetra :: RS_Tetra(int number, RS_Mesh *mesh, int parent, IntArray &nodes) :
    ...
    leIndex = 1;
    maxlength = elength1;
    if ( elength2 > maxlength ) {
        leIndex = 2;
        maxlength = elength2;
    }

    if ( elength3 > maxlength ) {
        leIndex = 3;
        maxlength = elength3;
    }

    if ( elength4 > maxlength ) {
        leIndex = 4;
        maxlength = elength4;
    }

    if ( elength5 > maxlength ) {
        leIndex = 5;
        maxlength = elength5;
    }

    if ( elength6 > maxlength ) {
        leIndex = 6;
        maxlength = elength6;
    }
    ...
    mlength = 0.0;
    if ( leIndex != 1 && elength1 > mlength ) {
        mlength = elength1;
    }

    if ( leIndex != 2 && elength2 > mlength ) {
        mlength = elength2;
    }

    if ( leIndex != 3 && elength3 > mlength ) {
        mlength = elength3;
    }

    if ( leIndex != 4 && elength4 > mlength ) {
        mlength = elength4;
    }

    if ( leIndex != 5 && elength5 > mlength ) {
        mlength = elength5;
    }

    if ( leIndex != 6 && elength6 > mlength ) {
        mlength = elength6;
    }
    ...
    if ( mlength > maxlength * 0.9999999 ) {
        ...
        int nd [ 4 ] = {
            0, 1, 2, 3
        }, ed [ 4 ] [ 4 ] = { { 0, 1, 3, 4 }, { 1, 0, 2, 5 }, { 3, 2, 0, 6 }, { 4, 5, 6, 0 } };
#ifdef __PARALLEL_MODE
        int g_nd [ 4 ];
#endif
        bool swap;
        ...
        // get largest edge
        maxlength = 0.0;
        if ( elength1 > maxlength ) {
            maxlength = elength1;
            leIndex = ed [ nd [ 0 ] ] [ nd [ 1 ] ];
        }

        if ( elength2 > maxlength ) {
            maxlength = elength2;
            leIndex = ed [ nd [ 0 ] ] [ nd [ 2 ] ];
        }

        if ( elength3 > maxlength ) {
            maxlength = elength3;
            leIndex = ed [ nd [ 0 ] ] [ nd [ 3 ] ];
        }

        if ( elength4 > maxlength ) {
            maxlength = elength4;
            leIndex = ed [ nd [ 1 ] ] [ nd [ 2 ] ];
        }

        if ( elength5 > maxlength ) {
            maxlength = elength5;
            leIndex = ed [ nd [ 1 ] ] [ nd [ 3 ] ];
        }

        if ( elength6 > maxlength ) {
            maxlength = elength6;
            leIndex = ed [ nd [ 2 ] ] [ nd [ 3 ] ];
        }
    }
}
...
#ifdef __PARALLEL_MODE
    if ( this->giveParallelMode() != Element_local ) {
        return;
    }
...
Subdivision :: RS_Tetra :: bisect(std :: queue< int > &subdivqueue, std :: list< int > &sharedIrregularsQueue) {
    ...
    if ( irregular_nodes.at(leIndex) ) {
        ...
        if ( boundary ) {
            irregular->setBoundary(boundary);
        }
        ...
        if ( this->giveIrregularSharedPartitions(leIndex, inode, jnode, partitions) ) {
            ...
            for ( i = 1; i <= partitions.giveSize(); i++ ) {
                ...
                len = strlen(buffer);
                if ( len >= 1024 ) {
                    OOFEM_ERROR("Subdivision::RS_Tetra::bisect: too small buffer");
                }
            }
...
{
    if ( bEntity.giveSize() != 2 ) {
        OOFEM_ERROR("Subdivision::RS_Element::addIrregularOn: bEntity size mismatch");
    }
    ...
    if ( !eIndex ) {
        OOFEM_ERROR4("Subdivision::RS_Element::addIrregularOn: no bEntity connecting nodes %d and %d found on elem %d",
...
{
    if ( bEntity.giveSize() != 2 ) {
        OOFEM_ERROR("Subdivision::RS_Element::giveIrregularOn: bEntity size mismatch");
    }
    ...
    eIndex = this->giveEdgeIndex( bEntity.at(1), bEntity.at(2) );
    if ( eIndex ) {
        return this->irregular_nodes.at(eIndex);
    }
...
Subdivision :: RS_Element :: giveIrregularSharedPartitions(int leIndex, int inode, int jnode, IntArray &partitions)
{
    ...
    if ( ( mesh->giveNode( nodes.at(inode) )->giveParallelMode() == DofManager_shared ) &&
        ...
        if ( base_neighbor ) {
            if ( mesh->giveElement(base_neighbor)->giveParallelMode() == Element_local ) {
        ...
        if ( !has_local_neighbor ) {
            ...
            if ( j == 0 ) {
                OOFEM_ERROR("Subdivision::RS_Element::giveIrregularPartitions: topology error");
            }
        }
    }
...
Subdivision :: RS_Element :: containsGlobalNode(int gnum)
{
    ...
    for ( in = 1; in <= nnodes; in++ ) {
        if ( this->mesh->giveNode( nodes.at(in) )->giveGlobalNumber() == gnum ) {
            return in;
        }
    }
...
Subdivision :: RS_Tetra :: generate()
{
    ...
    if ( this->number < 0 ) {
        return;                        // skip tmp tetras in bisection loop 2 and more
    }
...
Subdivision :: RS_Tetra :: update_neighbours()
{
    ...
    if ( this->number < 0 ) {
        return;                        // skip tmp tetras in bisection pass 2 and more
    }
...
Subdivision :: bisectMesh() {
...
#ifdef __PARALLEL_MODE
            if ( nelems_old == 0 ) {
                if ( mesh->giveElement(ie)->giveParallelMode() == Element_remote ) {
                    continue;                                                                                 // skip remote elems in first bisection pass
                }
            }
    ...
    // in parallel communicate with neighbours the irregular nodes on shared bondary
        ...
        for ( localOffset = 0, irank = 0; irank < myrank; irank++ ) {
    ...
    while ( cycles-- ) {
        for ( jn = 1; jn <= nnodes; jn++ ) {
            ...
            if ( ( mesh->giveNode(in)->giveParallelMode() == DofManager_shared ) ||
                ...
                continue;                                                                                 // skip shared and remote
            }
...
#ifdef __PARALLEL_MODE
        if ( mesh->giveElement(ielem)->giveParallelMode() == Element_remote ) {
            continue;
        }
        ...
        if ( _type == SUBIVISION_IRREGULAR_REC_TAG ) {
            ...
            for ( found = false, ie = 1; ie <= nelems; ie++ ) {
                ...
                if ( ( inode = elem->containsGlobalNode( edgeInfo.at(1) ) ) &&
                     ...
                     */
                    bEnt.at(1) = elem->giveNode(inode);
                    bEnt.at(2) = elem->giveNode(jnode);
                    found = true;
            ...
            if ( !found ) {
                OOFEM_ERROR("Subdivision::unpackSharedIrregulars: no element found\n");
            }
        } else {
        ...
        if ( _type == SUBIVISION_IRREGULAR_REC_TAG ) {
            ...
            for ( found = false, ie = 1; ie <= nelems; ie++ ) {
                ...
                if ( ( inode = elem->containsGlobalNode( edgeInfo.at(2) ) ) &&
                    ...
                    be.at(1) = elem->giveNode(inode);
                    be.at(2) = elem->giveNode(jnode);

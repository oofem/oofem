...
int
FreemInterface::createMesh (Domain* d, TimeStep* stepN, int domainNumber, int domainSerNum)
{
  ...
  while (!queue.isEmpty()) {
   ...
   for (j=1; j<=candidateConnectivity->giveSize(); j++) {
    ...
    for (k=1; k<= jelemNodes; k++) {
     ...
     if (neighbour == candidate) continue;
     ...
     if (((nodalDensities.at(neighbour)/nodalDensities.at(candidate)) > 1.3) &&
      ...
      for (found = 0, pos = queue.begin(); pos != queue.end(); ++pos)
       if (*pos == neighbour) {found = 1; break;}
      
      if (!found) queue.pushFront(neighbour);

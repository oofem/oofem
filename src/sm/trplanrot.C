...
// Returns angles between each side and global x-axis
{
 ...
 for (i=0;i<3;i++){
  j=i+1-i/2*3;
  k=j+1-j/2*3;
  if (x[k]==x[j]) { if (y[k]>y[j])   angles->at(i+1)=3.14159265358979/2.;
  else         angles->at(i+1)=3.14159265358979*3./2.;
       }
  if (x[k]>x[j])  { if (y[k]>=y[j])  angles->at(i+1)=atan((y[k]-y[j])/(x[k]-x[j]));
  else         angles->at(i+1)=2.*3.14159265358979-atan((y[j]-y[k])/(x[k]-x[j]));
       }
  if (x[k]<x[j])  { if (y[k]>=y[j])  angles->at(i+1)=3.14159265358979-atan((y[k]-y[j])/(x[j]-x[k]));
  else         angles->at(i+1)=3.14159265358979+atan((y[j]-y[k])/(x[j]-x[k]));
       }
 }
 return angles;
}
...
TrPlaneStrRot :: GiveDerivativeUX (GaussPoint *aGaussPoint)
{
 ...
 delete angles; delete shapeFunct; delete b;  delete c; delete d;
 return nx;
}
...
// evaluated at aGaussPoint.
{
 ...
 if (ui == ALL_STRAINS) {size = 4; ui = 4;} else  size = ui-li+1;
 if ((size <0) || (size > 4)) _error ("ComputeBmatrixAt size mismatch");

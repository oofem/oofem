...
HTSelement ::  giveSideNode(int elementSideNumber, int nodeNumber)
{
	...
	return this->giveNode(secondNodeNumber);
	  ...
	  {
//__error("Only two nodes per side");
	    return 0;
	    }
}
...
HTSelement :: computeUvMatrixAt(FloatMatrix &answer, GaussPoint* gp, int sideNumber)
{
 ...
 double By =  this->giveSideNode(sideNumber,2)->giveCoordinate(2) - cgY;
  ...
  //v2
  this->uv3(uv,x,y);
  Uv(1,0) = uv(0); Uv(1,1) = uv(1);
  //v3
  this->uv4(uv,x,y);
  Uv(2,0) = uv(0); Uv(2,1) = uv(1);
  //v4
  this->uv5(uv,x,y);
  Uv(3,0) = uv(0); Uv(3,1) = uv(1);
  //v5
  this->uv6(uv,x,y);
  Uv(4,0) = uv(0); Uv(4,1) = uv(1);
  //v6
  this->uv7(uv,x,y);
  Uv(5,0) = uv(0); Uv(5,1) = uv(1);
  //v7
  this->uv8(uv,x,y);
  Uv(6,0) = uv(0); Uv(6,1) = uv(1);
  //v8
  this->uv9(uv,x,y);
  Uv(7,0) = uv(0); Uv(7,1) = uv(1);
  //v9
  this->uv10(uv,x,y);
  Uv(8,0) = uv(0); Uv(8,1) = uv(1);
  //v10
  this->uv11(uv,x,y);
  Uv(9,0) = uv(0); Uv(9,1) = uv(1);
  //v11
  this->uv12(uv,x,y);
  Uv(10,0) = uv(0); Uv(10,1) = uv(1);
  //v12
  this->uv25_4(uv,x,y);
  Uv(11,0) = uv(0); Uv(11,1) = uv(1);
...
HTSelement :: computeSvMatrixAt(FloatMatrix &answer, GaussPoint* gp, int sideNumber)
{
  ...
  //v1
  this->sv1(sv,x,y);
  Sv(0,0) = sv(0); Sv(0,1) = sv(1); Sv(0,2) = sv(2);
  //v2
  this->sv3(sv,x,y);
  Sv(1,0) = sv(0); Sv(1,1) = sv(1); Sv(1,2) = sv(2);
  //v3
  this->sv4(sv,x,y);
  Sv(2,0) = sv(0); Sv(2,1) = sv(1); Sv(2,2) = sv(2);
  //v4
  this->sv5(sv,x,y);
  Sv(3,0) = sv(0); Sv(3,1) = sv(1); Sv(3,2) = sv(2);
  //v5
  this->sv6(sv,x,y);
  Sv(4,0) = sv(0); Sv(4,1) = sv(1); Sv(4,2) = sv(2);
  //v6
  this->sv7(sv,x,y);
  Sv(5,0) = sv(0); Sv(5,1) = sv(1); Sv(5,2) = sv(2);
  //v7
  this->sv8(sv,x,y);
  Sv(6,0) = sv(0); Sv(6,1) = sv(1); Sv(6,2) = sv(2);
  //v8
  this->sv9(sv,x,y);
  Sv(7,0) = sv(0); Sv(7,1) = sv(1); Sv(7,2) = sv(2);
  //v9
  this->sv10(sv,x,y);
  Sv(8,0) = sv(0); Sv(8,1) = sv(1); Sv(8,2) = sv(2);
  //v10
  this->sv11(sv,x,y);
  Sv(9,0) = sv(0); Sv(9,1) = sv(1); Sv(9,2) = sv(2);
  //v11
  this->sv12(sv,x,y);
  Sv(10,0) = sv(0); Sv(10,1) = sv(1); Sv(10,2) = sv(2);
  //v12
  this->sv25_4(sv,x,y);
  Sv(11,0) = sv(0); Sv(11,1) = sv(1); Sv(11,2) = sv(2);

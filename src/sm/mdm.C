    ...
    for ( int ii = 1; ii <= nsd; ii++ ) {
        if ( tempDamageTensorEigenVals.at(ii) < 0.0 ) {
            ...
            sprintf( buff, "giveRealStressVector: negative eigenvalue of damage tensor detected, element %d, ip %d",
                    gp->giveElementGeometry()->giveNumber(), gp->giveNumber() );
            OOFEM_ERROR(buff);
        }
    }
                                ...
                                GaussPoint *gp, TimeStep *tStep)
{
    ...
    for ( int im = 0; im < numberOfMicroplanes; im++ ) {
        ...
        //else MicroplaneMaterial::OOFEM_ERROR ("Unknown type of formulation");
        else {
    ...
    if ( ndc == 3 ) {
        ...
        //} else MicroplaneMaterial::OOFEM_ERROR ("computeDamageTensor: unknown ndc value encountered");
    } else {
        OOFEM_ERROR("computeDamageTensor: unknown ndc value encountered");
    }
}
...
MDM :: computeDamageOnPlane(GaussPoint *gp, Microplane *mplane, const FloatArray &strain)
{
    ...
    if ( type_dam == 0 ) {
        ...
        //} else MicroplaneMaterial::OOFEM_ERROR ("Unknown type of damage law");
    } else {
    ...
    if ( type_soft == 0 ) {
        ...
        //} else MicroplaneMaterial::OOFEM_ERROR ("Unknown type of softening");
    } else {
...
MDM :: applyDamageTranformation(FloatArray &strainPDC, const FloatArray &tempDamageTensorEigenVals)
{
    if ( mdmMode == mdm_3d ) {
        ...
        if ( formulation == COMPLIANCE_DAMAGE ) {
            ...
            //MicroplaneMaterial::OOFEM_ERROR ("Unknown type of formulation");
            OOFEM_ERROR("Unknown type of formulation");
        }
    } else if ( mdmMode == mdm_2d ) {
        double psi1 = tempDamageTensorEigenVals.at(1);
        ...
        //} else MicroplaneMaterial::OOFEM_ERROR ("Unknown type of mdm mode");
    } else {
...
MDM :: applyDamageToStiffness(FloatMatrix &d, GaussPoint *gp)
{
    ...
    if ( mdmMode == mdm_3d ) {
        ...
        if ( d.giveNumberOfRows() == 6 ) {
            ...
            d.at(3, 3) /= ( psi3 * psi3 );
            d.at(4, 4) /= ( psi1 * psi2 );
            //} else MicroplaneMaterial::OOFEM_ERROR ("Unknown type stiffness");
        } else {
...
//
{
    ...
    if ( this->nonlocal ) {
        ...
        if ( ( ir->hasField(_IFT_MDM_efp) ) && ( ir->hasField(_IFT_MDM_ep) ) ) {
            ...
            OOFEM_ERROR("instanciateFrom: unknown set of parameters");
        }
    } else { // local case
        if ( ( ir->hasField(_IFT_MDM_efp) ) && ( ir->hasField(_IFT_MDM_ep) ) ) {
            // read raw_params if available
...
// a matrix from local to global coordinates.
{
    ...
    default:
        //MicroplaneMaterial::OOFEM_ERROR ("Stress transformation matrix format not implemented");
...
MDM :: giveRawMDMParameters(double &Efp, double &Ep, const FloatArray &reducedStrain, GaussPoint *gp)
{
	...
	h  = gp->giveElementGeometry()->giveLenghtInDir(dir);
        ...
        if ( Efp <= 0. ) {
            //MicroplaneMaterial::OOFEM_ERROR("Warning: negative Efp encountered");
            OOFEM_ERROR("Warning: negative Efp encountered");
        }
...
MDM :: initializeData(int numberOfMicroplanes)
{
    if ( this->mdmMode == mdm_3d ) {
        ...
        if ( numberOfMicroplanes > MAX_NUMBER_OF_MICROPLANES ) {
            //MicroplaneMaterial::OOFEM_ERROR ("initializeData: required number of microplanes too big");
            OOFEM_ERROR("initializeData: required number of microplanes too big");
        }
        ...
        //} else MicroplaneMaterial::OOFEM_ERROR ("initializeData: Unknown MDMModeType ecountered");
    } else {
        OOFEM_ERROR("initializeData: Unknown MDMModeType ecountered");
    }
}
...
MDM :: MMI_map(GaussPoint *gp, Domain *oldd, TimeStep *tStep)
{
	...
	// compile source list to contain all elements on old odmain with the same material id
        for ( int i = 1; i <= oldd->giveNumberOfElements(); i++ ) {
            if ( oldd->giveElementGeometry(i)->giveMaterial()->giveNumber() == this->giveNumber() ) {
...
{
    //if (stream == NULL) StructuralMaterialStatus::OOFEM_ERROR ("saveContex : can't write into NULL stream");
    if ( stream == NULL ) {

...
B3SolidMaterial :: computeShrinkageStrainVector(FloatArray &answer, GaussPoint *gp, TimeStep *tStep, ValueModeType mode)
{
    ...
    if ( ( tf = fm->giveField(FT_Temperature) ) ) {
        ...
        if ( ( err = tf->evaluateAt(et2, gcoords, VM_Incremental, tStep) ) ) {
            OOFEM_ERROR("computeShrinkageStrainVector: tf->evaluateAt failed, error value %d", err);
        }
    ...
    if ( ( tflag == 0 ) || ( wflag == 0 ) ) {
        OOFEM_ERROR("computeTotalShrinkageStrainVector: external fields not found");
    }
...
B3SolidMaterial :: computeMicroPrestress(GaussPoint *gp, TimeStep *tStep, int option)
{
     ...
     * } else {
     *      OOFEM_ERROR("computeMicroPrestress: invalid option parameter");
     * }
     *
     * a = c0 * alpha * deltaT;
     * b = 1.;
...
B3SolidMaterial :: giveHumidityIncrement(GaussPoint *gp, TimeStep *tStep) //computes humidity increment at given TimeStep
{
    ...
    if ( ( tf = fm->giveField(FT_HumidityConcentration) ) ) {
        ...
        gp->giveElementGeometry()->computeGlobalCoordinates( gcoords, * gp->giveCoordinates() );
        if ( ( err = tf->evaluateAt(et2, gcoords, VM_Total, tStep) ) ) {
            OOFEM_ERROR("giveHumidityIncrement: tf->evaluateAt failed, error value %d", err);
        }
    ...
    if ( wflag == 0 ) {
        OOFEM_ERROR("giveHumidityIncrement: external fields not found");
    }

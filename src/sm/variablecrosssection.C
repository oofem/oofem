...
{
    return this->give(aProperty, gpx->giveCoordinates(), gpx->giveElementGeometry(), true);
}


double
VariableCrossSection :: give(CrossSectionProperty aProperty, const FloatArray *coords, ElementGeometry *elemGeometry, bool local)
{
    double value = 0.0;
    ...
    if ( propertyDictionary->includes(aProperty) ) {
        ...
        if (this->localFormulationFlag) {
            if (local) {
                ...
                if (!elemGeometry->computeLocalCoordinates(c, *coords)) {
                    OOFEM_ERROR2("VariableCrossSection::give: computeLocalCoordinates failed (element %d)", elemGeometry->giveNumber());
                }
            }
        } else { // global coordinates needed
            if (local) {
                // convert given coords into global cs
                if (!elemGeometry->computeGlobalCoordinates(c, *coords)) {
                    OOFEM_ERROR2("VariableCrossSection::give: computeGlobalCoordinates failed (element %d)", elemGeometry->giveNumber());
                }
            } else {
                c = *coords;
            }

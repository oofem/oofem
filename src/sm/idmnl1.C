...
IDNLMaterial :: computeAngleAndSigmaRatio(double &angle, double &ratio, GaussPoint *gp, double &flag)
{
    ...
    if ( ( matMode == _3dMat ) || ( matMode == _1dMat ) ) { //Check if the stress-based approach can be applied
        OOFEM_ERROR("computeAngleAndSigmaRatio: 3D or 1D realisation for Stress-based averaging not supported");
    }
        ...
        if ( gp->giveMaterialMode() == _1dMat ) {
            ...
            if ( ( gp->giveMaterialMode() == _3dMat ) || ( gp->giveMaterialMode() == _PlaneStrain ) ) {
                ...
                if ( indx ) {
                    ...
                    OOFEM_ERROR("giveRemoteNonlocalStiffnessContribution: internal error");
                }
            } else {
                OOFEM_ERROR("giveRemoteNonlocalStiffnessContribution: equivStrainType not supported");
            }
        }

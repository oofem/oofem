...
Tr2Shell7XFEM :: _export(FILE *stream, VTKXMLExportModule *expModule, IntArray &primaryVarsToExport, IntArray &internalVarsToExport, TimeStep *tStep) 
{
    ...
    FloatArray nodeCoords[numCellNodes];
    ...
    // Every cell should be independent (unique nodes) in the case of XFEM and thus the connectivity is just one long array from 1 and up.
    ...
    for ( int cell = 1; cell <= numCells; cell++ ) {
        int vtkCellType = 26; // Quadratic wedge
...
void
Tr2Shell7XFEM :: exportPrimVarAs(UnknownType valID, int regionDofMans, int ireg, FILE *stream, TimeStep *tStep)
{

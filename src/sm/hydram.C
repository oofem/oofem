...
{
 setMixture(mix);
 if ((usefr)&&(usefr<=frMixed)) useFindRoot = usefr;
 else _error("constructor: unknown FindRootMethod\n");
}
...
// Normalized chemical affinity regression function coefficients
 ...
 else _error("Unknown mixture type!");

 ear = 4000;  // activation term [K]
 if (!le) le = 190000; // latent heat [kJ/m3]
...
*/
{
 ...
 } else _error("giveStatus: Master status undefined.");
 return status;
}
...
// to enable usage from structural material
{
 ...
 if ((rmode == IntSource) || (rmode == IntSource_hh) || (rmode == IntSource_ww) || (rmode == IntSource_wh) || (rmode == IntSource_hw) ) {
  if (vec.isEmpty()) _error("giveCharacteristicValue: undefined state vector.");
  answer = computeIntSource(vec, gp, atTime, rmode);
 } else _error("giveCharacteristicValue: wrong MatResponseMode.");
...
//!!! sets aux values in material
{
 ...
 if (ksi<1.0) {
  ...
  auxdt = dt;
  switch (useFindRoot) {
   case frRegula:  result = regulafindroot(); break;
   case frBinTree: result = bintreefindroot(); break;
   case frMixed:   result = mixedfindroot(); break;
#ifdef DEBUG
   default: {
...
// returns the hydration degree in integration point gp
{
 ...
 if (mode == VM_Incremental) ksi -= status->giveHydrationDegree();
 return ksi;
}
...
*/
{
 ...
 if (vec.giveSize()) {
  T = vec(0);
  if (vec.giveSize()>1) h = vec(1);
  else h=1; // assume saturated if undefined
 } else _error("updateInternalState: undefined state vector.");
...
HydrationModel :: mixedfindroot()
{
 ...
 do {
  ...
  if (!done) {
   ...
   if (fabs(y0)<ROOT_PRECISION_DKSI) break;
   else
    if (y0<0) xl=x0;
    else xr=x0;
#ifdef VERBOSEFINDROOT
...
{
 if (hydrationModelStatus) hydrationModelStatus->updateYourself(atTime);
}
/// Outputs the status variables
void
HydrationModelStatusInterface::printOutputAt(FILE *file, TimeStep* atTime)
{
 if (hydrationModelStatus) hydrationModelStatus->printOutputAt(file, atTime);
...
*/
{
 if (hydrationModel) {
  ...
  if (atTime->giveTime() - atTime->giveTimeIncrement() < castAt) {
   if (atTime->giveTime() >= castAt) hydraTime->setTimeIncrement(atTime->giveTime()-castAt);
   else notime=1;
  }
  if (!notime) hydrationModel->updateInternalState(vec, gp, hydraTime);
  delete hydraTime;
 }
}
...
{
 if (hydrationModel) return hydrationModel->giveHydrationDegree(gp, atTime, mode);
 else return constantHydrationDegree;
}

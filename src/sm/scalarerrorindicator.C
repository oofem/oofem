...
ScalarErrorIndicator::estimateError (EE_ErrorMode mode, TimeStep* tStep) {
 if (indicatorType == 1) {
  if (mode == equilibratedEM) varType = IST_PrincipalDamageTensor; else varType = IST_PrincipalDamageTempTensor;
 } 
 return 1;
}
...
ScalarErrorIndicator::giveElementError (EE_ErrorType type, Element* elem, TimeStep* tStep) {
 ...
 if (type != indicatorET) return 0.0;
 ...
 for (i=0; i< nip; i++) {
  ...
  if (result) {
   ...
   if (i==0) maxVal = sval; else maxVal = max(maxVal, sval);
  }
 }
...
ScalarErrorIndicator :: initializeFrom (InputRecord* ir)
{
 ...
 if (indicatorType != 1) _error ("instanciateFrom: usupported varType");

...
RCM2Material :: giveCharacteristicElementLenght(GaussPoint *gp, const FloatArray &crackPlaneNormal)
{
    ...
    return gp->giveElementGeometry()->giveCharacteristicLenght(gp, crackPlaneNormal);
}
...
//
{
    ...
    if ( Le <= 0 ) {
        OOFEM_ERROR( "Element %d returned zero char length", gp->giveElementGeometry()->giveNumber() );
    }

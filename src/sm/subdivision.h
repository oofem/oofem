 ...
 protected:
  ...
  public:
    RS_Node (int n, Subdivision::RS_Mesh* m, int parent, FloatArray& c, double rd, bool boundary) {
      this->number=n; this->mesh = m; this->coords=c; this->requiredDensity = rd; this->boundary=boundary;
      this->parent=parent;
#ifdef __PARALLEL_MODE
  ...
  public:
    ...
    void makeSharedEdges () {shared_edges.resize(3); shared_edges.zero();}
#endif
  };
...
#ifdef __PARALLEL_MODE
    void numberSharedEdges(int iNode, IntArray &connNodes);
    void makeSharedEdges () {shared_edges.resize(6); shared_edges.zero();}
#endif
  };
  ...
  public:
#ifdef __PARALLEL_MODE
    RS_Mesh (Subdivision* s) : nodes(0,RS_ARRAY_CHUNK), elements(0,RS_ARRAY_CHUNK), edges(0, RS_ARRAY_CHUNK) {this->subdivision=s;sharedNodeMapInitialized=false;}
    ~RS_Mesh () {nodes.clear(); elements.clear(); edges.clear();}
#else

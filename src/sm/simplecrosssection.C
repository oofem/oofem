...
*SimpleCrossSection :: giveMaterial(IntegrationPoint *ip)
{
    if ( this->giveMaterialNumber() ) {
        ...
        return ip->giveElementGeometry()->giveMaterial();
    }
}
...
SimpleCrossSection :: giveTemperatureVector(FloatArray &answer, GaussPoint *gp, TimeStep *tStep)
{
    ...
    StructuralElement *sElemGeometry = dynamic_cast< StructuralElement * >( elemGeometry);
	elemEvaluator = sElemGeometry->giveDomain()->giveElementEvaluator(elemGeometry->giveNumber());
	StructuralElement *sElemEvaluator = dynamic_cast< StructuralElement * >( elemEvaluator);
	

    sElemEvaluator->computeResultingIPTemperatureAt(answer, tStep, gp, VM_Total);
	//selemGeometry->computeResultingIPTemperatureAt(answer, tStep, gp, VM_Total);
    ...
    if ( ( tf = fm->giveField(FT_Temperature) ) ) {
        ...
        if ( ( err = tf->evaluateAt(et2, gcoords, VM_Total, tStep) ) ) {
            OOFEM_ERROR3("StructuralMaterial :: giveTemperatureVector: tf->evaluateAt failed, element %d, error code %d", elemGeometry->giveNumber(), err);
        }

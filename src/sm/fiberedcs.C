/* $Header: /home/cvs/bp/oofem/sm/src/fiberedcs.C,v 1.5.4.1 2004/04/05 15:19:46 bp Exp $ */
/*
...
//
{
 ...
 for (int i=1; i<= numberOfFibers; i++) {
  ...
  ((StructuralMaterial*) fiberMat)
    -> giveRealStressVector (stressVector3d, FullForm, fiberGp, fiberStrain, tStep);
...
//
{
 ...
 if (mode != _3dBeam) _error ("give3dBeamMaterialStiffness : unsupported mode");
...
{ 
 ...
 if (mode == _1dFiber) {
  ...
  if (size != 6) _error ("giveFullCharacteristicVector: stressVector size mismatch");
  answer = charVector;
 } else {
...
#  endif
 ...
 if (numberOfFibers <= 0) _error ("instanciateFrom : numberOfFibers <= 0 is not alloved");
...
//
{
 if (masterMode == _3dBeam) return _1dFiber;
 else {
...
FiberedCrossSection :: give (int aProperty)
   ...
   // 'A') of the receiver.
{
 if (aProperty == THICKNESS) return this->thick;
 if (aProperty == WIDTH) return this->width;

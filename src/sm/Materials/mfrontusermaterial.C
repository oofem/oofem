...
FloatArrayF<9>
MFrontUserMaterial :: giveFirstPKStressVector_3d(const FloatArrayF<9> &vF, GaussPoint *gp, TimeStep *tStep) const
{
//    auto ms = static_cast<MFrontUserMaterialStatus * >( this->giveStatus(gp) );
//
//    FloatArrayF<9> mfront_stress; // PK1 or cauchy
//
...
//
//    // computes the Second Piola-Kirchoff stress from the the small strain behaviour
//    const auto pk2 = this->giveRealStressVector_3d(E, gp, tStep);

//    auto strainIncrement = strain - FloatArrayF<6>(ms->giveStrainVector());
//    FloatArray state = ms->giveStateVector();
//    FloatMatrixF<9,9> mfront_jacobian; // dPdF
//
//    // Times and increment
//    double dtime = tStep->giveTimeIncrement();
//
...
//
//    // Change the component order
//    auto mfront_strain = strain[mfront2oo6];
//    auto mfront_strainIncrement = strainIncrement[mfront2oo6];
//
//    constexpr const auto n = mgis::size_type{1};
//    const auto b = load(this->libname, this->modelname, Hypothesis::TRIDIMENSIONAL);
//    ThreadPool p{2};
...
//        m.s1.gradients[idx * m.s1.gradients_stride+0] = strain[0];
//        m.s1.gradients[idx * m.s1.gradients_stride+1] = strain[1];
//        m.s1.gradients[idx * m.s1.gradients_stride+2] = strain[2];
//        m.s1.gradients[idx * m.s1.gradients_stride+3] = strain[3];
//        m.s1.gradients[idx * m.s1.gradients_stride+4] = strain[4];
//        m.s1.gradients[idx * m.s1.gradients_stride+5] = strain[5];
//    }
//
//    integrate(p, m, IntegrationType::INTEGRATION_TANGENT_OPERATOR, dtime);
//
//    printf("%f ", m.s1.thermodynamic_forces[0]);
//    printf("%f ", m.s1.thermodynamic_forces[1]);
//    printf("%f ", m.s1.thermodynamic_forces[2]);
//    printf("%f ", m.s1.thermodynamic_forces[3]);
//    printf("%f ", m.s1.thermodynamic_forces[4]);
//    printf("%f ", m.s1.thermodynamic_forces[5]);
//    printf("\n");
//
//    mfront_stress[0] = m.s1.thermodynamic_forces[0];
//    mfront_stress[1] = m.s1.thermodynamic_forces[1];
//    mfront_stress[2] = m.s1.thermodynamic_forces[2];
...
//        auto stress = mfront_stress[mfront2oo9];
...
//        return vP;
//    } else {
//        auto vsigma = mfront_stress[mfront2oo6];

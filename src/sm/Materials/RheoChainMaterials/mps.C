    ...
    KelvinChainSolidMaterialStatus(n, d, gp, nunits)
{
    ...
    dryingShrinkageStrainIncrement = 0.;
    autogenousShrinkageStrain = 0.;
    autogenousShrinkageStrainIncrement = 0.;
...
#ifdef keep_track_of_strains
    dryingShrinkageStrain += dryingShrinkageStrainIncrement;
    dryingShrinkageStrainIncrement = 0.;

    creepStrain.add(creepStrainIncrement);
    creepStrainIncrement.zero();

    autogenousShrinkageStrain += autogenousShrinkageStrainIncrement;
    autogenousShrinkageStrainIncrement = 0.;
#endif
...
//
{
    ...
    /* is it necessary?
    this->setHum(-1.);
    this->setHumIncrement(-1.);
    this->setT(-1.);
    this->setTIncrement(-1.);
    this->setFlowTermViscosityTemp(-1.);
    this->setEquivalentTimeTemp(-1.);
    this->setEmodulusFlag(false);
    */
                                         ...
                                         ValueModeType mode)
{
  ...
  if ( (this->kSh == 0.) || ( this->CoupledAnalysis == MPS_temperature ) || ( !this->isActivated(tStep) ) ) {
...
#ifdef keep_track_of_strains
  MPSMaterialStatus *status = static_cast< MPSMaterialStatus * >( this->giveStatus(gp) );
  if(answer.giveSize()>=1){
    double dryShrIncr = answer.at(1);
    status->setDryingShrinkageStrainIncrement(dryShrIncr);
  }
#endif
  ...
  if ( ( this->eps_cas0 != 0. ) || ( this->b4_eps_au_infty != 0.) ) {
    ...
    double autoShrIncr = eps_as.at(1);
    status->setAutogenousShrinkageStrainIncrement(autoShrIncr);
    }
#endif

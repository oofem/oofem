...
double
CebFip78Material :: computeCreepFunction(double t, double t_prime, GaussPoint *gp, TimeStep *tStep) const
{
    // computes the value of creep function at time t
    // when load is acting from time t_prime
    // t-t_prime = duration of loading

    double t0 = this->kap_tt * this->kap_c * t_prime;
    double tt  = this->kap_tt * this->kap_c * t;

    double e0 = E28 * sqrt( 1.36 * t0 / ( t0 + 10. ) );

    double fi0 = 0.95 * pow(t0, -0.3) - 0.1;
    if ( fi0 < 0. ) {
        fi0 = 0.;
    }

    double firv = 0.11 + 0.2 * atan( 0.05 * pow( ( tt - t0 ), 2. / 3. ) );
    if ( firv < 0.4 ) {
        firv = 0.4;
    }

    double hd = this->kap_a_per_area * 1000. * 1000. / this->u;
    if ( hd < 50. ) {
        hd = 50.;
    }
    if ( hd > 1500. ) {
    ...
    double alpha = 0.078 * exp( -1.20 * log10(hd) );
    double beta  = 0.530 * exp( -0.13 * log10(hd) );

    double fiir = 0.25 * ( 5.4 - log10(hd) ) * ( exp( -pow(alpha * t0, beta) ) - exp( -pow(alpha * tt, beta) ) );
    fiir *= fibf;

    double fi = fi0 + firv + fiir;

...
IRResultType RVEMaterial :: initializeFrom(InputRecord *ir)
{
    ...
    IR_GIVE_FIELD(ir, this->rveFilename, IFT_MicroMaterialFileName, "file"); //, MAX_FILENAME_LENGTH);

    OOFEM_LOG_INFO("************************** Instanciating microproblem from file %s\n", rveFilename.c_str());
    OOFEMTXTDataReader drMicro(rveFilename.c_str());

    this->rve = InstanciateProblem(& drMicro, _processor, 0); //0=contextFlag-store/resore
    drMicro.finish();

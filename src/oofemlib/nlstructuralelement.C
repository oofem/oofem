   ...
   // this version assumes Total Lagrange approach.
{
 ...
 if (mode == TL) {     // Total Lagrange formulation
  ...
  this -> computeVectorOf(EID_MomentumBalance, VM_Total,stepN, u) ;
   ...
   if (rot) u.rotatedWith(this->rotationMatrix, 'n') ;
  ...
  //
{
  ...
  bs.times (dV);
    if (rot) bs.rotatedWith(this->rotationMatrix, 't');
    answer.add(bs);
    //delete bs;
    //delete b;
    //delete bt;
...
//
{
  ...
  answer.symmetrized() ;
  if (rot) answer.rotatedWith(*this->rotationMatrix) ;
  return  ;
}

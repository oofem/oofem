...
DynCompCol::DynCompCol(const DynCompCol &S) : base_(S.base_)
{
 ...
 if (S.columns_) {
  ...
  for (i=0; i<S.nColumns; i++) this->columns_[i]= new FloatArray(*S.columns_[i]);
 } else this->columns_ = NULL;

 if (S.rowind_) {
  this->rowind_ = new IntArray*[S.nColumns];
  for (i=0; i<S.nColumns; i++) this->rowind_[i]= new IntArray(*S.rowind_[i]);
 } else this->rowind_ = NULL;

#else
 int i;
 if (S.columns) {
  this->columns = new std::map<int, double>*[S.nColumns];
  for (i=0; i<S.nColumns; i++) this->columns[i]= new std::map<int, double>(*S.columns[i]);
...
DynCompCol& DynCompCol::operator=(const DynCompCol &C)  
{
 ...
 if (C.columns_) {
  ...
  for (i=0; i<C.nColumns; i++) this->columns_[i]=new FloatArray(*C.columns_[i]);
 } else this->columns_ = NULL;
 ...
 if (C.rowind_) {
  ...
  for (i=0; i<C.nColumns; i++) this->rowind_[i]=new IntArray(*C.rowind_[i]);
 } else this->rowind_ = NULL;
 ...
 if (this->columns) {
  for (i=0; i< nColumns; i++) this->columns[i] = C.columns[i];
 }
#endif
...
int DynCompCol::buildInternalStructure (EngngModel*eModel, int di, EquationID ut) 
{
/*
...
*/
 ...
 rowind_ = (IntArray**) new IntArray*[neq];
 for (j=0; j<neq; j++) rowind_[j] = new IntArray();
 ...
 */
 int rowIndx;
 if ((rowIndx= this->giveRowIndx (j-1, i-1))) return columns_[j-1]->at(rowIndx);
...
double DynCompCol::operator()(int i, int j)  const
{
  ...
  */
 int rowIndx;
 if ((rowIndx= this->giveRowIndx (j, i))) return columns_[j]->at(rowIndx);
 if (i < nRows && j < nColumns) return 0.0;
 else {
...
void  DynCompCol :: checkSizeTowards (IntArray& loc) 
{
 ...
 // adjust the size of receiver
 for (i=0; i<size; i++) maxid = max(maxid, loc(i));
 this->growTo (maxid);
...
int DynCompCol::giveRowIndx (int col, int row) const
{
 ...
 if (right == 0) return 0;
 if (this->rowind_[col]->at(right) == row) return right;
 ...
 if (middleVal == row) return middle;
 return 0;
}

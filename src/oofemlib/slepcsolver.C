...
SLEPcSolver::solve (SparseMtrx* a, SparseMtrx* b, FloatArray* _eigv, FloatMatrix* _r, double rtol, int nroot)
{
  ...
  if ((!a) || (!b)) _error ("SLEPcSolver :: solveYourselfAt : matrices are not defined\n");
  ...
  if (_eigv == NULL) _error("SLEPcSolver :: solveYourselfAt: unknown eigenvalue array");
  if (_eigv->giveSize() != nroot) _error ("SLEPcSolver :: solveYourselfAt: eigv size mismatch");

  // check matrix for storing resulted eigen vectors at the end
  if (_r == NULL) _error ("SLEPcSolver :: solveYourselfAt: unknown eigen vectors mtrx");
  ...
  */
  ...
  ierr = EPSGetIterationNumber(eps, &nite); CHKERRQ(ierr);
...
#ifdef __PARALLEL_MODE
    Vec Vr2;
    ierr = VecCreateSeq(PETSC_COMM_SELF,size,&Vr2);CHKERRQ(ierr);
#endif

    for( int i=0; i<nconv && i<nroot; i++ ) {
      ierr = EPSGetEigenpair(eps,nconv-i-1,&kr,PETSC_NULL,Vr,PETSC_NULL);CHKERRQ(ierr);
      ...
      ierr = VecGetArray(Vr2,&array);CHKERRQ(ierr);      
      for (int j=0; j<size; j++) {
	_r->at(j+1,i+1) = array[j];
      }
      ierr = VecRestoreArray(Vr2,&array);CHKERRQ(ierr);    
#else
      ierr = VecGetArray(Vr,&array);CHKERRQ(ierr); 
      for (int j=0; j<size; j++) {
	_r->at(j+1,i+1) = array[j];
      }
      ierr = VecRestoreArray(Vr,&array);CHKERRQ(ierr);    

#endif
    }
    ierr = VecDestroy(Vr);CHKERRQ(ierr);
#ifdef __PARALLEL_MODE
      ierr = VecDestroy(Vr2);CHKERRQ(ierr);
#endif

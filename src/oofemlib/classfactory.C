...
BaseElement *ClassFactory :: createElement(const char *name, int number, Domain *domain)
{
    return ( elemList.count(name) == 1 ) ? elemList [ name ](number, domain) : NULL;
}

bool ClassFactory :: registerElement( const char *name, BaseElement * ( *creator )( int, Domain * ) )
{
...
IntegrationRule *ClassFactory :: createIRule(IntegrationRuleType type, int number, ElementGeometry *e)
{
    if ( type == IRT_Gauss ) {

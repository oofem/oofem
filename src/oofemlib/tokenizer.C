/* $Header: /home/cvs/bp/oofem/oofemlib/src/tokenizer.C,v 1.2.4.1 2004/04/05 15:19:44 bp Exp $ */
/*
...
Tokenizer :: readStringToken (int& bpos, const char*& line, char*& token) 
{
  ...
  if (*line == '"') line++; // check if terminating '"' was found
  else OOFEM_WARNING ("Tokenizer::readStringToken : Missing closing separator (\")");
}
...
Tokenizer :: readStructToken (int& bpos, const char*& line, char*& token) 
{
  ...
  do {
    c=*line++; 
    if (bpos<MAX_TOKENS_LENGTH-1) {*token = c; token ++; bpos++;}
    if (c=='{') refLevel++;
    else if (c=='}') refLevel--;
  } while (refLevel && c != '\0' && c != '\n');
  if (refLevel) OOFEM_WARNING ("Tokenizer::readStructToken : Missing closing separator (})");
}
  ...
  nTokens = 0;
	...
	while (nTokens < MAX_TOKENS && *ptr != '\n' && *ptr != '\0') {
		...
		if (separator == 0) {
			/* skip whitespaces */
			while ((c=*ptr) != '\0' && c != '\n' && isspace(c)) ptr++;
      if (c=='"') readStringToken (bpos, ptr, token);
      else if (c == '{') readStructToken (bpos, ptr, token);
      else this->readToken (bpos, ptr, token, 0);
		} else {
      this->readToken (bpos, ptr, token, separator);
	    if (c==separator) ptr ++; /* skip separator */
	}
...
const char* Tokenizer :: giveToken (int i)
{
	...
	if (i <= nTokens) return tokenPosition[i-1];
	else return NULL;
}

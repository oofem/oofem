...
BSplineInterpolation :: initializeFrom(InputRecord *ir)
{
    ...
    for ( int n = 0; n < nsd; n++ ) {
        ...
        if ( size < 2 ) {
            OOFEM_ERROR("BSplineInterpolation::initializeFrom - invalid size of knot vector %s", IFT_knotVector [ n ]);
        }
        ...
        for ( int i = 1; i < size; i++ ) {
            if ( knotValues [ n ].at(i + 1) <= knotVal ) {
                OOFEM_ERROR("BSplineInterpolation::initializeFrom - knot vector %s is not monotonic", IFT_knotVector [ n ]);
            }
        ...
        if ( knotMultiplicity [ n ].giveSize() == 0 ) {
            ...
            if ( knotMultiplicity [ n ].giveSize() != size ) {
                OOFEM_ERROR("BSplineInterpolation::initializeFrom - knot multiplicity %s size mismatch", IFT_knotMultiplicity [ n ]);
            }
            ...
            for ( int i = 1; i < size - 1; i++ ) {
                if ( knotMultiplicity [ n ].at(i + 1) < 1 || knotMultiplicity [ n ].at(i + 1) > degree [ n ] ) {
                    OOFEM_ERROR( "BSplineInterpolation::initializeFrom - knot multiplicity %s out of range - value %d",
...
int BSplineInterpolation :: giveKnotSpanBasisFuncMask(const IntArray &knotSpan, IntArray &mask)
{
    ...
    if ( nsd == 1 ) {
        ...
        OOFEM_ERROR("BSplineInterpolation :: giveKnotSpanBasisFunctMask not implemented for nsd = %d", nsd);
    }
    return 1;
}

...
int
GaussIntegrationRule :: SetUpPointsOnWedge(int nPoints, MaterialMode mode, GaussPoint ***arry)
// creates array of nPoints Gauss Integration Points
// ( don't confuse with GaussPoint - elem is only the container where to
//   store coordinates and weights)
{
...
int
GaussIntegrationRule :: SetUpPointsOnWedge2(int nPointsTri, int nPointsDepth, MaterialMode mode)
// creates array of nPoints Gauss Integration Points
// ( don't confuse with GaussPoint - elem is only the container where to
//   store coordinates and weights)
{
    ...
    this->gaussPointArray  = new GaussPoint * [ nPointsTri*nPointsDepth ];
    ...
    for ( int i = 1, ind = 0; i <= nPointsTri; i++ ) {
        for ( int j = 1; j <= nPointsDepth; j++ ) {
            ...
            weight = weightsTri.at(i)*weightsDepth.at(j);
            this->gaussPointArray[ind] = new GaussPoint(this, 1, coord, weight, mode);
            ind++;
        }
    }
        return 1;
...
int
GaussIntegrationRule :: SetUpPointsOnLine2(int nPointsLine, MaterialMode mode)
// creates array of nPoints Gauss Integration Points
// ( don't confuse with GaussPoint - elem is only the container where to
//   store coordinates and weights)
{
    ...
    this->giveLineCoordsAndWeights(nPointsLine, coords_xi, weights);
    double weight;
    this->numberOfIntegrationPoints = nPointsLine;
    this->gaussPointArray  = new GaussPoint * [ nPointsLine ];

        for ( int i = 1, ind = 0; i <= nPointsLine; i++ ) {
            coord = new FloatArray(1);
...
// Coordinates xi1 and xi2 are the two firts area coordinates of a tringle 
{

    coords_xi1.resize(nPoints); coords_xi2.resize(nPoints); weights.resize(nPoints);
    switch(nPoints){
    case 1:
...
{
    coords_xi.resize(nPoints); weights.resize(nPoints);

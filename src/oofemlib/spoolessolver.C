/* $Header: /home/cvs/bp/oofem/oofemlib/src/spoolessolver.C,v 1.1.4.1 2004/04/05 15:19:43 bp Exp $ */
/*
...
SpoolesSolver::~SpoolesSolver () {
 if (msgFileCloseFlag) fclose (msgFile);

 if (frontmtx) FrontMtx_free(frontmtx) ;
 if (newToOldIV) IV_free(newToOldIV) ;
 if (oldToNewIV) IV_free(oldToNewIV) ;
 if (frontETree) ETree_free(frontETree) ;
 if (symbfacIVL) IVL_free(symbfacIVL) ;
 if (mtxmanager) SubMtxManager_free(mtxmanager) ;
...
SpoolesSolver::solve (SparseMtrx* A, FloatArray* b, FloatArray* x)
{
 ...
 if (A->giveType() != SMT_SpoolesMtrx) _error ("solveYourselfAt: SpoolesSparseMtrx Expected");
 ...
 DenseMtx_zero(mtxY) ;
 for (i=0; i< neqns; i++) DenseMtx_setRealEntry(mtxY, i, 0, b->at(i+1)) ;

...
SPRNodalRecoveryModel :: recoverValues (InternalStateType type, TimeStep* tStep)
{
 ...
 for (ireg=1; ireg<= nregions; ireg++) {
  ...
  if (this->initRegionNodeNumbering (regionNodalNumbers, regionDofMans, ireg )==0) continue;

  neq = regionDofMans*regionValSize;
  dofManValues.resize(neq); dofManValues.zero();
  dofManPatchCount.resize(regionDofMans); dofManPatchCount.zero();
  ...
  for (i=1; i<= nnodes; i++) {
   if (regionNodalNumbers.at(i)) {
    ...
    if (dofManPatchCount.at(regionNodalNumbers.at(i))) {
     for (j=1; j<=regionValSize; j++) dofManValues.at(eq+j) /= dofManPatchCount.at(regionNodalNumbers.at(i));
    } else {
     OOFEM_ERROR2 ("SPRNodalRecoveryModel::recoverValues : values of dofmanager %d udetermined",i);
     abortFlag = true;
    }
   }
  }
                    ...
                    IntArray& regionTypes, InternalStateType type)
{
 ...
 regionTypes.resize(nregions); regionTypes.zero();
...
SPRNodalRecoveryModel::determinePatchAssemblyPoints (IntArray& pap, int ireg, SPRPatchType regType) 
{
 ...
 for (idofMan=1; idofMan <= ndofMan; idofMan++) dofManFlags.at(idofMan) = papStatus_noPap;
 ...
 for (idofMan=1; idofMan <= ndofMan; idofMan++) {
  ...
  if (dofManFlags.at(idofMan) == papStatus_littleNIPs) {
   ...
   for (ielem=1; ielem<=papDofManConnectivity->giveSize(); ielem++) {
    ...
    if ((interface = (SPRNodalRecoveryModelInterface*) element->giveInterface (SPRNodalRecoveryModelInterfaceType))) {
     ...
     for (ipap=1; ipap <= npap; ipap++) {
      ...
      if (dofManFlags.at(elemPap.at(ipap)) == papStatus_littleNIPs) continue;
   ...
   if (foundRegularPap) continue; // can be determined from regular pap - ok
 ...
 for (idofMan=1; idofMan <= ndofMan; idofMan++)
  if (dofManFlags.at(idofMan) ==  papStatus_regular) count++;
 
 pap.resize(count);
 count = 0;
 for (idofMan=1; idofMan <= ndofMan; idofMan++)
  if (dofManFlags.at(idofMan) ==  papStatus_regular) pap.at(++count) = idofMan;
                 ...
                 IntArray& pap, int papNumber, int ireg)
{
 ...
 count = 0;
 for (ielem = 1; ielem<= nelem; ielem++) 
  if (domain->giveElement(papDofManConnectivity->at(ielem))->giveRegionNumber() == ireg) count++;
  ...
  if ((interface = (SPRNodalRecoveryModelInterface*) element->giveInterface (SPRNodalRecoveryModelInterfaceType))) {
   ...
   for (i=1; i<=toDetermine.giveSize(); i++) {
    ...
    for (dofManToDetermineListIter = dofManToDetermineList.begin(); 
       ...
       ++dofManToDetermineListIter) if (*(dofManToDetermineListIter) == toDetermine.at(i)) includes = 1;
    if (!includes) dofManToDetermineList.pushBack (toDetermine.at(i));
    ...
    for (ipap=1; ipap <= npap; ipap++) {
     ...
     if (pap.findFirstIndexOf(elemPap.at(ipap)) ==  0) {
      ...
      for (dofManToDetermineListIter = dofManToDetermineList.begin(); 
         dofManToDetermineListIter!= dofManToDetermineList.end();
         ++dofManToDetermineListIter) if ((*dofManToDetermineListIter) == elemPap.at(ipap)) includes = 1;
      if (!includes) dofManToDetermineList.pushBack (elemPap.at(ipap));
      ...
      for (j=1; j<=toDetermine2.giveSize(); j++) {
       ...
       for (dofManToDetermineListIter = dofManToDetermineList.begin(); 
          dofManToDetermineListIter!= dofManToDetermineList.end();
          ++dofManToDetermineListIter) if (*dofManToDetermineListIter == toDetermine2.at(j)) includes = 1;
       if (!includes) dofManToDetermineList.pushBack (toDetermine2.at(j));
      }
 ...
 for (dofManToDetermineListIter = dofManToDetermineList.begin(); 
    dofManToDetermineListIter!= dofManToDetermineList.end();
    ++dofManToDetermineListIter) count++;
 dofManToDetermine.resize(count);

 count = 0;
 for (dofManToDetermineListIter = dofManToDetermineList.begin(); 
    dofManToDetermineListIter!= dofManToDetermineList.end();
    ++dofManToDetermineListIter) dofManToDetermine.at(++count) = *dofManToDetermineListIter;
                   ...
                   SPRPatchType regType, InternalStateType type, TimeStep* tStep) 
{
 ...
 neq = this->giveNumberOfUnknownPolynomialCoefficients (regType);
 a.resize(neq,regionValSize); a.zero();
 rhs.resize(neq,regionValSize); rhs.zero();
 A.resize(neq,neq); A.zero();
                         ...
                         SPRPatchType type) 
{
 ...
 for (dofMan=1; dofMan<=ndofMan; dofMan++) {
  ...
  for (i=1; i<= regionValSize; i++) 
   for (j=1; j<= lneq; j++) vals.at(i)+=P.at(j)*a.at(j,i);
  // assemble values 
  
  eq = (regionNodalNumbers.at(dofManToDetermine.at(dofMan))-1)*regionValSize;
  for (i = 1; i<= regionValSize; i++) dofManValues.at(eq+i) += vals.at(i);
...
{
 if (regType == SPRPatchType_2dxy) return 3;
 else if (regType == SPRPatchType_3dBiLin) return 4;
 else if (regType == SPRPatchType_2dquadratic) return 6;
 else return 0;
}

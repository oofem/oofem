...
void DofManager :: computeLoadVector(FloatArray &answer, Load *load, CharType type, TimeStep *tStep, ValueModeType mode)
{
    if ( load->giveBCGeoType() != NodalLoadBGT ) {
        OOFEM_ERROR("computeLoadVector: incompatible load type applied");
    }
...
#ifdef DEBUG
    if ( !dofArray ) {
        OOFEM_ERROR("giveDof: dof is not defined");
    }
#endif
...
#ifdef DEBUG
    if ( !indx ) {
        OOFEM_ERROR("giveDofWithID: dof with given DofID does not exists");
    }
#endif
...
void DofManager :: setDof(int i, Dof *dof)
{
    if ( i <= numberOfDofs ) {
        ...
        OOFEM_ERROR("setDof: DOF index out of range");
    }
}
...
// and again constructed with new Dof
{
    ...
    if ( this->findDofWithDofId( dof->giveDofID() ) != 0 ) {
        OOFEM_ERROR("DofManager::addDof: DOF with dofID %d already present (dofman %d)", dof->giveDofID(), this->number);
    }
#endif
...
void DofManager :: removeDof(DofIDItem id)
{
    ...
    if ( position ) { // dof id found
        ...
        OOFEM_ERROR("removeDof::no DOF with dofID %d found", id);
    }
}
    ...
    if ( !hasSlaveDofs ) {
        ...
        for ( int i = 1; i <= size; i++ ) {
            if ( ( indx = this->findDofWithDofId( ( DofIDItem ) dofIDArry.at(i) ) ) == 0 ) {
                OOFEM_ERROR( "giveLocationArray: incompatible dof (%d) requested", dofIDArry.at(i) );
            }
...
// The DofID's are determining the physical meaning of particular DOFs
{
    ...
    for ( int i = 1; i <= size; i++ ) {
        if ( ( answer.at(i) = this->findDofWithDofId( ( DofIDItem ) dofIDArry.at(i) ) ) == 0 ) {
            OOFEM_ERROR("giveDofArray : incompatible dof requested");
        }
    }
}
...
DofManager :: initializeFrom(InputRecord *ir)
{
    ...
    if ( hasBc ) {
        if ( mBC.giveSize() != dofIDArry.giveSize() ) {
            OOFEM_ERROR( "initializeFrom: bc size mismatch. Size is %d and need %d", mBC.giveSize(), dofIDArry.giveSize() );
        }
    ...
    if ( hasIc ) {
        if ( ic.giveSize() != dofIDArry.giveSize() ) {
            OOFEM_ERROR( "initializeFrom: ic size mismatch. Size is %d and need %d", ic.giveSize(), dofIDArry.giveSize() );
        }
    ...
    if ( hasTypeinfo ) {
        if ( dofTypeMask.giveSize() != dofIDArry.giveSize() ) {
            OOFEM_ERROR( "initializeFrom: dofTypeMask size mismatch. Size is %d and need %d", dofTypeMask.giveSize(), dofIDArry.giveSize() );
        }
...
#ifdef DEBUG
        if ( pos == 0 ) {
            OOFEM_ERROR( "DofManager :: giveUnknownVector - Couldn't find dof with Dof ID %d", dofIDArry.at(i) );
        }
#endif
...
void DofManager :: giveUnknownVectorOfType(FloatArray &answer, UnknownType ut, ValueModeType mode, TimeStep *tStep)
{
    ...
    for ( int i = 1; i <= this->numberOfDofs; i++ ) {
        ...
        if ( ut == DisplacementVector || ut == EigenVector ) { // Just treat eigenvectors as displacement vectors (they are redundant)
            ...
            OOFEM_ERROR("DofManager :: giveUnknownVectorOfType - Can't produce vector for unknown type: %d", ut);
        }
    }

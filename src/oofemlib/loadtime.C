/* $Header: /home/cvs/bp/oofem/oofemlib/src/loadtime.C,v 1.5.4.1 2004/04/05 15:19:43 bp Exp $ */
/*
...
{
  if (mode == VM_Total) return this->__at(atTime->giveTime());
  else if (mode == VM_Velocity) return this->__derAt(atTime->giveTime());
  else if (mode == VM_Acceleration) return this->__accelAt(atTime->giveTime());
  else if (mode == VM_Incremental) {
    if (atTime->isTheFirstStep()) return this->__at(atTime->giveTime());
    else return this->__at(atTime->giveTime()) - this->__at(atTime->giveTime()-atTime->giveTimeIncrement());
  } else {

...
//
{
  ...
  if ((mode == _3dShell) || (mode ==  _3dBeam) || (mode == _2dPlate) || (mode == _2dBeam) || (mode==_PlaneStressRot)) {
    if ((mode == _3dShell) || (mode ==  _3dBeam) || (mode == _2dPlate) || (mode == _2dBeam)) answerSize = 12;
    if (mode==_PlaneStressRot) answerSize = 7;
   ...
   // 'A') of the receiver.
{
 ...
 if ((aProperty == 'A') || (aProperty == AREA)) {
   ...
   } else return this->give(THICKNESS)*this->give(WIDTH);
 }

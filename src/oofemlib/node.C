...
//
{
  ...
  if (mode == OGC_nodeAnnotation) {
    ...
    if (!this -> giveLoadArray() -> isEmpty()) {
      ...
      for (i = 1; i<= numberOfDofs; i++) {
        switch (giveDof(i)->giveDofID()) {
        case D_u: force.at(1) = defScale*load.at(i); break;
        case D_v: force.at(2) = defScale*load.at(i); break;
        case D_w: force.at(3) = defScale*load.at(i); break;
        case R_u: momentum.at(1) = defScale*load.at(i); break;
        case R_v: momentum.at(2) = defScale*load.at(i); break;
        case R_w: momentum.at(3) = defScale*load.at(i); break;
        default: break;
        }
      }
      ...
      pp[1].x = force.at(1); pp[1].y = force.at(2); pp[1].z = force.at(3);
      go = CreateVector3D(pp);
      EGWithMaskChangeAttributes(COLOR_MASK | LAYER_MASK | VECMTYPE_MASK , go);
      EMAddGraphicsToModel(ESIModel(), go);
      // draw moment
      EASValsSetVecMType(DOUBLE_ARROW_VECMARKER);
      pp[1].x = momentum.at(1); pp[1].y = momentum.at(2); pp[1].z = momentum.at(3);
      go = CreateVector3D(pp);
      EGWithMaskChangeAttributes(COLOR_MASK | LAYER_MASK | VECMTYPE_MASK , go);
      EMAddGraphicsToModel(ESIModel(), go);

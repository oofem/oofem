...
Node :: computeLoadVector(FloatArray &answer, Load *load, CharType type, TimeStep *tStep, ValueModeType mode)
{
    ...
    if ( !loadN ) {
        OOFEM_ERROR("computeLoadVectorAt: incompatible load");
    }

    if ( loadN->giveBCGeoType() != NodalLoadBGT ) {
        OOFEM_ERROR("computeLoadVectorAt: incompatible load type applied");
    }
...
#ifdef DEBUG
    if ( ( ic < 1 ) || ( ic > 3 ) ) {
        OOFEM_ERROR("giveUpdatedCoordinate: Can't return non-existing coordinate (index not in range 1..3)");
        return 0.;
    }
#endif
    ...
    if ( localCoordinateSystem == NULL ) {
        ...
        if ( dofIDArry.isEmpty() ) {
            ...
            for ( int i = 1; i <= numberOfDofs; i++ ) {
                ...
                default:
                    OOFEM_ERROR( "computeGNTransformation: unknown dofID (%s)", __DofIDItemToString(id).c_str() );
                }
            }

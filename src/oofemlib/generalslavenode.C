...
{
    bool mastersFlag, weightsFlag, listFlag;
    IntArray masterList, masterWeights;
    ...
    PM_UPDATE_PARAMETER_AND_REPORT(masterSizes, ppm, ir, this->number, IPK_GeneralSlaveNode_masterSizes, priority, mastersFlag) ;
    PM_UPDATE_PARAMETER_AND_REPORT(masterWeights, ppm, ir, this->number, IPK_GeneralSlaveNode_masterWeights, priority, weightsFlag) ;
    PM_UPDATE_PARAMETER_AND_REPORT(masterList, ppm, ir, this->number, IPK_GeneralSlaveNode_masterList, priority, listFlag) ;

    //
    Node::initializeFrom(ir, priority);
   
    if (mastersFlag && weightsFlag && listFlag) {
    ...
    } else if (mastersFlag || weightsFlag || listFlag) {
        OOFEM_ERROR("OOFEM ERROR: GeneralSlaveNode::initializeFrom: all three parameters (masterSizes, masterWeights, masterList) must be set together");
    }
}

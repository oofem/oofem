...
//  if (! input->get(ch)) return curr_tok = END;
  if (! (ch = *(parsedLine++))) return curr_tok = END;
 } while (ch != '\n' && isspace (ch));
...
//   while (input->get(ch) && isalnum (ch)) *p++ = ch;
    ...
    *p++ = ch;
    if (len++ >= Parser_CMD_LENGTH) error ("command too long");
   }
   *p = 0;
//   input->putback(ch);
   parsedLine--;
...
// empty Parser table
 ...
 for (int i=0; i<Parser_TBLSZ; i++) {
  if ((entry = table[i])) {
   do {next = entry->next; delete entry; } while ((entry = next));
   table[i]=0;
  }
 }
}

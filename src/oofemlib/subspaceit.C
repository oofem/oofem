 ...
 */
  ...
  */
  ...
  b->times(h, tt);
  for (i = 1; i<= nn ; i++) r.at(i,1)=tt.at(i);
  ...
  for (j = 2; j<= nc ; j++) {
    ...
    w.at(ij) = 0. ;
    for (i = 1; i<= nn ; i++) 
      if (i==ij) h.at(i) = 1.0; else h.at(i) = 0.0;
    b->times(h, tt);
    for (i = 1; i<= nn ; i++) r.at(i,j) = tt.at(i);
  } // (r = z)
...
# endif
    ...
    for (j = 1; j<= nc; j++) {
      ...
      for (k = 1; k<= nn; k++) r.at(k,j) = tt.at(k);   // (r = xbar)
    }
    ...
    for (j = 1; j<= nc; j++) {   
      ...
      for (k = 1; k<= nn; k++) r.at(k,j)=temp.at(k); // (r=zbar)
      //delete temp;
    }                           // label 160
    br.symmetrized();
#ifdef DETAILED_REPORT
...
# endif
    for (i = 1; i<= nroot; i++) {
      if (rtolv.at(i) > rtol) goto label400 ;
    }
    fprintf(outStream,

...
// tion between two points).
{
    ...
    for ( int i = 1; i <= this->dates.giveSize(); i++ ) {
        if ( fabs(this->dates.at(i) - time) < precision ) {
            ...
            if ( i == 1 ) {
                OOFEM_WARNING( "PiecewiseLinFunction :: evaluateAtTime: computational time %f is out of given time %f, extrapolating value(s)", time, dates.at(i) );
                return 0.;
            }
...
PiecewiseLinFunction :: initializeFrom(InputRecord *ir)
{
    ...
    if ( ir->hasField(_IFT_PiecewiseLinFunction_dataFile) ) {
        ...
        if ( !file.is_open() ) {
            OOFEM_ERROR( "PieceWiseLinFunction :: initializeFrom - Failed to open data file: %s\n", fname.c_str() );
        }

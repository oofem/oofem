/* $Header: /home/cvs/bp/oofem/oofemlib/src/fei3dhexalin.C,v 1.2.4.1 2004/04/05 15:19:43 bp Exp $ */
/*
...
FEI3dHexaLin :: global2local (FloatArray& answer, const FloatArray** nc, const FloatArray& coords, double time)
{
 ...
 do {
  ...
  if (dotProduct(r,r,3) < 1.e-20) break;            // sqrt(1.e-20) = 1.e-10
						   ...
						   double time)
{
 ...
 if(length < 1.0e-10){
  // try nodes 1 3 4
  for (i=1; i<=3; i++)b.at(i) = coords[n4]->at(i)-coords[n1]->at(i);
 ...
 c.times(1.0 / length);
 for (i=1; i<=3; i++)b.at(i) = coords[n]->at(i)-coords[n1]->at(i);
 ...
 for (i=1; i<=3; i++)c.at(i) = coords[n2]->at(i)-coords[n1]->at(i);
 sn[1].at(1) = dotProduct (c,a,3);
 sn[1].at(2) = dotProduct (c,b,3);
 
 for (i=1; i<=3; i++)c.at(i) = coords[n4]->at(i)-coords[n1]->at(i);

...
HydratingIsoHeatMaterial :: giveCharacteristicValue (MatResponseMode rmode, GaussPoint* gp, TimeStep* atTime)
{
 ...
 if (rmode == Capacity) {
  if (castAt && (atTime->giveTime()<castAt)) answer = c * this->give('d')/1000; // Zero capacity before cast
  else answer = c * this->give('d');
 } else if (!hydrationLHS) answer=0;
...
HydratingIsoHeatMaterial::giveIPValueType (InternalStateType type)
{
 ...
 else return TransportMaterial::giveIPValueType (type);
}

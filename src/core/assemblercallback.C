...
void ReferenceForceAssembler :: vectorFromSurfaceLoad(FloatArray& vec, Element& element, BoundaryLoad* load, int boundary, TimeStep* tStep, ValueModeType mode) const
{
    vec.clear();
    if ( load->reference )
        element.computeBoundarySurfaceLoadVector(vec, load, boundary, ExternalForcesVector, mode, tStep);
...
void EffectiveTangentAssembler :: matrixFromSurfaceLoad(FloatMatrix& mat, Element& element, BoundaryLoad* load, int boundary, TimeStep* tStep) const
{
    element.computeTangentFromSurfaceLoad(mat, load, boundary, this->rmode, tStep);
    mat.times(this->k);
}

...
void DenseMatrixArithmetics::ComputeInversionByCholesky(double *C,double* Inv)
{
	...
	for (i=0;i<n;i++) {
		for (j=i;j<n;j++) {
			sum = 0;
			for (k=j; k<n; k++) sum += a[k+n*i]*a[k+n*j];
...
void DenseMatrixArithmetics::L_BlockSolve(double *C,double* B)
{
	...
	for (j=0;j<n;j++)
	{
		for (i=0;i<n;i++)
		{	//Solve L y = b, storing y in x.
			for (sum=b[i],k=i-1;k>=0;k--) sum -= a[i+n*k]*b[k];
			b[i]=sum/a[bn*i];
		}
		b += n;
	}
...
void DenseMatrixArithmetics::SubstSolveL(double *a,double* x)
{
	...
	for (i=0;i<n;i++)
	{	//Solve L y = b, storing y in x.
		for (sum=x[i],k=i-1;k>=0;k--) sum -= a[i+n*k]*x[k];
		x[i]=sum/a[bn*i];
	}
}
...
{													
	...
	for (k=0;k<n;k++)
	{
		...
		{ 
			sum=B[k*n+j];
			for (i=0;i<j;i++) sum -= a[n*j+i]*B[k*n+i];
			B[k*n+j]=sum*a[j+n*j];
		}
		for (j=n-2;j>=0;j--) 
		{ 
			sum=0;
			for (i=j+1;i<n;i++) sum += a[n*j+i]*B[k*n+i];
			B[k*n+j]-=sum; 
		} //All done!
	}
}
...
void DenseMatrixArithmetics::FactorizeBlock(double* A)
{
	...
	{
	case eLDL_decomposition:	LDL_Decomposition(A);	break;
	case eLL_decomposition:		LL_Decomposition(A);	break;			
	}
}
...
void DenseMatrixArithmetics::SubstSolve(double* A,double* b)
{
	switch (prefered_decomposition)
	{
	case eLDL_decomposition:	LDL_Solve(A,b);	break;
	case eLL_decomposition:		LL_Solve(A,b,b);	break;			
	}
}
...
{
	if (*A==0.0) {
		DenseMatrixArithmetics::zero_pivots++; *A = 1.0;
	} else {
		*A = 1.0/(*A);
	}
}

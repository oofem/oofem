...
void SparseMatrixF::Delete()
{
	...
	if (ci) {delete [] ci;ci=NULL;}
	if (adr) {delete [] adr;adr = NULL;}
	neq = 0;
	bLocalCopy = false;
}
...
void SparseMatrixF::CreateLocalCopy()
{
	...
	{
		long neql = neq;
		if (bJardaConvention) neql++;
		long nnz = Nonzeros();

		ULONG* old_adr = adr;	adr = new ULONG[neql];
		Array::Copy((long*)old_adr,(long*)adr,neql);

		ULONG* old_ci = ci;	ci = new ULONG[nnz];
		Array::Copy((long*)old_ci,(long*)ci,nnz);

		double* old_a = a;	a = new double[nnz];

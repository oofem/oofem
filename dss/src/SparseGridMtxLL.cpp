...
SparseGridMtxLL::SparseGridMtxLL(SparseMatrixF& sm,BYTE block_size,Ordering* block_order,MathTracer* eMT,BOOL load_data)
...
// Allocates new space according to bskl and reads old matrix with respect
// to permutation blockP
SparseGridMtxLL::SparseGridMtxLL(SparseMatrixF& sm,BYTE block_size,Ordering* block_order,Ordering* node_order,MathTracer* eMT,BOOL load_data )
...
{
	if (Columns_data) {delete Columns_data; Columns_data = NULL;}
}
...
// x = A^(-1) * b
void SparseGridMtxLL::SolveLL(double* x)
{
	int bi;			
	long* ord = this->block_order->order->Items;
	double* cd = this->Columns_data;
	double* dd = this->Columns_data;
	...
	for (bi=0; bi<n_blocks; bi++)
	{
		...
		BlockArith->SubstSolveL(dd+block_storage*bi,x+block_size*ord[bi]);
	}
	...
	{
		BlockArith->SubstSolveLT(dd+block_storage*bi,x+block_size*ord[bi]);

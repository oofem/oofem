...
{ 
 ...
 if (GtoLRotationMatrix == NULL) {
  ...
  delete e1; delete e2; delete e3; delete help;
 }
...
/*
void
RerShell :: computeTemperatureStrainVectorAt (FloatArray& answer, GaussPoint* gp, TimeStep* stepN, ValueModeType mode)
{
...
{
 if ((type == IST_ShellForceMomentumTensor)) return 12;
...
(FloatMatrix& answer, GaussPoint* aGaussPoint, InternalStateType type)
{
 ...
 if ((type == IST_ShellForceMomentumTensor)) answer.resize(1,3) ;
  else return;
...
RerShell::giveIPValue (FloatArray& answer, GaussPoint* aGaussPoint, InternalStateType type, TimeStep* atTime)
{
 ...
 if ((type == IST_ShellForceMomentumTensor) || (type == IST_ShellStrainCurvatureTensor)) {
   ...
   if (type == IST_ShellForceMomentumTensor) cht = GlobalForceTensor;
   else cht = GlobalStrainTensor;
   ...
   if (type == IST_ShellForceMomentumTensor) cht = GlobalMomentumTensor;
   else cht = GlobalCurvatureTensor;

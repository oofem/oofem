...
{
 if (gp->giveClassID() == MicroplaneClass) return NULL;
 else return new MDMStatus(1,this->nsd,this->numberOfMicroplanes,MicroplaneMaterial::giveDomain(), gp);
}
...
//
{
 if ((mode == _3dMat) || (mode == _PlaneStress) || (mode == _PlaneStrain)) return 1;
 return 0;
}
 ...
 */
 ...
 if (form == ReducedForm) answer=stress;
 else crossSection->giveFullCharacteristicVector(answer, gp, stress) ;
}
              ...
              GaussPoint* gp, TimeStep* atTime)
{
 ...
 } else computeLocalDamageTensor (damageTensor, totalStrain, gp, atTime);
                ...
                GaussPoint* gp, TimeStep* atTime)
{
 ...
 for (int im=0; im<numberOfMicroplanes; im++){
  ...
  if (formulation==COMPLIANCE_DAMAGE)
   ...
   for (int i=1; i<=6; i++) damageVector.at(i) += this->N[im][i-1] * this->microplaneWeights[im] * Psi;
  ...
  else _error ("Unknown type of formulation");
 }
 ...
 } else _error ("computeDamageTensor: unknown ndc value encountered");
...
MDM :: computeDamageOnPlane(GaussPoint* gp, Microplane* mplane, const FloatArray& strain)
{
 ...
 if (prevStress.isNotEmpty()) for (i=1; i<=nsd; i++) if (mask.at(i)) sv+= prevStress.at(mask.at(i));
 ...
 if (type_dam==0){
    if (en<0.) en = 0.; // take the positive part of normal strain
  Enorm = en;
 } else if (type_dam==1) {
    if (en<0.) en = 0.; // take the positive part of normal strain
  Enorm = sqrt(en*en+em*em+el*el);  
 //} else MicroplaneMaterial::_error ("Unknown type of damage law");
 } else _error ("Unknown type of damage law");
 ...
 } else _error ("Unknown type of softening");
...
MDM::applyDamageTranformation(FloatArray& strainPDC, const FloatArray& tempDamageTensorEigenVals)
{
 if (mdmMode == mdm_3d) {
  ...
  } else _error ("Unknown type of mdm mode");
}
...
MDM::applyDamageToStiffness(FloatMatrix& d, GaussPoint* gp)
{
 ...
 if (mdmMode == mdm_3d) {
  ...
  } else _error ("Unknown type stiffness");
  return ;
 } else if (mdmMode == mdm_2d) {
...
MDM::giveIPValue (FloatArray& answer, GaussPoint* aGaussPoint, InternalStateType type, TimeStep* atTime)
{
 ...
 if (type == IST_DamageTensor) {
  ...
  d.beProductOf (c,c);
  if (ndc == 3) {
   answer.resize(3); answer.at(1) = 1.-d.at(1,1); answer.at(2) = 1.-d.at(2,2); answer.at(3) = d.at(1,2);
  } else {
   answer.resize(6);
   answer.at(1) = 1.-d.at(1,1); answer.at(2) = 1.-d.at(2,2); answer.at(3) = 1.-d.at(3,3);
   answer.at(4) = d.at(2,3); answer.at(5) = d.at(1,3); answer.at(6) = d.at(1,2);
  }
  return 1;
  ...
  if (formulation==COMPLIANCE_DAMAGE) {
   for (i=1; i<=nsd; i++) answer.at(i) = 1.-1./d->at(i)/d->at(i);
  } else {
   for (i=1; i<=nsd; i++) answer.at(i) = 1.-d->at(i)*d->at(i);
  }
  return 1;
...
MDM::giveIPValueType (InternalStateType type)
{
 ...
 else if (type == IST_MicroplaneDamageValues) return ISVT_VECTOR;
...
MDM::giveIntVarCompFullIndx (IntArray& answer, InternalStateType type, MaterialMode mmode)
{
 if ((type == IST_DamageTensor)|| (type == IST_DamageTensorTemp)||
  ...
  for (int i =1; i<=nsd; i++) answer.at(i) = i; 
  return 1;
 } else if (type == IST_MicroplaneDamageValues) {
  answer.resize(numberOfMicroplanes);  answer.zero();
  for (int i =1; i<=numberOfMicroplanes; i++) answer.at(i) = i; 
  return 1;
 } else 
...
MDM::giveIPValueSize (InternalStateType type, GaussPoint* aGaussPoint)
{
 if ((type == IST_DamageTensor)||(type == IST_DamageTensor)||
   (type == IST_DamageTensor)||(type == IST_DamageTensor)) return ndc;
 else if ((type == IST_PrincipalDamageTensor)||(type == IST_PrincipalDamageTempTensor)) return nsd;
 else if (type == IST_MicroplaneDamageValues) return numberOfMicroplanes;
...
//
{
 ...
 if (this->nonlocal) {
  ...
  } else _error ("instanciateFrom: unknown set of parameters");
...
{
 if (this->mdmMode == mdm_3d) MicroplaneMaterial::initializeData (numberOfMicroplanes);
 else if (this->mdmMode == mdm_2d) {
 ...
 } else _error ("initializeData: Unknown MDMModeType ecountered");
}
...
MDM::MMI_map (GaussPoint* gp, Domain* oldd, TimeStep* tStep)
{
 ...
 if (formulation==COMPLIANCE_DAMAGE) {
  for (int i=1; i<=intVal.giveSize(); i++) if (intVal.at(i) < 1.0) intVal.at(i) = 1.0;
 } else {
  for (int i=1; i<=intVal.giveSize(); i++) {
   if (intVal.at(i) < 0.0) intVal.at(i) = 0.0;
   if (intVal.at(i) > 1.0) intVal.at(i) = 1.0;
  }
 }

 if (result) status-> setMicroplaneTempDamageValues (intVal);
 ...
 if (result) status-> letTempStressVectorBe (intVal);

 result = mapper2.mapVariable (intVal, gp, IST_StrainTensor, tStep);
 if (result) status-> letTempStrainVectorBe (intVal);
...
MDM::giveInterface (InterfaceType type)
{
 ...
 else if (type == MaterialModelMapperInterfaceType) return (MaterialModelMapperInterface*) this;
 else return NULL;
...
contextIOResultType
MDMStatus::saveContext (FILE* stream, void *obj)
{
 ...
 if ((iores = Psi.storeYourself(stream)) != CIO_OK) THROW_CIOERR(iores);
 if ((iores = DamageTensor.storeYourself(stream)) != CIO_OK) THROW_CIOERR(iores);
 if ((iores = damageTensorEigenValues.storeYourself(stream)) != CIO_OK) THROW_CIOERR(iores);
 if ((iores = damageTensorEigenVectors.storeYourself(stream)) != CIO_OK) THROW_CIOERR(iores);
...
contextIOResultType
MDMStatus::restoreContext(FILE* stream, void *obj)
{
 contextIOResultType iores;
 ...
 if ((iores = Psi.restoreYourself(stream)) != CIO_OK) THROW_CIOERR(iores);
 if ((iores = DamageTensor.restoreYourself(stream)) != CIO_OK) THROW_CIOERR(iores);
 if ((iores = damageTensorEigenValues.restoreYourself(stream)) != CIO_OK) THROW_CIOERR(iores);
 if ((iores = damageTensorEigenVectors.restoreYourself(stream)) != CIO_OK) THROW_CIOERR(iores);
...
MDMStatus :: printOutputAt  (FILE *file, TimeStep* tStep)
{
 ...
 for (i=1 ; i<=n ; i++) {
  fprintf (file,"{");
  for (j=1; j<=n; j++) fprintf (file," % .4e",damageTensorEigenVectors.at(j,i)) ;
  fprintf (file,"}");
 }
 fprintf (file,"}");

 fprintf (file,", complianceTensorEigenValues ") ;
  for (i=1; i<=n; i++) fprintf (file," % .4e",damageTensorEigenValues.at(i)) ;

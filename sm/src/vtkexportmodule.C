...
{
 if (this->smoother) delete this->smoother;
}
...
VTKExportModule::doOutput (TimeStep* tStep)
{
 ...
 if (omode == wdmode) { // (Whole Domain)
  ...
  for (inode = 1; inode<= nnodes; inode++) {
   ...
   for (i=coords->giveSize()+1; i<= 3; i++) fprintf(stream, "%e ", 0.0);
   fprintf(stream, "\n");
  }
#ifdef RBR_SUPPORT
 ...
 if (omode == wdmode) { // (Whole Domain)
  ...
  for (ielem = 1; ielem <= nelem; ielem++) {
   ...
   for (i=1; i<=nelemNodes; i++) fprintf (stream, "%d ", elem -> giveNode (i) -> giveNumber() - 1);
   fprintf (stream, "\n");
  }
  ...
  for (ireg = 1; ireg<= nregions; ireg++) {
   ...
   for (ielem = 1; ielem <= nelem; ielem++) {
    elem = d->giveElement(ielem);
    if (elem->giveRegionNumber() != ireg) continue;
    nelemNodes = elem->giveNumberOfNodes();
    fprintf (stream, "%d ",nelemNodes);
    for (i=1; i<=nelemNodes; i++) fprintf (stream, "%d ", regionNodalNumbers.at(elem ->giveNode (i)->giveNumber())-1);
    fprintf (stream, "\n");
   }
  }
...
{
 if (cellType == 10) return 4;
 else if (cellType == 5) return 3;
 else if (cellType == 9) return 4;
 else if (cellType == 12) return 8;
 else {
...
VTKExportModule::giveElementCell (IntArray& answer, Element* elem, int cell)
{
 ...
 if ((elemGT == EGT_triangle_1)||(elemGT == EGT_tetra_1)||(elemGT == EGT_quad_1)||(elemGT == EGT_hexa_1)) {
  ...
  for (i=1; i<=nelemNodes; i++) answer.at(i) = elem -> giveNode (i) -> giveNumber() - 1;
 } else  {
...
VTKExportModule::giveNumberOfElementCells (Element* elem)
{
 ...
 if ((elemGT == EGT_triangle_1)||(elemGT == EGT_tetra_1)||(elemGT == EGT_quad_1)||(elemGT == EGT_hexa_1)) return 1;
 else {
   OOFEM_ERROR("VTKExportModule: unsupported element geometry type");
 }
 return 0;
...
//
{
 ...
 for (i=1; i<=nregions; i++) {
  ...
  for (j=0; j<nnodes; j++) rbrnodes += map[j];
 }
 return rbrnodes;
}
...
//  elemSides = element->giveNumberOfSides();
 ...
 if (mode == 1) {
  ...
  int i;
  for (i=1; i<=nnodes;i++)
   if (regionNodalNumbers.at(i)) answer.at(regionNodalNumbers.at(i)) = i;
  ...
  if (omode == wdmode) { // (Whole Domain)
   ...
   for (inode = 1; inode<=nnodes; inode ++) {
    ...
    if (valID == IST_DisplacementVector) {
     ...
     val = &iVal;
     for (j=1; j<= 3 ; j++) {
      iVal.at(j) = d->giveNode(inode)->giveUpdatedCoordinate(j, tStep,EID_MomentumBalance, 1.0) - 
       d->giveNode(inode)->giveCoordinate(j);
     }
     /*
    } else if (valID == IST_Velocity) {
      iVal.resize(3); iVal.zero();
      val = &iVal;
      DofManager* dman = d->giveNode(inode);
      int numberOfDofs = dman->giveNumberOfDofs();

      for (j=1 ; j<=numberOfDofs ; j++) {
        if (dman->giveDof(j)->giveDofID() == V_u) 
          iVal.at(1) = dman->giveDof(j)->giveUnknown (EID_MomentumBalance,VM_Total,tStep);
        else if (dman->giveDof(j)->giveDofID() == V_v) 
          iVal.at(2) = dman->giveDof(j)->giveUnknown (EID_MomentumBalance,VM_Total,tStep);
        else if (dman->giveDof(j)->giveDofID() == V_w) 
     ...
     */
     ...
     if (!val) {
      fprintf (stderr, "VTKExportModule::exportIntVars: internal error: node %d not belongs to any region\n",inode);
      exit(1);
     }
    }
    
    if (type == ISVT_SCALAR) {
     if (val->giveSize()) fprintf (stream, "%e ",val->at(1));
     else fprintf (stream, "%e ",0.0);
    } else if (type == ISVT_VECTOR) {
     jsize = min (3, val->giveSize());
     for (j=1; j<= jsize ; j++) fprintf (stream, "%e ",val->at(j));
     for (j=jsize+1; j<= 3 ; j++) fprintf (stream, "0.0 ");
     ...
     if (type == ISVT_TENSOR_S3) factor = 1.0;
     else if (type == ISVT_TENSOR_S3E) factor = 0.5; // engineering notation
     t.zero();
     if (regionVarMap.at(1)) t.at(1,1) = val->at(regionVarMap.at(1));
     if (regionVarMap.at(2)) t.at(2,2) = val->at(regionVarMap.at(2));
     if (regionVarMap.at(3)) t.at(3,3) = val->at(regionVarMap.at(3));
     ...
     int ii,jj;
     for (ii=1; ii<=3; ii++) {
      for (jj=1; jj<=3; jj++) fprintf (stream, "%e ",t.at(ii,jj));
      fprintf (stream, "\n");
     }
    ...
    for (inode = 1; inode<=regionDofMans; inode ++) {
     ...
     if (type == ISVT_SCALAR) {
      ...
      for (ii=1; ii<=regionVarMap.giveSize(); ii++) {
       ...
       if ((ii==1) && iii) t.at(1,1) = val->at(iii);
       else if ((ii==2) && iii) t.at(2,2) = val->at(regionVarMap.at(2));
       else if ((ii==3) && iii) t.at(3,3) = val->at(regionVarMap.at(3));
       else if ((ii==4) && iii) {t.at(2,3) = val->at(regionVarMap.at(4));t.at(3,2) = val->at(regionVarMap.at(4));} 
       else if ((ii==5) && iii) {t.at(1,3) = val->at(regionVarMap.at(5));t.at(3,1) = val->at(regionVarMap.at(5));}
       else if ((ii==6) && iii) {t.at(1,2) = val->at(regionVarMap.at(6));t.at(2,1) = val->at(regionVarMap.at(6));}
      }
      for (ii=1; ii<=3; ii++) {
       for (jj=1; jj<=3; jj++) fprintf (stream, "%e ",t.at(ii,jj));
       fprintf (stream, "\n");
...
VTKExportModule::exportPrimVarAs (UnknownType valID, FILE* stream, TimeStep* tStep)
{
 ...
 for (inode = 1; inode<=nnodes; inode ++) {
   ...
   if ((valID == DisplacementVector) || (valID == VelocityVector)) {
     ...
     for (j=1 ; j<=numberOfDofs ; j++) {
       id = dman->giveDof(j)->giveDofID();
       if ((id == P_f)) iVal.at(1) = dman->giveDof(j)->giveUnknown (EID_ConservationEquation,VM_Total,tStep);
     }
   ...
   if (type == ISVT_SCALAR) {
    if (iVal.giveSize()) for (j=1; j<= nScalarComp ; j++) fprintf (stream, "%e ",iVal.at(j));
    else fprintf (stream, "%e ",0.0);
   } else if (type == ISVT_VECTOR) {
    jsize = min (3, iVal.giveSize());
    for (j=1; j<= jsize ; j++) fprintf (stream, "%e ",iVal.at(j));
    for (j=jsize+1; j<= 3 ; j++) fprintf (stream, "0.0 ");
   }
   fprintf (stream, "\n");
  }

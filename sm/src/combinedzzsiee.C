/* $Header: /home/cvs/bp/oofem/sm/src/combinedzzsiee.C,v 1.5 2003/04/06 14:08:30 bp Exp $ */
/*
...
CombinedZZSIErrorEstimator::giveElementError (EE_ErrorType type, Element* elem, TimeStep* tStep)
{
 ...
 if (type == indicatorET) return siee.giveElementError (type, elem, tStep);
 else return zzee.giveElementError (type, elem, tStep);
...
CombinedZZSIRemeshingCriteria::giveRequiredDofManDensity (int num, TimeStep* tStep, int relative) 
{
 ...
 if (indicatorVal > dirc.giveMinIndicatorLimit()) {
  ...
  } else return proposedDensity;
 } else return zzrc.giveRequiredDofManDensity (num, tStep, relative);
...
//if ((s1 == RemeshingFromPreviousState_RS) || (s2 == RemeshingFromPreviousState_RS)) return RemeshingFromPreviousState_RS;
 if ((s1 == RemeshingFromPreviousState_RS) || (s2 == RemeshingFromPreviousState_RS)) return RemeshingFromCurrentState_RS;
 else if ((s1 == RemeshingFromCurrentState_RS) || (s2 == RemeshingFromCurrentState_RS)) return RemeshingFromCurrentState_RS;
 else return NoRemeshing_RS;
}

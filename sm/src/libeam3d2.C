   ...
   // Sets up the array of Gauss Points of the receiver.
{
  ...
  integrationRulesArray[0]->setUpIntegrationPoints (_Line, 1 , this,  _3dBeam);
 ...
 // luated at aGaussPoint.
{
 ...
 answer.at(1,1) = n1 ; answer.at(1,7) = n2 ;
 answer.at(2,2) = n1 ; answer.at(2,8) = n2 ;
 answer.at(3,3) = n1 ; answer.at(3,9) = n2 ;

 answer.at(4,4) = n1 ; answer.at(4,10) = n2 ;
 answer.at(5,5) = n1 ; answer.at(5,11) = n2 ;
 answer.at(6,6) = n1 ; answer.at(6,12) = n2 ;
...
/*
void
LIBeam3d2 :: computeTemperatureStrainVectorAt (FloatArray& answer, GaussPoint* gp, TimeStep* stepN, ValueModeType mode)
{
...
LIBeam3d2 ::     updateTempTriad (TimeStep* tStep) {
 ...
 if (tStep->giveSolutionStateCounter() == tempTcCounter) return;
 ...
 if (isT_NtoG) u.rotatedWith(T_NtoG,'n') ;
...
*/
 ...
 if (this->nlGeometry == 0) return NLStructuralElement::updateRotationMatrix();

 if (rotationMatrix == NULL) rotationMatrix = new FloatMatrix();
 ...
 if (isT_GtoL && T_NtoG.isNotEmpty()) rotationMatrix->beProductOf(T_GtoL,T_NtoG);
 else if (isT_GtoL) *rotationMatrix = T_GtoL;
 else if (T_NtoG.isNotEmpty()) *rotationMatrix = T_NtoG;
 else {
  int i, ndofs = this->computeNumberOfDofs(EID_MomentumBalance);
  rotationMatrix->resize (ndofs, ndofs);
  for (i=1; i<= ndofs; i++) rotationMatrix->at (i,i) = 1.0;
 }
...
LIBeam3d2 ::  computeRotMtrx (FloatMatrix& answer, FloatArray& psi) {
 ...
 if (psi.giveSize() != 3) _error ("computeSMtrx: psi param size mismatch");
...
LIBeam3d2 :: computeSMtrx (FloatMatrix& answer, FloatArray& vec) {
 if (vec.giveSize() != 3) _error ("computeSMtrx: vec param size mismatch");
...
LIBeam3d2 :: computeStrainVector (FloatArray& answer, GaussPoint* gp,TimeStep* tStep) {
 ...
 if (this->updateRotationMatrix()) ui.rotatedWith(*this->rotationMatrix, 'n') ;
...
LIBeam3d2 :: drawScalar   (oofegGraphicContext& context)
{
  ...
  if (giveIPValue (v, gp, context.giveIntVarType(), tStep) == 0 ) return;

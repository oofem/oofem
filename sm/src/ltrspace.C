   ...
   // luated at aGaussPoint.
{
 int i;
 FloatMatrix dn (4,3);
 interpolation.evaldNdx (dn, domain, dofManArray, *aGaussPoint->giveCoordinates(), 0.0);
...
LTRSpace :: initializeFrom (InputRecord* ir)
{
 ...
 IR_GIVE_OPTIONAL_FIELD (ir, numberOfGaussPoints, IFT_LTRSpace_nip, "nip"); // Macro
  ...
  if ((numberOfGaussPoints <= 1) ||  (numberOfGaussPoints >3)) numberOfGaussPoints = 1; 
  if ( numberOfGaussPoints == 0) numberOfGaussPoints = 1;
   ...
   // Returns the lumped mass matrix of the receiver.
{
   ...
   for (int i = 1; i<= 12 ; i++)  answer.at(i,i) = mss1 ;
...
void
LTRSpace :: ZZNodalRecoveryMI_ComputeEstimatedInterpolationMtrx  
...
LTRSpace::SPRNodalRecoveryMI_giveDofMansDeterminedByPatch (IntArray& answer, int pap)
{
 ...
 for (i=1; i<=4; i++) if (this->giveNode(i)->giveNumber() == pap) found = 1;
...
LTRSpace::DirectErrorIndicatorRCI_giveCharacteristicSize () {
 FloatArray lc (4);
 double volume = interpolation.giveTransformationJacobian(domain, dofManArray, lc, 0.0);
                                ...
                                InternalStateType type, TimeStep* tStep)
{
 ...
 l1 = lcoords.at(1); l2 = lcoords.at(2); l3 = lcoords.at(3); l4 = 1.0 - l1 - l2 - l3;
...
LTRSpace :: GetSurfaceIntegrationRule (int approxOrder)
{
 ...
 iRule ->setUpIntegrationPoints (_Triangle, npoints, this, _Unknown);
 return iRule;
}

/* $Header: /home/cvs/bp/oofem/sm/src/huertaerrorestimator.C,v 1.2.4.1 2004/04/05 15:19:46 bp Exp $ */
/*
...
HuertaErrorEstimator::estimateError (EE_ErrorMode mode, TimeStep* tStep)
{
 ...
 if(this -> stateCounter == tStep -> giveSolutionStateCounter())return 1;
...
// freopen("/dev/null", "w", stdout);
 ...
 if(wError == true)OOFEM_LOG_INFO("Global wNorm2: %15.8e\n", globalWENorm);
 ...
 if(maxSkipSteps != 0 && this -> mode == HEE_nlinear){
   if(lastError < 0.0){
     ...
     if(requiredError > pe){
       ...
       if(skippedSteps == 0)stepsToSkip--;
       if(stepsToSkip > maxSkipSteps)stepsToSkip = maxSkipSteps;
       if(stepsToSkip < 0)stepsToSkip = 0;
     }
     else{
...
/* HUHU CHEATING do not return just by one step */
	 ...
	 // it would be much cleaner to call restore from engng model
				 while(stepNumber < curNumber){
					 ...
					 if(lastError > requiredError)return 1;
 ...
 if(wError == true)this -> globalWENorm = sqrt(this -> globalWENorm);
...
HuertaErrorEstimator::giveElementError (EE_ErrorType type, Element* elem, TimeStep* tStep)
{
 ...
 if(type == primaryUnknownET)return this -> eNorms.at(elem -> giveNumber());
 return 0.0;
}
...
{
 this -> estimateError(equilibratedEM, tStep);
 if(type == globalErrorEEV)return this -> globalENorm; 
 else if(type == globalNormEEV)return this -> globalUNorm;
 else if(type == globalWeightedErrorEEV)return this -> globalWENorm;
 return 0.0;
}
...
HuertaErrorEstimator :: initializeFrom (InputRecord* ir)
{
 ...
 if(level >= 0)this -> refineLevel = level;
 ...
 if(masterRun == true){         // prevent overwriting of static variables
  ...
  if(n > 0){
   exactFlag = true;
   if(n != 1)huertaFlag = true;            // run also error estimate
  }
  else
   exactFlag = false;
#endif
...
HuertaRemeshingCriteria::giveRequiredDofManDensity (int num, TimeStep* tStep, int relative) 
{
 ...
 if(relative)return size / this -> giveDofManDensity(num);
 return size;
}
...
HuertaRemeshingCriteria::estimateMeshDensities (TimeStep* tStep)
{
 ...
 for(i = 0; i < nnode; i++)connectedElems[i] = 0;
...
// limit the coarsening
   if(iratio < 1.0 / MAX_COARSE_RATE)iratio = 1.0 / MAX_COARSE_RATE;
  }
...
// huerta + error indicator
	...
	if(maxVal > 0.75)elemSize = min(elemSize, 1.0);
...
// Element *element;
 ...
 if(this -> refinedMesh.completed == 1)return;
...
#ifdef EXACT_ERROR
     if(wholeFlag == true)bc = 0;
#endif

     if(bc == 1)localBcId += dofs;
    }
   }
  }
...
// it is necessary to reproduce bc from coarse mesh
       ...
       if(m == 0){                                // at node
        ...
        for(idof = 1; idof <= dofs; idof++){
         ...
         if(nodeDof -> hasBc(tStep) != 0)bcDofId = nodeDof -> giveBcIdValue();
...
// count boundary conditions (for element (nodeId == 0), for patch (nodeId != 0))
      ...
      if(nodeId == 0){
       if(m == 0 && boundary.at(1) == 0)bc = 1;
       if(n == 0 && boundary.at(2) == 0)bc = 1;
      }
      else{
       if(n == level + 1 || m == level + 1)bc = 1;
...
// copy node load
         ...
         index = 0;
         if(m == 0 && sideNumBc.at(1) != 0)index = 1;       // along next side
         if(n == 0 && sideNumBc.at(2) != 0)index = 2;       // along prev side
         if(index != 0){
          sprintf(line, " bc %d", dofs);
          str += line;
...
// boundary load is not copied on non-boundary sides
     ...
     if(hasLoad == true && (m == 0 || n == 0)){
      if(m == 0 && n == 0){
       loads = 0;
       for(iside = 1; iside <= 2; iside++){
        if(boundary.at(iside) == 0)continue;
       ...
       if(loads != 0){
        ...
        for(iside = 1; iside <= 2; iside++){
         if(boundary.at(iside) == 0)continue;
         if((loads = (loadArray = boundaryLoadList.at(iside)) -> giveSize()) == 0)continue;
         for(iload = 1; iload <= loads; iload++){
          sprintf(line, " %d", loadArray -> at(iload));
          str += line;
         }
       ...
       index = 0;
       if(m == 0 && boundary.at(1) != 0 && boundaryLoadList.at(1) -> giveSize() != 0)index = 1;
       if(n == 0 && boundary.at(2) != 0 && boundaryLoadList.at(2) -> giveSize() != 0)index = 2;
       if(index != 0){
...
// count boundary conditions (for element (nodeId == 0), for patch (nodeId != 0))
       ...
       if(nodeId == 0){
        if(m == 0 && boundary.at(1) == 0)bc = 1;
        if(n == 0 && boundary.at(2) == 0)bc = 1;
        if(k == 0 && boundary.at(3) == 0)bc = 1;
       }
       else{
        if(k == level + 1 || n == level + 1 || m == level + 1)bc = 1;
...
// copy node load
          ...
          if((m == 0 && n == 0) || (m == 0 && k == 0) || (n == 0 && k == 0)){
           index = 0;
           if(n == 0 && k == 0 && sideNumBc.at(1) != 0)index = 1;
           if(m == 0 && k == 0 && sideNumBc.at(2) != 0)index = 2;
           if(m == 0 && n == 0 && sideNumBc.at(3) != 0)index = 3;
           if(index != 0){
            sprintf(line, " bc %d", dofs);
            str += line;
...
// I rely on the fact that bc dofs to be reproduced are ordered with respect to the dof ordering of the corner node
           ...
           index = 0;
           if(m == 0 && faceNumBc.at(1) != 0)index = 1;
           if(n == 0 && faceNumBc.at(2) != 0)index = 2;
           if(k == 0 && faceNumBc.at(3) != 0)index = 3;
           if(index != 0){
            sprintf(line, " bc %d", dofs);
            str += line;
...
// boundary load is not copied on non-boundary sides
      ...
      if(hasLoad == true && (m == 0 || n == 0 || k == 0)){
       ...
       for(iside = 1; iside <= 3; iside++){
        if(boundary.at(iside) == 0)continue;
        if(m != 0 && iside == 1)continue;
        if(n != 0 && iside == 2)continue;
        if(k != 0 && iside == 3)continue;
        loads += boundaryLoadList.at(iside) -> giveSize();
       }
       if(loads != 0){
        sprintf(line, " boundaryLoads %d", loads);
                        ...
                        TimeStep* tStep)
{
 ...
 if(parMode == DofManager_remote || parMode == DofManager_null)return;
#endif
...
/* HUHU CHEATING */
  ...
  if(this -> skipRegion(element -> giveRegionNumber()) != 0)skipped++;
 }
...
/* HUHU CHEATING */
  ...
  for(inode = 1; inode <= element -> giveNumberOfNodes(); inode++){
   if(element -> giveNode(inode) -> giveNumber() != nodeId)continue;
...
// store fine solution in primaryUnknownError
 for(inode = 1; inode <= localNodeId; inode++){
  ...
  for(idof = 1; idof <= dofs; idof++)primaryUnknownError.at((pos - 1) * dofs + idof) = nodeSolution.at(idof);
 }
#ifdef TIME_INFO
 ::getRelativeUtime(et_error, st_error);
 ::getRelativeUtime(et_total, st_total);
...
*/
  ...
  int dim, pos, nelems;
		...
		while(--dim)nelems *= this -> refineLevel + 1;
                    ...
                    int dofs, TimeStep* tStep)
{
 ...
 for(inode = 1; inode <= element -> giveNumberOfNodes(); inode++){
  ...
  for(idof = 1; idof <= dofs; idof++)answer.at(++p) = vector.at(pos + idof);
 }
}  
  ...
  if(problemId != 0){
   ...
   if(controlMode == 1){
    ...
    for(i = 1; i <= ddmSize; i += 2){
     if(localNodeIdArray.at(ddm.at(i)) == 0)continue;
     ddActiveSize++;
    }
   }
   ...
   if(controlMode == 1){
    ...
    for(i = 1; i <= ddvSize; i++){
     if(localNodeIdArray.at(ddm.at(2 * i - 1)) == 0)continue;

/* $Header: /home/cvs/bp/oofem/sm/src/Attic/masonry02.C,v 1.1.2.1 2004/04/05 15:19:47 bp Exp $ */
/*
...
Masonry02:: initializeFrom (InputRecord* ir)
{
 ...
 if (ir->hasField(IFT_Masonry02_cplane, "cplane")) this->rmType = mpm_CuttingPlane;
                                                  ...
                                                  const FloatArray& dlambda)
{
  ...
  int indx;
  answer.resize(3,actSurf); answer.zero();
  ...
  if ((indx=activeConditionMap.at(3))) {
    if (dlambda.at(3) < 0.) answer.at(3,indx) = 0.0;
    else answer.at(3,indx)= sqrt(p1*p1+p2*p2);
  }    
}
...
//
{
 //MaterialMode mode  = gp -> giveMaterialMode ();
  ...
  if (mmode == _2dInterface) return ind;
  else return MPlasticMaterial2::giveStressStrainComponentIndOf(form, mmode, ind);
}
  ...
  //
{
 ...
 if (mmode == _2dInterface) {
   answer.resize (2);
   for (i = 1; i<=2; i++) answer.at(i) = i;
 } else MPlasticMaterial2::giveStressStrainMask(answer, form, mmode);
...
//
{
 ...
 if (mode == _2dInterface) {
   answer = charVector3d;
   return ;
 } else MPlasticMaterial2::giveReducedCharacteristicVector(answer,gp,charVector3d);
}
...
{ 
 MaterialMode mode = gp-> giveMaterialMode ();
 if (mode == _2dInterface) {answer = strainVector; return; }
 else MPlasticMaterial2::giveFullCharacteristicVector(answer, gp, strainVector);
}
...
Masonry02::computeF3HardeningGradient (double k)
{
  ...
  */
  ...
  if (k<0.) {
    double grad = -2.0*((sic-spc)/kp/kp)*kp;
    if ((sic+grad*k) <=0.) return 0.; else return grad;
  } else if ((k>=0.)&&(k<kp)) {
    return 2.*k*(sic-spc)/kp/kp-2.0*((sic-spc)/kp/kp)*kp;
    //return (spc-sic)*2.0*(1./kp-k/kp/kp)/(2.0*sqrt(2.*k/kp-k*k/kp/kp));
  } else if ((k>=kp)&&(k<km)) {

...
{
 if (stiffnessMatrix) delete stiffnessMatrix;
...
*/
 if ((iStep > 0) && (iStep <= discreteTimes.giveSize()))return (discreteTimes.at(iStep));
 ...
 if (tStep->giveNumber() == this->giveNumberOfFirstStep()) {
  ...
  for (i=1;i<=ndofs;i++)  {
   ...
   if (iDof->hasBc(tStep))           // bound . cond.
    //val = iDof -> giveBcValue() -> give(DisplacementVector,IncrementalMode,tStep) ;
    val = iDof -> giveBcValue (VM_Incremental,tStep) ;
   else  val = this->incrementOfDisplacementVector.at(iDof->giveEquationNumber());

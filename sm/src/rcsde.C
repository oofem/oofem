/* $Header: /home/cvs/bp/oofem/sm/src/rcsde.C,v 1.3.4.1 2004/04/05 15:19:47 bp Exp $ */
/*
...
//
{
 ...
 if (status->giveTempMode () == RCSDEMaterialStatus:: rcMode) {
  ...
  if ((rMode == TangentStiffness) || (rMode == SecantStiffness))  {
   ...
   if (dCoeff < RCSDE_DAMAGE_EPS) dCoeff = RCSDE_DAMAGE_EPS;
   reducedAnswer.times(dCoeff);
   
   if (form == ReducedForm) answer = reducedAnswer;
   else {
...
RCSDEMaterial :: initializeFrom (InputRecord* ir)
{
 ...
 IR_GIVE_FIELD (ir, SDTransitionCoeff, IFT_RCSDEMaterial_sdtransitioncoeff, "sdtransitioncoeff"); // Macro
...
contextIOResultType
RCSDEMaterial :: saveContext (FILE* stream, void *obj)
//
...
contextIOResultType 
RCSDEMaterial :: restoreContext (FILE* stream, void *obj)
// 
//
...
double
RCSDEMaterial :: give ( int aProperty)
   // Returns the value of the property aProperty (e.g. the Young's modulus
   // 'E') of the receiver.
{
  if (aProperty == pscm_SDTransitionCoeff) return this->SDTransitionCoeff;
...
contextIOResultType
RCSDEMaterialStatus :: saveContext (FILE* stream, void *obj)
//
...
//
{
 ...
 if (fwrite(&rcsdMode,sizeof(__rcsdModeType),1,stream)!= 1) THROW_CIOERR(CIO_IOERR);
 if (fwrite(&transitionEps,sizeof(double),1,stream)!= 1) THROW_CIOERR(CIO_IOERR);
 if (fwrite(&epsF2,sizeof(double),1,stream)!= 1) THROW_CIOERR(CIO_IOERR);
...
contextIOResultType
RCSDEMaterialStatus :: restoreContext(FILE* stream, void *obj)
//
...
//
{
 ...
 if (fread(&rcsdMode,sizeof(__rcsdModeType),1,stream)!= 1) THROW_CIOERR(CIO_IOERR);
 if (fread(&transitionEps,sizeof(double),1,stream)!= 1) THROW_CIOERR(CIO_IOERR);
 if (fread(&epsF2,sizeof(double),1,stream)!= 1) THROW_CIOERR(CIO_IOERR);

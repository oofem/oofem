/* $Header: /home/cvs/bp/oofem/sm/src/m4.C,v 1.3.4.1 2004/04/05 15:19:47 bp Exp $ */
/*
            ...
            TimeStep* tStep)
{
 ...
 strainIncrement.substract (status->giveStrainVector ());
  if (!previousStress.isNotEmpty()) {previousStress.resize (4); previousStress.zero();}
  ...
  */
  ...
  if (SEL>F) answer.at(3)=F;
  else if (SEL<-F) answer.at(3)=-F; 
  else answer.at(3)=SEL;  
  
  if (SEM>F) answer.at(4)=F; 
  else if (SEM<-F) answer.at(4)=-F;
  else answer.at(4)=SEM;
...
{
  if (mode == _3dMicroplane) return 4;

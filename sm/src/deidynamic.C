...
// for first time step we need special start code
 Domain* domain = this->giveDomain(1);
  ...
  massMatrix.resize(neq); massMatrix.zero();
   ...
   for (j=1 ; j<=n; j++) {
    if (charMtrx.at(j,j) > ZERO_MASS) {
     ...
     if (init) maxOmEl = (maxOmEl > maxOmi) ? (maxOmEl) : (maxOmi) ; 
  ...
  nextDisplacementVector.resize (neq); nextDisplacementVector.zero();
  ...
  accelerationVector.resize (neq); accelerationVector.zero();
    ...
    for (j = 1; j <= neq; j++) nextDisplacementVector.at(j) -= velocityVector.at(j)*(deltaT);
  return ;
  }   // end of init step
  ...
  for (i=1; i<=nelem; i++)
   {
    ...
    for (j=1; j<=n; j++) {
     ...
     if (jj) {
      for (k=1; k<=n; k++) {
       kk = loc.at(k);
       if (kk) loadVector.at(jj) -= charMtrx.at(j,k) * displacementVector.at(kk);
      }
      // 

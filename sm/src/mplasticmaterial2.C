/* $Header: /home/cvs/bp/oofem/sm/src/Attic/mplasticmaterial2.C,v 1.1.2.1 2004/04/05 15:19:47 bp Exp $ */
/*
  ...
  // completely formulated in Reduced stress-strain space
{
  ...
  */
  ...
  for (i=1; i<=nsurf; i++) if (gamma.at(i) > 0.) yieldFlag=true;
  if (yieldFlag)  newState = MPlasticMaterial2Status::PM_Yielding; // test if plastic loading occur
  // no plastic loading - check for unloading
  else if ((state == MPlasticMaterial2Status::PM_Yielding) || (state == MPlasticMaterial2Status::PM_Unloading)) newState = MPlasticMaterial2Status::PM_Unloading; 
  else newState = MPlasticMaterial2Status::PM_Elastic;
  status-> letTempStateFlagBe (newState);
  
  if (form == FullForm) {answer = fullStressVector; return ;}
  else {
                                        ...
                                        TimeStep* atTime)
{
 ...
 huhu:
  ...
  dgamma.resize(nsurf); dgamma.zero(); 
  gamma.resize(nsurf); gamma.zero(); 
  ...
  //
  elastic = 1;
  actSurf = 0; activeConditionMap.zero();
  if (init) {
  ...
  for (i=1; i<= this->nsurf; i++) {
      ...
      if (yieldValue > YIELD_TOL) {
        elastic = 0;
        if (init) initialConditionMap.at(i) = 1;
    ...
    do {
      do { // restart loop
        ...
        if (debug) {
          ...
          for (i=1; i<=nsurf; i++) printf ("%d ", activeConditionMap.at(i));
      ...
      // check convergence
        ...
        for (i=1; i<=nsurf; i++) {
          if (activeConditionMap.at(i)) {
            yieldValue = this-> computeYieldValueAt (gp, i, fullStressVector, strainSpaceHardeningVariables);
            if ((fabs(yieldValue) >= YIELD_TOL)) yieldConsistency=false;
#ifdef DEBUG
            if (debug) printf ("%e(%e) ", yieldValue, gamma.at(i));
#endif
          }
        }
#ifdef DEBUG
...
#ifdef DEBUG
          if (debug) printf ("Consistency: ");
#endif
          for (i=1; i <= this->nsurf; i++) {
            yieldValue = this-> computeYieldValueAt (gp, i, fullStressVector, strainSpaceHardeningVariables);

#ifdef DEBUG
            if (debug) printf ("%e ", yieldValue);
#endif
	  ...
	  if (debug) {
	    printf ("\nStress: ");
	    for (i=1; i<=fullStressVector.giveSize(); i++) printf (" %e", fullStressVector.at(i));
	    printf ("\nKappa : ");
	    for (i=1; i<=strainSpaceHardeningVariables.giveSize(); i++) printf (" %e", strainSpaceHardeningVariables.at(i));
          ...
          if (restart) {
            ...
            if (this->getNewPopulation(activeConditionMap,newmap,this->giveMaxNumberOfActiveYieldConds(gp),nsurf)==0) {
              ...
              if (!debug) {
                ...
                printf ("ep:"); status -> givePlasticStrainVector(plasticStrainVectorR); plasticStrainVectorR.printYourself();
                printf ("kp:"); status -> giveStrainSpaceHardeningVars(strainSpaceHardeningVariables);strainSpaceHardeningVariables.printYourself();
                elasticStrainVectorR = totalStrain;
                elasticStrainVectorR.substract(plasticStrainVectorR);
                printf ("sg:"); this-> computeTrialStressIncrement (fullStressVector, gp, elasticStrainVectorR, atTime); 
            ...
            for (actSurf=0, i=1; i<=nsurf; i++) if (activeConditionMap.at(i)) actSurf++;
            /*
            if (actSurf) {
        ...
        rhs.resize(actSurf); rhs.zero();
        gmat.resize(actSurf,actSurf); gmat.zero();
        lmat.resize(actSurf,strSize); lmat.zero();
        rmat.resize(strSize,actSurf); rmat.zero();
        if (hasHardening) {
        ...
        for (i=1; i<=nsurf; i++) {
          if ((indx = activeConditionMap.at(i))) {
            ...
            if (hasHardening) {
              ...
              for (j=1; j<=strSize; j++) lmat.at(indx,j)=helpVector.at(j);
            } else {
              for (j=1; j<=strSize; j++) lmat.at(indx,j)=yieldGradSigVec[i-1].at(j);
            }
            ...
            for (j=1; j<=strSize; j++) rmat.at(j,indx)+=(*loadGradSigVecPtr)[i-1].at(j);

            if (hasHardening) {
              helpVector2.beTProductOf(kl, yieldGradKVec[i-1]);
              for (j=1; j<=actSurf; j++) gmat.at(indx,j)=(-1.0)*helpVector2.at(j);
            }
            yieldValue = this-> computeYieldValueAt (gp, i, fullStressVector, strainSpaceHardeningVariables);
      ...
      gamma.add(dgamma);
      for (i=1; i<= strSize; i++) plasticStrainVectorR.at(i) += helpVector2.at(i);
      ...
      if (nIterations > PLASTIC_MATERIAL_MAX_ITERATIONS) {
        ...
        for (i=1; i<=nsurf; i++) fprintf (stderr, "%d ", activeConditionMap.at(i));
        fprintf (stderr, "\n");
        if (!debug) {
          debug = true;
          this->clearPopulationSet();
            ...
            if (initialConditionMap.at(i)) {
              ...
              if (this->getNewPopulation(activeConditionMap,newmap,this->giveMaxNumberOfActiveYieldConds(gp),nsurf)==0) {
                ...
                goto huhu;
                  ...
                  printf ("ep:"); status -> givePlasticStrainVector(plasticStrainVectorR); plasticStrainVectorR.printYourself();
                  printf ("kp:"); status -> giveStrainSpaceHardeningVars(strainSpaceHardeningVariables);
  ...
  init = false;
	...
	if (!elastic) {
        ...
        gmat.plus(helpMtrx2);
          ...
          if (restart) {
			...
			for (i=1; i<= this->nsurf; i++) {
				if (activeConditionMap.at(i)) {
					(*loadGradSigVecPtr)[i-1].times(dgamma.at(i));
          for (j=1;j<=strSize; j++) helpVector.at(j) += (*loadGradSigVecPtr)[i-1].at(j);
				}
			}
      ...
      for (i=1; i<=nsurf; i++) {
        if (activeConditionMap.at(i)) {
          yieldValue = this-> computeYieldValueAt (gp, i, fullStressVector, strainSpaceHardeningVariables);
          if ((yieldValue >= YIELD_TOL)) yieldConsistency=false;
        }
      }
                                         ...
                                         std::vector<FloatArray>& gradientVectorR)
{
  ...
  for (i=1; i<=this->nsurf; i++) {
    if (activeConditionMap.at(i)) 
      for (j=1; j<=size; j++) answer.at(j) += gamma.at(i)*gradientVectorR[i-1].at(j);
  }
}
                                              ...
                                              const FloatArray& strainSpaceHardeningVariables)
{
  ...
  */
  ...
  for (i=1; i<=size; i++) {
    for (j=1; j<=size; j++) helpInverse.at(i,j) += elasticModuliInverse.at(i,j);
  }

  // prevent too big numbers
  for (i=1; i<=size; i++) if (fabs(helpInverse.at(i,i)) > 1.e16) helpInverse.at(i,i) = sgn(helpInverse.at(i,i))*1.e16;
                                                   ...
                                                   TimeStep* atTime)
{
 ...
 if (this->plType == associatedPT) {
  ...
  for (i=1; i<=nsurf; i++) {if (activeConditionMap.at(i)) actSurf++;}
  ...
  umat.resize(strSize, actSurf); umat.zero();
  vmat.resize(actSurf, strSize); vmat.zero();
  gmat.resize(actSurf,actSurf);  gmat.zero();
  ...
  for (i=1; i<=nsurf; i++) {
    if ((indx = activeConditionMap.at(i))) {
      ...
      if (hasHardening) {
        helpVector.beTProductOf(ks, yieldGradKVec[i-1]);
        helpVector.add(yieldGradSigVec[i-1]);
        for (j=1; j<=strSize; j++) vmat.at(indx,j)=helpVector.at(j);
      } else {
        for (j=1; j<=strSize; j++) vmat.at(indx,j)=yieldGradSigVec[i-1].at(j);
      }
      ...
      for (j=1; j<=strSize; j++) umat.at(j,indx)+=((*loadGradSigVecPtr)[i-1]).at(j);
...
MPlasticMaterial2::getPopulationSignature (IntArray &mask)
{
  ...
  if (size > (int) (sizeof (long)*8)) printf ("MPlasticMaterial2: too many yield conditions");
  
  for (int i = 1; i<= mask.giveSize(); i++) {
    if (mask.at(i)) val |= (1L<<(i-1));
  }
...
{
  if (populationSet.find (pop) != populationSet.end()) return 0;
  else return 1;
}
...
MPlasticMaterial2::getNewPopulation (IntArray& result, IntArray &candidateMask, int degree, int size)
{
  ...
  for (i=1; i<=candSize; i++) if (candidateMask.at(i)) candDegree++;
  result.resize(size);
  ...
  if ((candDegree <= degree)) {
    ...
    int j=1; IntArray candidateArray(candDegree);
    IntArray ind(degree);
    for (i=1; i<=candSize; i++) if (candidateMask.at(i)) candidateArray.at(j++) = i;
    // first try first degree candidates
    int activeIndx = degree;
    for (i=1; i<=degree; i++) ind.at(i) = i;
    ...
    do {
      ...
      for (i=1; i<=degree; i++) {
        ...
        if (!result.at(ii)) result.at(ii) = count++;
      }
      // order result array
      count = 1;
      for (i=1; i<=size; i++) if (result.at(i)) result.at(i) = count++;
...
MPlasticMaterial2Status :: printOutputAt  (FILE *file, TimeStep* tStep)
{
 ...
 if ((state_flag == MPlasticMaterial2Status::PM_Yielding) || (state_flag == MPlasticMaterial2Status::PM_Unloading)) {
   if (state_flag == MPlasticMaterial2Status::PM_Yielding) fprintf(file," Yielding, ");
   else fprintf(file," Unloading, ");
...
contextIOResultType
MPlasticMaterial2Status :: saveContext (FILE* stream, void *obj)
//
...
contextIOResultType
MPlasticMaterial2Status :: restoreContext(FILE* stream, void *obj)
//

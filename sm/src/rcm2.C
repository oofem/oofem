/* $Header: /home/cvs/bp/oofem/sm/src/rcm2.C,v 1.4.4.1 2004/04/05 15:19:47 bp Exp $ */
/*
...
*/   
   ...
   //
   elastStrain = principalStrain; elastStrain.substract (crackStrainVector);
   sigmaEl.beProductOf (de, elastStrain);
   sigmaCr.zero();
   //delete elastStrain;
  }
  ...
  for (i = 1; i <= dSigma.giveSize(); i++) {
   if (fabs(dSigma.at(i)) > maxErr) maxErr = fabs(dSigma.at(i));
  }
...
//
{
 ...
 for (i=1; i<=3; i++) { // loop over each possible crack plane
   ...
   if ((crackMap.at(i) == 0) && 
    ...
    status->giveTempCrackDirs (tempCrackDirs);
       for (j=1;j<=3;j++) crackPlaneNormal.at(j) = tempCrackDirs.at(j,i);
   ...
   upd = 1;
     ...
     if (localStress.at(i) > initStress) {
       ...
       answer.at(i) = 1;
       activationFlag = 1;
   if (upd) this->updateStatusForNewCrack (gp,i,Le);
     }
   } // end of tested crack 
...
//
{
 ...
 if (isClosing) this->updateActiveCrackMap(gp);
...
//
{
 ...
 if (form == FullForm) // 3x3 full form required
  ...
  sd = 0;
  for (i=1; i<=3;i++)
   if ((this->giveStressStrainComponentIndOf(FullForm,gp->giveMaterialMode(),i))) sd++;
...
{ 
 ...
 status ->giveCrackMap (crackMap);
 if (numberOfActiveCracks == 0) {answer.resize (0,0) ; return; }
   ...
   // 'E') of the receiver.
{
 ...
 if (aProperty == pscm_Gf) return this->Gf;
  // if (aProperty == pscm_Beta) return this->beta;
 if (aProperty == pscm_Ft) return this->Ft;
 if (aProperty == pscm_Ee) aProperty = 'E';
 if (aProperty == pscm_G ) aProperty = 'G';
...
RCM2Material::giveIPValue (FloatArray& answer, GaussPoint* aGaussPoint, InternalStateType type, TimeStep* atTime)
{
 ...
 if (type == CrackedFlag) {
   answer.resize(1); answer.at(1) =  status->giveAlreadyCrack();
  return 1;
  ...
  for (int i=1; i<=3; i++) answer.at(i) = crackStatus.at(i);
  return 1;
 } else 
...
{
 if (type == CrackedFlag) return ISVT_SCALAR;
 else if (type == CrackDirs) return ISVT_VECTOR;
 else if (type == CrackStatuses) return ISVT_VECTOR;
...
RCM2Material::giveIntVarCompFullIndx (IntArray& answer, InternalStateType type, MaterialMode mmode)
{
 if (type == CrackedFlag) {
  ...
  answer.resize(9);
  for (int i=1; i<=9; i++) answer.at(i) = i;
  return 1;
 } else if (type == CrackStatuses) {
  answer.resize(3);
  for (int i=1; i<=3; i++) answer.at(i) = i;
  return 1;
...
{
 if (type == CrackedFlag) return 1;
 else if (type == CrackDirs) return 9;
 else if (type == CrackStatuses) return 3;
...
RCM2MaterialStatus :: RCM2MaterialStatus (int n, Domain*d, GaussPoint *g) 
  ...
  for (int i=1; i<=3; i++) crackDirs.at(i,i) = tempCrackDirs.at(i,i) = 1.0;
...
//
{
 if (crackStrainVector.at(i) > 0.) return 1;
 // handle situation for new crack with crackStrainVector->at(i) = 0.
 if (crackMap.at(i)) return 1;
 return 0;
}
...
//
{
 ...
 for (i=1; i<= 3; i++) {
  if (oldCrackStrainVector.at(i)>0.) answer++;
 }

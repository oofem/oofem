...
FETISolver::estimateMaxPackSize (IntArray &map, CommunicationBuffer &buff, int &packUnpackType) 
{
 ...
 for (i=1; i<= mapSize; i++) {
  ...
  for (j=1; j<=ndofs; j++) {
   if ((eqNum = locationArray.at(j))) count++;
  }
 } 
...
FETISolver :: initializeFrom (InputRecord* ir)
{
 ...
 if (fabs(limit) < 1.e-20) limit = 1.e-20;
 if (err < 1.e-20) err = 1.e-20;
 if (err > 0.1) err = 0.1;
...
void FETISolver :: setUpCommunicationMaps ()
{
 ...
 for (i=1; i<= nnodes; i++) {
  if (domain->giveDofManager (i)->giveParallelMode () == DofManager_shared) boundaryDofManNum ++;
 }
 ...
 for (i=1; i<= size; i++) {
  ...
  for (j=1; j<=ndofs; j++) {
   if ((eqNum = locationArray.at(j))) {
    ...
    if (nse == 0) result &= send_buff->packDouble (0.0);
   }
  }
 }
 return result;
  ...
  for (i=1; i<= size; i++) {
   ...
   for (idof = 1; idof <= masterCommunicator->giveDofManager(to)->giveNumberOfDofs(); idof++) {
    for (irbm = 1; irbm <= nsem.at(receivedRank+1); irbm++) {
     ...
     if (masterCommunicator->giveDofManager(to)->giveReferencePratition() == receivedRank)
      { // contribution from reference partition localizes to all
       ...
       for (j=1; j<= nshared; j++) {
        ...
        if (part == processComm.giveRank()) continue;
   ...
   for (idof = 1; idof <= masterCommunicator->giveDofManager(to)->giveNumberOfDofs(); idof++, locpos++) {
    ...
    while (locationArray.at(locpos) == 0) 
     { locpos++;
      ...
      if (locpos > locationArray.giveSize()) _error ("Consistency dof error"); 
     }
...
FETISolver::masterMapSolution ()
{
 ...
 for (i=1; i<= size; i++) {
  ...
  for (idof = 1; idof <= masterCommunicator->giveDofManager(from)->giveNumberOfDofs(); idof++, locpos++) {
   ...
   value = 0.0;
   if (masterCommunicator->giveDofManager(from)->giveReferencePratition() == receivedRank)
    { // contribution from reference partition localizes to all
     // corresponding DOFs in boundary node
...
//  commBuff.init();
  ...
  rbmAddr.resize(size); rbmAddr.zero();
  rbmAddr.at(1) = 1;
  for (i=2; i<=size; i++) rbmAddr.at(i) = rbmAddr.at(i-1)+nsem.at(i-1);
  ...
  if (tnse) {l.resize(masterCommunicator->giveNumberOfEquations(), tnse); l.zero();}
 ...
 if (rank==0) {
  ...
  /***********************************/
   if (rank==0){
     ...
     if (tnse) this->projection (g,l,l1);
...
//#ifdef __VERBOSE_PARALLEL
     ...
     } else OOFEM_LOG_DEBUG ("iteration      gradient vector norm\n================================================\n");
//#endif
   } 
  ...
  if (rank==0) {
   ...
   denom = dotProduct (d,p,masterCommunicator->giveNumberOfEquations());
    ...
    break;
      ...
      /****************************/
      for (j=1; j<=masterCommunicator->giveNumberOfEquations(); j++) d.at(j)= beta*d.at(j)-g.at(j);
    }

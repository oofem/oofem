/* $Header: /home/cvs/bp/oofem/sm/src/refinedmesh.C,v 1.2.4.1 2004/04/05 15:19:47 bp Exp $ */
/*
...
/* note: ordering of nodes on hexa faces is important (normal: inner, outer, outer, outer, outer, inner) */
 ...
 if(fe_nodes == 0)error_message("Not enough nodes found");
...
/* setup 3D ngb elems */
 ...
 for(i = 0; i < fe_tetras; i++, fe_tetra++){
  ...
  for(j = 0; j < 4; j++){
   ...
   for(k = node_num_elems[node1 - 1]; k < node_num_elems[node1]; k++){
    elem2 = node_con_elems[k];
    if(elem2 == elem1)continue;
    if(is_tetra(elem2) == 0)continue;                                           /* macro */
    for(m = node_num_elems[node2 - 1]; m < node_num_elems[node2]; m++){
 ...
 for(i = 0; i < fe_hexas; i++, fe_hexa++){
  ...
  for(j = 0; j < 6; j++){
   ...
   for(k = node_num_elems[node1 - 1]; k < node_num_elems[node1]; k++){
    elem2 = node_con_elems[k];
    if(elem2 == elem1)continue;
    if(is_hexa(elem2) == 0)continue;                                          /* macro */
    for(m = node_num_elems[node2 - 1]; m < node_num_elems[node2]; m++){
     if(elem2 != node_con_elems[m])continue;
     for(n = node_num_elems[node3 - 1]; n < node_num_elems[node3]; n++){
 ...
 for(i = 0; i < fe_tetras; i++, tmp_tetra++){
  ...
  for(j = 0; j < 4; j++){
   if(tmp_tetra -> ngb_elem_id[j] == 0)continue;
 ...
 for(i = 0; i < fe_hexas; i++, tmp_hexa++){
  ...
  for(j = 0; j < 6; j++){
   if(tmp_hexa -> ngb_elem_id[j] == 0)continue;
...
#ifdef DEBUG
   if(node > nd)error_message("Unexpected situation");
#endif
...
/* refine mesh edges */
 ...
 for(i = 0; i < mesh_edges; i++, mesh_edge++){
  for(j = 0; j < 2; j++){
   ...
   for(k = 1; k < level + 1; k++)mesh_edge -> fine_id[j][k] = ++refine_node_id;
...
/* I do rely on the fact that mesh_edge starts with node with smaller id */
   ...
   for(m = 1; m < level + 1; m++)tmp_array_cen[j][m] = ++refine_node_id;
  ...
  for(j = 0; j < 3; j++){
   ...
   pos = 0;
   for(k = 0; k < level + 2; k++)mesh_face -> fine_id[j][pos++] = tmp_array_start[j2][k];
   for(k = 1; k < level + 1; k++){
    mesh_face -> fine_id[j][pos++] = tmp_array_end[j1][k];
    for(m = 1; m < level + 1; m++)mesh_face -> fine_id[j][pos++] = ++refine_node_id;
    mesh_face -> fine_id[j][pos++] = tmp_array_cen[j2][k];
   }
   for(k = 0; k < level + 2; k++)mesh_face -> fine_id[j][pos++] = tmp_array_cen[j1][k];
  }
 }
...
/* I do rely on the fact that mesh_edge starts with node with smaller id */
  ...
  for(j = 0; j < 4; j++){
   ...
   pos = 0;
   for(k = 0; k < level + 2; k++)mesh_quad -> fine_id[j][pos++] = tmp_array_start[j2][k];
   for(k = 1; k < level + 1; k++){
    mesh_quad -> fine_id[j][pos++] = tmp_array_end[j1][k];
    for(m = 1; m < level + 1; m++)mesh_quad -> fine_id[j][pos++] = ++refine_node_id;
    mesh_quad -> fine_id[j][pos++] = tmp_array_cen[j2][k];
   }
   for(k = 0; k < level + 2; k++)mesh_quad -> fine_id[j][pos++] = tmp_array_cen[j1][k];
  }
 }

#ifdef COMPLETE_DATA_STRUCTURE
 if(node_num_nodes != NULL)free(node_num_nodes);
 if(node_con_nodes != NULL)free(node_con_nodes);
#endif
...
/* this is really not necessary because everything is stored in mesh_edge;
 ...
 for(i = 0; i < fe_edges; i++, mesh_edge++){
  for(j = 0; j < 2; j++){
   ...
   for(k = 0; k < level + 2; k++)fine_edge -> fine_id[k] = mesh_edge -> fine_id[j][k];
  }
 }
...
/* I do rely on the fact that mesh_edge starts with node with smaller id */
   ...
   if(node1 < node2){
    ...
    for(k = node_num_nodes[node1 - 1]; k < node_num_nodes[node1]; k++){
     mesh_edge = &(mesh_edge_array[node_con_nodes[k] - 1]);
     if(mesh_edge -> node[1] -> id == node2)break;
    }
#endif
    ...
    for(k = node_num_nodes[node2 - 1]; k < node_num_nodes[node2]; k++){
     mesh_edge = &(mesh_edge_array[node_con_nodes[k] - 1]);
     if(mesh_edge -> node[1] -> id == node1)break;
    }
#endif
   ...
   tmp_array_cen[j][0] = mesh_edge -> mid_node -> id;
   for(k = 1; k < level + 1; k++)tmp_array_cen[j][k] = ++refine_node_id;
  ...
  for(j = 0; j < 3; j++){
   ...
   pos = 0;
   for(k = 0; k < level + 2; k++)fine_quad -> fine_id[pos++] = tmp_array_start[j2][k];
   for(k = 1; k < level + 1; k++){
    fine_quad -> fine_id[pos++] = tmp_array_end[j1][k];
    for(m = 1; m < level + 1; m++)fine_quad -> fine_id[pos++] = ++refine_node_id;
    fine_quad -> fine_id[pos++] = tmp_array_cen[j2][k];
   }
   for(k = 0; k < level + 2; k++)fine_quad -> fine_id[pos++] = tmp_array_cen[j1][k];
  }
 }
...
/* I do rely on the fact that mesh_edge starts with node with smaller id */
...
#ifdef COMPLETE_DATA_STRUCTURE
 if(tmp_face_array != NULL)free(tmp_face_array);
 if(tmp_quad_array != NULL)free(tmp_quad_array);
#else
 if(node_num_nodes != NULL)free(node_num_nodes);
 if(node_con_nodes != NULL)free(node_con_nodes);
#endif
 ...
 for(i = 0; i < mesh_edges; i++, mesh_edge++){
  for(j = 0; j < 2; j++){
   if(mesh_edge -> fine_id[j] != NULL)free(mesh_edge -> fine_id[j]);
  }
 }

 if(mesh_edge_array != NULL)free(mesh_edge_array);
...
/* form inner tmp fine quads "perpendicular" to bottom face;
  ...
  for(j = 0; j < 3; j++){
   ...
   for(m = 0; m < level + 2; m++)tmp_array[j][pos++] = mesh_face -> fine_id[kk][p++];
   ...
   if(level != 0){
    ...
    for(n = 1; n < level + 1; n++){
     ...
     for(m = 0; m < level; m++)tmp_array[j][pos++] = ++refine_node_id;
     tmp_array[j][pos++] = tmp_array_cen[0][n];
    }
   }

   for(m = 0; m < level + 2; m++)tmp_array[j][pos++] = tmp_array_cen[j + 1][m];
  }
...
/* form inner tmp fine quads "parallel" to bottom face;
  ...
  for(j = 0; j < 3; j++){
   ...
   for(m = 0; m < level + 2; m++)tmp_array[j + 3][pos++] = mesh_face -> fine_id[kk][p++];
   ...
   if(level != 0){
    ...
    for(n = 1; n < level + 1; n++){
     ...
     for(m = 0; m < level; m++)tmp_array[j + 3][pos++] = ++refine_node_id;
     tmp_array[j + 3][pos++] = tmp_array_cen[j + 1][n];
    }
   }

   for(m = 0; m < level + 2; m++)tmp_array[j + 3][pos++] = tmp_array_cen[jj][m];
  }
...
/* note: I ignore that some entries of 3d matrix are rewritten several times;
  ...
  for(j = 0; j < 3; j++){
   ...
   mesh_face = mesh_fc[0];
   for(k = 0; k < 3; k++){
    if(mesh_face -> node[k] == fe_tetra -> node[j])break;
   }
...
/* form top hexa */
  ...
  mesh_face = mesh_fc[1];
  for(k = 0; k < 3; k++){
   if(mesh_face -> node[k] == fe_tetra -> node[3])break;
  }
...
/* form inner bottom tmp fine quads "perpendicular" to bottom face;
  ...
  for(j = 0; j < 4; j++){
   ...
   for(m = 0; m < level + 2; m++)tmp_array[j][pos++] = mesh_quad -> fine_id[kk][p++];
...
/* form top tmp fine quads "perpendicular" to top face;
  ...
  for(j = 0; j < 4; j++){
   ...
   for(m = 0; m < level + 2; m++)tmp_array[j + 4][pos++] = mesh_quad -> fine_id[kk][p++];
   ...
   if(level != 0){
    ...
    for(n = 1; n < level + 1; n++){
     ...
     for(m = 0; m < level; m++)tmp_array[j + 4][pos++] = ++refine_node_id;
     tmp_array[j + 4][pos++] = tmp_array_cen[5][n];
    }
   }

   for(m = 0; m < level + 2; m++)tmp_array[j + 4][pos++] = tmp_array_cen[j + 1][m];
  }
...
/* form inner tmp fine quads "parallel" to bottom and top face;
  ...
  for(j = 0; j < 4; j++){
   ...
   for(m = 0; m < level + 2; m++)tmp_array[j + 8][pos++] = mesh_quad -> fine_id[kk][p++];
   ...
   if(level != 0){
    ...
    for(n = 1; n < level + 1; n++){
     ...
     for(m = 0; m < level; m++)tmp_array[j + 8][pos++] = ++refine_node_id;
     tmp_array[j + 8][pos++] = tmp_array_cen[j + 1][n];
    }
   }

   for(m = 0; m < level + 2; m++)tmp_array[j + 8][pos++] = tmp_array_cen[jj][m];
  }
...
/* note: I ignore that some entries of 3d matrix are rewritten several times;
  ...
  for(j = 0; j < 4; j++){
   ...
   mesh_quad = mesh_qd[0];
   for(k = 0; k < 4; k++){
    if(mesh_quad -> node[k] == fe_hexa -> node[j])break;
   }
...
/* form top fine hexas */
  ...
  for(j = 0; j < 4; j++){
   ...
   mesh_quad = mesh_qd[5];
   for(k = 0; k < 4; k++){
    if(mesh_quad -> node[k] == fe_hexa -> node[j + 4])break;
   }
 ...
 if(fe_node_array != NULL)free(fe_node_array);
 if(fe_edge_array != NULL)free(fe_edge_array);
 if(fe_face_array != NULL)free(fe_face_array);
 if(fe_quad_array != NULL)free(fe_quad_array);
 if(fe_tetra_array != NULL)free(fe_tetra_array);
 if(fe_hexa_array != NULL)free(fe_hexa_array);

 mesh_face = mesh_face_array;
 for(i = 0; i < mesh_faces; i++, mesh_face++){
  for(j = 0; j < 3; j++){
   if(mesh_face -> fine_id[j] != NULL)free(mesh_face -> fine_id[j]);
  }
 }
 ...
 for(i = 0; i < mesh_quads; i++, mesh_quad++){
  for(j = 0; j < 4; j++){
   if(mesh_quad -> fine_id[j] != NULL)free(mesh_quad -> fine_id[j]);
  }
 }

 if(mesh_face_array != NULL)free(mesh_face_array);
 if(mesh_quad_array != NULL)free(mesh_quad_array);
...
*/
 ...
 if(fine_node_array != NULL)free(fine_node_array);
...
/* the following 4 arrays cannot be released earlier because they are used to recover boundary flag */
 ...
 if(node_num_elems != NULL)free(node_num_elems);
 if(node_con_elems != NULL)free(node_con_elems);

 if(tmp_tetra_array != NULL)free(tmp_tetra_array);
 if(tmp_hexa_array != NULL)free(tmp_hexa_array);

 fine_edge = fine_edge_array;
 for(i = 0; i < fine_edges; i++, fine_edge++){
  if(fine_edge -> fine_id != NULL)free(fine_edge -> fine_id);
 }
 fine_quad = fine_quad_array;
 for(i = 0; i < fine_quads; i++, fine_quad++){
  if(fine_quad -> fine_id != NULL)free(fine_quad -> fine_id);
 }
 fine_hexa = fine_hexa_array;
 for(i = 0; i < fine_hexas; i++, fine_hexa++){
  if(fine_hexa -> fine_id != NULL)free(fine_hexa -> fine_id);
 }

 if(fine_edge_array != NULL)free(fine_edge_array);
 if(fine_quad_array != NULL)free(fine_quad_array);
 if(fine_hexa_array != NULL)free(fine_hexa_array);

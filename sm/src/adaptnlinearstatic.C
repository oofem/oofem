...
{
 if (timeStepLoadLevels.isEmpty()) timeStepLoadLevels.resize(this->giveNumberOfSteps());
...
// This function translates this request to numerical method language
{
 ...
 if (d->giveNumber() == 2) {
   ...
   switch (mode)
    ...
    case VM_Incremental:
     ...
     if (d2_incrementOfDisplacement.isNotEmpty()) return d2_incrementOfDisplacement.at(eq); else return 0.;
     
    case VM_Total:
     if (d2_totalDisplacement.isNotEmpty()) return d2_totalDisplacement.at(eq);
     else return 0.;
 ...
 } else return NonLinearStatic::giveUnknownComponent (chc, mode, tStep, d, dof);
...
AdaptiveNonLinearStatic::initializeAdaptiveFrom (EngngModel* sourceProblem)
{
 ...
 if (sourceProblem->giveClassID() != AdaptiveNonLinearStaticClass) _error("sory");
...
/* HUHU CHEATING */
#ifdef __PARALLEL_MODE
	 if(this->giveDomain(2)->giveElement (ielem)-> giveParallelMode() == Element_remote)continue;
#endif
...
/* HUHU CHEATING */
#ifdef __PARALLEL_MODE
	 if(this->giveDomain(1)->giveElement (ielem)-> giveParallelMode() == Element_remote)continue;
#endif
...
AdaptiveNonLinearStatic:: giveTimeStepLoadLevel(int istep)
{
 ...
 } else _error ("solution step out of range");

...
*/
   ...
   for (i=1; i<=dim1; i++)
    for (j=1; j<=dim2; j++) contrib.at(i,j) = -1.0*lcontrib.at(i)*rcontrib.at(j)*coeff;
...
IDNLMaterial::NonlocalMaterialStiffnessInterface_showSparseMtrxStructure(GaussPoint* gp, oofegGraphicContext& gc, TimeStep* atTime)
{
 ...
 for (pos = list->begin(); pos!= list->end(); ++pos) {
  ...
  for (i=1; i<=n;i++) {
   ...
   for (j=1; j<=m; j++) {
    if (rloc.at(j)==0) continue;

    if (gc.getSparseProfileMode() == 0) {
   
     p[0].x = (FPNum) loc.at(i)-0.5; p[0].y = (FPNum) rloc.at(j)-0.5; p[0].z = 0.;
     p[1].x = (FPNum) loc.at(i)+0.5; p[1].y = (FPNum) rloc.at(j)-0.5; p[1].z = 0.;
     p[2].x = (FPNum) loc.at(i)+0.5; p[2].y = (FPNum) rloc.at(j)+0.5; p[2].z = 0.;
     p[3].x = (FPNum) loc.at(i)-0.5; p[3].y = (FPNum) rloc.at(j)+0.5;  p[3].z = 0.;
     go =  CreateQuad3D(p);
     EGWithMaskChangeAttributes(WIDTH_MASK | FILL_MASK | COLOR_MASK | LAYER_MASK, go);
     EMAddGraphicsToModel(ESIModel(), go);
    } else {

     p[0].x = (FPNum) loc.at(i); p[0].y = (FPNum) rloc.at(j); p[0].z = 0.;
...
*/  
  ...
  f = (this->e0/(equivStrain*equivStrain))*exp(-(equivStrain-this->e0)/(this->ef-this->e0))
   + (this->e0/equivStrain)*exp(-(equivStrain-this->e0)/(this->ef-this->e0))*1.0/(this->ef-this->e0);
  ...
  for (i=1; i<= nrows; i++) {
   sum = 0.0;
   for (j=1; j<=nsize; j++) sum+= b.at(j,i)*stress.at(j);
   lcontrib.at(i) = sum*f;
  }
 }
 return 1;
 ...
 if (this->equivStrainType == EST_Rankine) {
  ...
  if (gp->giveMaterialMode() == _1dMat) {
   ...
   if ((gp->giveMaterialMode() == _3dMat) || (gp->giveMaterialMode() == _PlaneStrain)) {
    ...
    double swap;
    for (int i=1; i<=3; i++) {
     if (fabs(principalStress.at(i)) > 1.e-10) zeroFlag = 0;
     if (princDir.at(3,i) > 0.90) indx = i;
    }
    if (indx) {
     for (int i=1; i<=3; i++) {
  ...
  sum = 0.0;
  for (i=1; i<=3; i++) if (principalStress.at(i) > 0.0) sum+= principalStress.at(i)*principalStress.at(i);

  if (sum > 1.e-15) coeff = 1./ (lmat->give ('E')* sqrt (sum));
  else coeff = 0.0;
//
  if (gp->giveMaterialMode() != _1dMat) this->giveStressVectorTranformationMtrx (t, princDir,0);
  
//
  //  if (gp->giveMaterialMode() != _1dMat) this->giveStressVectorTranformationMtrx (t, princDir,1);
  
  // extract positive part
  for (i=1; i<=principalStress.giveSize(); i++) principalStress.at(i) = max (principalStress.at(i), 0.0);

#if 0
  this->giveNormalElasticStiffnessMatrix(den, SecantStiffness, gp, atTime);
  help.beProductOf (den, principalStress);
  fullHelp.resize (6);
  for (i=1; i<=3; i++) fullHelp.at(i) = help.at(i);
...
*/
  ...
  for (i=1; i<=3; i++) fullPrincStress.at(i) = principalStress.at(i);
  ...
  nu = stress;
  coeff = 1.0/(lmat->give ('E') * equivStrain);
 ...
 for (i=1; i<=ncols; i++) {
  sum = 0.0;
  for (j=1; j<=nsize; j++) sum += nu.at(j)*b.at(j, i);

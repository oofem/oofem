...
void AgingIsoLEMaterial::setE(double newE) { E = newE; G = 0.5*E/(1.+nu); }
...
/// Initializes the auxiliary material constants for plasticity according to material options and parameters
{
 ...
 else auxd = -2. * auxkdp * (1.-omega) * 3.*alpha / pow(chi1u,2.);
}
...
HellmichMaterial::giveMaterialGp() {
 if (!materialGp) createMaterialGp();
 return materialGp;
}
...
HellmichMaterial::initializeFrom (InputRecord* ir)
{
 ...
 if (value>=0.) options = options | moHydration;
 else options = options & ~moHydration;
 ...
 } else options = options & ~moPlotStressStrain;
 ...
 if ((intvalue == 0) || ir->hasField(IFT_HellmichMaterial_noshr, "noshr")) { // noshr set
  ...
  if (intvalue & 1) { options = options | moShrinkage; printf ("\nHellMat: Autogenous shrinkage enabled."); }
  else { options = options & ~moShrinkage; printf ("\nHellMat: No autogenous shrinkage."); }
  if (intvalue & 2) { options = options | moHumidityStrain; printf ("\nHellMat: Humidity shrinkage enabled."); }
  else { options = options & ~moHumidityStrain; printf ("\nHellMat: No humidity shrinkage."); }
  if (intvalue & 4) { options = options | moDryingShrinkage; printf ("\nHellMat: Drying shrinkage enabled."); }
  else { options = options & ~moDryingShrinkage; printf ("\nHellMat: No drying shrinkage."); }
 }
 // if autogenous shrinkage is used, check ashr options
 if (options & moShrinkage)
  if (!(options & moHydration)) printf ("\nHellMat: No chemical shrinkage without hydration.");
 ...
 if (options & moDryingShrinkage) {
   ...
   if (ir->hasField(IFT_HellmichMaterial_dryingc, "dc")) printf ("\nHellMat: Drying shrinkage coefficient set to %.3e", c);
 } 
 ...
 if (options & moCreep) {
  ...
  if (intvalue >= 0) { // devc set
   ...
   if (!intvalue) printf (" disabled.");
   else {
    printf(" enabled for");
    if (options & moDeviatoricCreepE) printf (" viscous creep");
    if (options & moDeviatoricCreepF) printf (" flow creep");
   }
  }
 }
...
// Hellmich A.14
{
 if ((options & moLinearEModulus) || (mixture==mtHuber)) return(ae*ksi); // Huber mixture, input forced
 else return(ae*sqrt(ksi)); // Lafarge mixture
...
*/
{
 ...
 if (mmode==_1dMat) {
  if (form==ReducedForm) stress.resize(1);
  else {
   stress.resize(6);
   stress.zero();
  }
  ...
  double kv, gv;
  if (coeff) giveKvCoeffs(gp, atTime, kv, gv, VM_Total);
  else kv=gv = 1.;
  ...
  for (i=0;i<3;i++) stress(i) = evKG + 2*Gv * strain(i);
  for (i=3;i<6;i++) stress(i) = Gv * strain(i);
 }
 else if (strain.containsOnlyZeroes()) stress = strain;
  ...
  } else _error("elasticStiffness: unsupported material mode.");
 }
}
...
/// Returns the isotropic linear elastic strain vector for the given stress vector
{
 ...
 if (ksi<HYDRATION_MINDEGREE) ksi = HYDRATION_MINDEGREE;
 E= agingE(ksi);
 if (!E) _error("Zero stiffness in elastic compliance!");
 if (mmode==_1dMat) {
  if (form==ReducedForm) strain.resize(1);
  else {
   strain.resize(6);
   strain.zero();
  }
  ...
  for (i=0;i<3;i++) strain(i) = svE + stress(i) / (2*Gv);
  for (i=3;i<6;i++) strain(i) = stress(i) / Gv;
 }
 else if (stress.containsOnlyZeroes()) strain = stress;
 else {
  FloatMatrix d;
  LinearElasticMaterial* lMat;
  lMat = giveLinearElasticMaterial(gp, atTime);
...
// 4.36
{
 ...
 if (options & moDryingShrinkage) ca = (gamman + base*base*expterm - c * fabs(dT * log(h)  +  T * dh / h)) / ca;
 else ca = (gamman + base*base*expterm) / ca; 
...
// 3.24
{
 if (ksi<=ksi0) return(0);
 else return( fc*(ksi-ksi0)/(1-ksi0) );
}
...
// full dRcdchi1 = fcStrength(ksi)*(1-omega) * (chi1u-chi1)/(chi1u*sqrt(chi1*(2*chi1u-chi1)))
{
 if (chi1<chi1u)
  if (options&moSqrtHardeningLaw) {
   if (chi1>=0) {
    if (chi1<NEWTON_DERIVATIVEDX*chi1u) chi1 = NEWTON_DERIVATIVEDX*chi1u;
    return( fcStrength(ksi) * (chi1u - chi1)/sqrt(chi1*(2*chi1u-chi1)) );
   } else {
    if (fabs(chi1)<NEWTON_DERIVATIVEDX*chi1u) chi1 = -NEWTON_DERIVATIVEDX*chi1u;
    return( fcStrength(ksi) * (chi1u + chi1)/sqrt(chi1*(-2*chi1u-chi1)) );
   }
  } else // quadratic hardening law - original
...
void HellmichMaterial::deviator(FloatArray& dev, FloatArray& stress)
{
 ...
 for (i=0;i<3;i++) dev(i) = stress(i) - sv;
 for (i=3;i<6;i++) dev(i) = stress(i);
}
...
double HellmichMaterial::deviatorNorm(FloatArray& stress)
{
 ...
 for (i=0;i<3;i++) deviator(i) = stress(i) - sv;
 for (i=3;i<6;i++) deviator(i) = stress(i);

 result = 0;
 for (i=3;i<6;i++) result += pow(deviator(i), 2);
 result*=2.;
 for (i=0;i<3;i++) result += pow(deviator(i), 2);
...
// returns activeSurface and computed flowIncrements
{
 ...
 } else _error("HellmichMaterial::projection: Unknown ActiveSurface value.");
...
// sets (now material!) gp->status->activeSurface, plasticStrain, flowIncrement, activeSurface
{
 ...
 // depsp_ij = dl1 * sigtr_ij/snorm
  ...
  for (i=0;i<6;i++) stress(i) = trialStress(i) - stress(i);

  // plastic status
  if (options & moHardening) status->setTempHardeningVar(auxchi1 + 3*alpha*dl1);
...
//
{
 ...
 HellmichMaterialStatus* status;
#ifdef DEBUG
 if (gp->giveMaterialMode()!=_1dMat) _error("give1dStiffMtrx: wrong stress-strain mode!");
#endif
 ksi = giveHydrationDegree(gp, atTime, VM_Total);
 if (ksi<HYDRATION_MINDEGREE) ksi = HYDRATION_MINDEGREE;
 ...
 } else _error("Unknown active surface.");
 if (form==ReducedForm)
  answer.resize(1,1);
 else { // FullForm
  answer.resize(6,6);
...
#ifdef DEBUG
 if (gp->giveMaterialMode()!=_3dMat) _error("give3dStiffMtrx: wrong stress-strain mode!");
#endif
 ...
 */
 ...
 // --- Drucker-Prager active ---
  ...
  if (options&moHardening) df += auxd * dRcdchi1(chi, ksi);

  deviator(normdev, stress);
  snorm = deviatorNorm(stress);
  for (i=0;i<6;i++) normdev(i) /= snorm;
  ...
  if (as==asDP) {
   for (i=0;i<6;i++)
    for (j=0;j<6;j++) {
     ...
     if (i==j) curent = GGv - GGv2 * dl1/snorm;               // I 
     else curent=0;                                                  // Idev + 1x1
     if ((i<3)&&(j<3)) curent += KGv  +  GGv2/3. * dl1/snorm;  // 1x1
...
// derivative extrapolated from 'small' dx - useless in general case
{
 ...
 if (prec<FINDROOT_SMALLNUM) prec = FINDROOT_SMALLNUM;
 do {
...
//    array sp -> principal strains or stresses
{
 ...
 for (i=0; i<6; i++) if (fabs(s[i]) > 1.e-20) nonzeroFlag = 1;
 if (nonzeroFlag==0) return;
 ...
 */
 ...
 if (i > 2) answer[2]= s3;
#ifdef DEBUG
 else printf("principalStresses: 3 valid roots not found!\n");
#endif
 ...
 for (i=1; i<3; i++)
  for (j=0; j< 2; j++)
   if (answer[j+1] > answer[j])
    {swap=answer[j+1]; answer[j+1]=answer[j]; answer[j]=swap;}
...
#ifdef DEBUG
 if (outputStream == NULL) _error ("plotReturn: can't write into NULL stream");
#endif
...
// stress-strain graph output
{
 ...
 ksi = giveHydrationDegree(gp, atTime, VM_Total);
 status = (HellmichMaterialStatus*)giveStatus(gp);
 if (status->giveTempStressVector().giveSize()) stress = status->giveTempStressVector()(idx);
 else stress = 0;
 if (status->giveTempStrainVector().giveSize()) strain = status->giveTempStrainVector()(idx);
 else strain = 0;
 if (((options&moHasShrinkage)==moHasShrinkage) || (options & moHumidityStrain)) {
  ...
  if (auxVector.giveSize()) eshr = auxVector(idx);
  else eshr = 0;
 } else eshr = 0;
 if (options & moPlasticity) {
  status->giveTempPlasticStrainVector(auxVector);
  if (auxVector.giveSize()>idx) epl = auxVector(idx);
  chi = status->giveTempHardeningVar()/chi1u;
 }
 if (options & moCreep) {
  status->giveTempViscousStrainVector(auxVector);
  if (auxVector.giveSize()>idx) evisc = auxVector(idx);
  status->giveTempFlowStrainVector(auxVector);
  if (auxVector.giveSize()>idx) eflow = auxVector(idx);
 }
...
{
 if (!hemoMaterial || !giveDomain()->giveMaterial(hemoMaterial)) _error("giveHeMoMaterial: undefined reference to heat and moisture transport material.");
...
//    sets Gamma0 if ksi>=ksi0;
{
 ...
 HellmichMaterialStatus* status = (HellmichMaterialStatus*)giveStatus(gp);
 if (status->giveUpdateFlag()) return;
 else status->setUpdateFlag(1);
 ...
 if (options & moIsothermal) { // === Isothermal ===
  ...
  if (giveTTimeFunction()) T = ((LoadTimeFunction*)giveTTimeFunction())->evaluate(atTime, VM_Total);
  else T = giveTemperature(gp);
  if (givehTimeFunction()) h = ((LoadTimeFunction*)givehTimeFunction())->evaluate(atTime, VM_Total);
  else h = giveHumidity(gp, VM_Total);
 }
 else { // === Non-isothermal ===
  ...
  if ((tf = fm->giveField (FT_Temperature))) {
   ...
   if (flatTemperature) {
    if (flatTemperature == -1) { // temperature xy -> xz (2D cross-section for 3D girder)
     ...
     if (flatTemperature & 1) gcoords.at(1) = 0.;
     if ((gcoords.giveSize()>1) && (flatTemperature & 2)) gcoords.at(2) = 0.;
     if ((gcoords.giveSize()>2) && (flatTemperature & 4)) gcoords.at(3) = 0.;
    }
   }
   tf->evaluateAt (et, gcoords, dofid, VM_Total, atTime);
   T = et.at(1) + temperatureFieldBase;
  } else { // no temperature field
   if (initialTemperature>0) T = initialTemperature;
   else _error("InitAuxStatus: Temperature field not registered, initial temperature not set. Use isoT or iniT setting.");
#ifdef VERBOSE_HELLMAT
...
*/
{
 ...
 if (options & moHydration) {
  if (options & moIsothermal) gp = giveMaterialGp();
  return hydrationModel->giveHydrationDegree(gp, atTime, mode);
 }
 else return (mode==VM_Total)?constantHydrationDegree:0.;
}
...
// returns the microprestress force Gamma
{
 if (options & moCreep) {
  ...
  double g0;
  g0 = status->giveGamma0();
  if (g0<=0) return(0.); // gamma0 not set yet
  else return ( g0 - modulusH * status->giveTempViscousSlip() );
 }
 else return 0.; // no creep
}
...
// 4.34
{
 if (options & moIsothermal) gp = giveMaterialGp();
 HellmichMaterialStatus* status = (HellmichMaterialStatus*) giveStatus(gp);
 if (status->giveGamma0()<=0.) return(0.); // gamma0 not set yet
 else return( status->giveViscosity());
}
...

{
 ...
 if (options & moCreep) {
  ...
  if (dt>0) {
   ...
   if (((HellmichMaterialStatus*)giveStatus(gp))->giveGamma0()>0) fv = dt*agingE(ksi)*giveViscosity(gp);
   else fv = 0.;

   if (mode == VM_Total) {
    gv = 1. + ev + fv; // everything is always included in deviatoric coefficient
    kv = 1.;
    if (! (options & moDeviatoricCreepE)) kv += ev; // add visc creep to volumetric coeff
    if (! (options & moDeviatoricCreepF)) kv += fv; // add flow creep to volumetric coeff
   } else {
    kv = ev;
    gv = fv;
   }
 ...
 if (((options&moHasShrinkage)==moHasShrinkage) || (options & moHumidityStrain)) {
  ...
  if ((options&moHasShrinkage)==moHasShrinkage) {
   ...
   if (mode == VM_Incremental) {
    if (giveTimeIncrement(atTime)>0) eshr -= autoShrinkageCoeff(ksi-giveHydrationDegree(gp, atTime, VM_Incremental));
    else eshr = 0;
   }
  }
  ...
  if (options & moHumidityStrain) {
   ...
   if (mode == VM_Incremental) eshrh -= dryingShrinkageCoeff(giveHumidity(gp, VM_Velocity)); // Previous
   if (eshrh) eshr+= eshrh;
  }

  for (i=0;i<3;i++) answer(i) = eshr;
  for (i=3;i<size;i++) answer(i) = 0;
                                ...
                                GaussPoint* gp, TimeStep* atTime, ValueModeType mode)
{
 ...
 if (prestress) {
  ...
  if (mode == VM_Incremental) {
   if (giveTimeIncrement(atTime)>0) prestr -= prestressValue(time - giveTimeIncrement(atTime));
   else prestr = 0;
  }
  if (prestr) {
   ksi = giveHydrationDegree(gp, atTime, VM_Total);
   if (ksi < HYDRATION_MINDEGREE) _error ("Can't apply prestress: zero hydration degree!");
   answer.resize(6); answer.zero();
...
// + creep strains: (Kv-1)C:sig_n - 1/(1+tau/dt)*eps_v,n
{
 ...
 if (prestress) {
  ...
  if (auxvec.giveSize()) {
   if (!answer.giveSize()) { answer.resize(6); answer.zero(); }
   answer.add(auxvec);
  }
 }
 ...
 if ((options&moCreep)&&(dt>0)) {
  ...
  if (options & moDeviatoricCreepE) deviator (redvec, stress);
  else redvec = stress;
  redvec.times(ev);   // viscous creep
  // compute auxvec tensor for flow creep
  if (options & moDeviatoricCreepF) deviator (auxvec, stress);
  else auxvec = stress;
...
// ------ ( Total Mode ) -------
  ...
  if (options&moCreep) _error("Can't use VM_Total with creep.");
#ifdef DEBUG
...
// ------ ( Total Konec ) ------
 else _error("giveRealStressVector: Unknown ValueModeType!");

 // save reduced Trial Stress vector
 if (options & moPlasticity) status->setTrialStressVector(trialStressVector);
...
// ======  1D  ======
 ...
 if (mmode == _1dMat) {
  ...
  if ((options & moPlasticity) && (auxksi > ksi0)) {
   ...
   // save Plastic status
   if (options & moHardening) status->setTempHardeningVar(auxchi1 + 3*alpha*dl);
   status->givePlasticStrainVector(auxStrain);
   auxStrain(0)+= depsp;
...
// maybe should work 3D projection + stiffness matrix inversion->reduction?
  ...
  } else redStressVector = trialStressVector;
...
// ====== 2D Konec ======
  ...
  if ((options&moPlasticity) && (auxksi > ksi0)) {
   ...
   } else _error("giveRealStress: unsupported material mode");
  } else { // elastic
 ...
 if ((options&moCreep)&&(dt>0)) {                             // moDeviatoricCreepE
  ...
  if (options & moDeviatoricCreepE)  deviator (auxStress, redStressVector);
  else auxStress = redStressVector;
  ...
  // compute auxvec tensor for flow creep
  if (options & moDeviatoricCreepF)  deviator (auxStress, redStressVector);
  else auxStress = redStressVector;
  elasticCompliance(elasticStrain, auxStress, gp, atTime, ReducedForm, false); // using no Kv, Gv!
 ...
 if (form == FullForm) answer = fullStressVector;
 else answer = redStressVector;
                             ...
                             GaussPoint *gp, TimeStep *atTime, ValueModeType mode)
{
 ...
 if (et.giveSize()) if (fullAnswer.giveSize()) fullAnswer.add(et); else fullAnswer = et;
...
//
{
 if (options & (moDeviatoricCreepE | moDeviatoricCreepF)) // deviatoric creep supported only in 3D analysis
  ...
  if ((mode == _3dMat)||(mode == _1dMat)) return 1;
  else return 0;
 else return(linMat->hasMaterialModeCapability(mode));
/* LinearElastic:
...
//  _warning("HellmichMaterial::giveCharMtrx: No stiffness - zero hydration degree.",10);
 ...
 if (mMode == _1dMat) give1dMaterialStiffnessMatrix(answer, form, rMode, gp, atTime);
 else if (mMode == _3dMat) give3dMaterialStiffnessMatrix(answer, form, rMode, gp, atTime);
 ...
 else _error("giveCharMtrx: unsupported stress-strain mode!");

#ifdef VERBOSE_TANGENT
 if (options & moPlotStressStrainIter) {
  HellmichMaterialStatus* status = (HellmichMaterialStatus*)giveStatus(gp);
  if (mMode == _1dMat) printf("Time: %.2f, sig: %.10e, Eep: %.10e\n", giveTime(atTime),
...
void HellmichMaterial::setMixture(MixtureType mix)
{
 ...
 if (hydrationModel) hydrationModel->setMixture(mix);
}

LinearElasticMaterial*
HellmichMaterial::giveLinearElasticMaterial(GaussPoint* gp, TimeStep* atTime)
{
 if (!linMat) _error("giveLinearElasticMaterial: base linear elastic material is not initialized.");
...
// returns the equilibrated IP value. should be called after step update or context restore - temp=equlib
{
 ...
 if (type == IST_PlasticStrainTensor) {
  if (options & moPlasticity) status->givePlasticStrainVector(answer);
  else {
...
*/
{
 if ((aProperty == 'G') || (aProperty == Gyz) || (aProperty == Gxz) ||
  (aProperty == Gxy)) return agingG(1.0);
 if ((aProperty == 'E') || (aProperty == Ex) || (aProperty == Ey) ||
  (aProperty == Ez)) return agingE(1.0);
 if ((aProperty == NYxy) || (aProperty == NYxz) || (aProperty == NYyz)) return ny;
 if ((aProperty == 'n') || (aProperty == NYzx) || (aProperty == NYzy) ||
     (aProperty == NYyx)) return ny;
 else return this -> Material :: give(aProperty);
}
...
HellmichMaterial::giveIPValueType (InternalStateType type)
{
 ...
 if ((type == IST_PlasticStrainTensor) || (type == IST_PrincipalPlasticStrainTensor)) return ISVT_TENSOR_S3E;
 else if (type == IST_DamageTensor) return ISVT_TENSOR_G;
 else if (type == IST_HydrationDegree) return ISVT_SCALAR;
 else if (type == IST_Temperature) return ISVT_SCALAR;
...
HellmichMaterial::giveIntVarCompFullIndx (IntArray& answer, InternalStateType type, MaterialMode mmode)
{
 if (type == IST_PlasticStrainTensor) {
  ...
  answer.resize(6);
  answer.at(1) = 1; answer.at(2) = 2; answer.at(3) = 3;
  return 1;
...
HellmichMaterialStatus::HellmichMaterialStatus(int n, Domain* d, GaussPoint* g) : StructuralMaterialStatus(n,d,g), HydrationModelStatusInterface()
{
 ...
 else nonisoData = NULL;

 // Plasticity
 if (options & moPlasticity) plastData = new PlastData();
 else plastData = NULL;

 // Creep
 if (options & moCreep) creepData = new CreepData();
 else creepData = NULL;
}
...
{
 if (gp->giveNumber()) return ((HellmichMaterial*)gp->giveMaterial())->giveOptions();
 else return moMaterialLevel;
}
...
{
 if (type == HydrationModelStatusInterfaceType) return (HydrationModelStatusInterface*) this;
 else return NULL;
}
...
*/
{
 ...
 if (plastData) plastData->initTempStatus(gp);
 if (creepData) creepData->initTempStatus(gp);
...
/// Update after equilibrium has been reached: temp -> equilib.
{
 ...
 if (nonisoData) nonisoData->updateYourself();
 if (plastData) plastData->updateYourself();
 if (creepData) creepData->updateYourself();
}
...
// OOFEM output routine
{
 ...
 if (options & moIsothermal) mat->printOutputAt(stream, atTime);
 ...
 if (nonisoData) {
  ...
  if (((options & moHasShrinkage) == moHasShrinkage) || (options & moHumidityStrain)) {
   ...
   if (helpVec.isEmpty()) { helpVec.resize(1); helpVec.zero(); }
 ...
 if (plastData) {
  ...
  if (!as) fprintf(stream," Elastic");
  else {
   fprintf(stream," Plast");
   if (as==asDP) fprintf(stream,"DP");
   else if (as==asTC) fprintf(stream,"TC");
   else if (as==asCorner) fprintf(stream,"CO");
   ...
   for (i=0; i<n; i++) fprintf(stream," %.4e", fullHelpVec(i));
  }
 }

 if (creepData) {
  fprintf(stream,"   Creep");
  if (nonisoData) fprintf(stream," slip %.5e", giveViscousSlip());
  fprintf(stream," eps_visc");
  cs->giveFullCharacteristicVector(helpVec, gp, creepData->viscousStrainVector);
  n = helpVec.giveSize();
  for (i=0; i<n; i++) fprintf(stream," %.4e", helpVec(i));

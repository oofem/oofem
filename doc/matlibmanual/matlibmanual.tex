fc0documentclass[a4paper]{article}
\usepackage{html}
\usepackage{graphicx}
%\usepackage{hyperref}
...
&- \param{rateflag} flag which allows for the consideration of the effect of strain rates (using a modified CEB-FIP Model code model) if set to 1, 2 or 3. Default value 0 which corresponds to no strain rate effect.\\
\begin{itemize}\setlength{\itemsep}{-3pt}
\item[1 -] CEB-Model code 2010 rate factor for strength and square rate factor for fracture energy  
\item[2 -] modified CEB-Model code 2010 rate factor for strength and fracture energy
\item[3 -] modified CEB-Model code 2010 rate factor for strength and no rate factor for fracture energy
\end{itemize}\\
...
\subsubsection{Latticedamage}
This is a damage lattice material used together with lattice elements.
...
\end{equation}
where $\boldsymbol{\sigma}$ is a vector of tractions and $\boldsymbol{\varepsilon}$ is a vector of strains obtained from displacement jumps smeared over the element length.
...
Supported modes& 1dlattice, 2dlattice, 3dlattice\\
\hline
\end{mmt}
...
\subsubsection{Latticslip}
This is a elasto-plastic model for slip between lattice elements used together with lattice link elements.

The theory of the model is described in the paper ``P. Grassl and A. Antonelli. "3D network modelling of fracture processes in fibre-reinforced geomaterials", International Journal of Solids and Structures, vol. 156-157, Pages 234-242, 2019.``

...
Linear isotropic material model for moisture transport problems. The model parameters are summarized
in table~\ref{IsoLinmoistureMat_table}.
...
in reference \cite{Bazant:72}. The governing equation was derived
assuming that the slope of the sorption isotherm is linear, i.e. the
moisture capacity is constant. This equation reads
\begin{equation}\label{BNmodel:governing}
\frac{\partial h}{\partial t} = \nabla \cdot \left( C(h) \nabla h \right)
\end{equation}
where $h$ is the relative humidity [-] and $C(h)$ is highly nonlinear
humidity-dependent diffusivity approximated by
\begin{equation}\label{BNmodel:diffusivity}
C (\varphi) = C_1 \left( \alpha_0
+ \frac{1-\alpha_0}{1+\left(\frac{1-h}{1-h_c}\right)^n} \right)
\end{equation}

The model parameters are summarized in table~\ref{BazantNajjarMoistureMat}.
\begin{table}[!htb]
%\begin{center}
\begin{tabular}{|l|p{9cm}|}
\hline

...
In a recent paper \cite{Gra13}, special techniques that modify the averaging procedure based on the distance from a physical boundary of the domain or on the stress state have been considered. The details are explained in \cite{Gra13}. These techniques can be invoked by setting the optional parameter {\it nlVariation} to 1, 2 or 3 and specifying additional parameters $\beta$ and $\zeta$ for distance-based averaging, or $\beta$ for stress-based averaging.  

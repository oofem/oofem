...
\end{equation}
where $h$ is the relative humidity, $c$ is the moisture capacity,
and $k$ is the moisture permeability.\footnote{The notation in
\refeq{lindiff} is not standard. For instance, 
Ba\v{z}ant uses $c$ for the moisture permeability
and $k$ for the inverse moisture capacity. Here, the objective is to 
avoid confusion between parameters in \refeq{lindiff0} and \refeq{lindiff}.}
The model parameters are summarized
...
Nonlinear isotropic material for moisture transport in cementitious
materials. A detailed description of this material model can be found
in reference \cite{Bazant:72}. The governing equation 
\begin{equation}\label{BNmodel:governing}
\frac{\partial h}{\partial t} = \nabla \cdot \left( C(h) \nabla h \right)
\end{equation} 
was derived
assuming that the slope of the sorption isotherm is linear, i.e.\ the
...
the sorption isotherm with respect to relative humidity),  $c(h)$
[kg/m$\cdot$s] is a moisture permeability, and $h$ is a relative
humidity. 

So far, six different functions for the sorption
isotherm (or moisture capacity) have been implemented:
\begin{enumerate}

\item
Linear isotherm $isothermType=0$ is characterized only by its
slope given by parameter $moistureCapacity$.

\item
Piecewise isotherm $isothermType=1$ is defined by two arrays
with the coordinates of the relative humidity $iso\_h$ and the
corresponding moisture content $iso\_w(h)$. The arrays must be of
the same size. 

\item
Ricken isotherm (reference appears in Kuenzel \cite{Kuenzel})
...
an approximation coefficient. (In the input record only $d$ is
necessary. Note that for $h=1$ this isotherm
gives infinite moisture content. 

\item
Isotherm proposed by Kuenzel \cite{Kuenzel} $isothermType=3$ of the form 
...
$b$ [-] is an approximation factor greater than 1.

\item
Isotherm proposed by Hansen \cite{Hansen} $isothermType=4$
\begin{equation}\label{nlisomoisture:hansen}
u(h) = u_h \left(1- \frac{\ln h}{A} \right)^{-1/n}
\end{equation} 
expresses adsorbed water by means of moisture ratio $u$ [kg/kg]. To obtain the
...
by the density of the solid phase. In (\ref{nlisomoisture:hansen})
 $u_h$ is maximum hygroscopically
bound water by adsorption, and $A$ and $n$ are constants
obtained from fitting experimental data.

\item
Isotherm ``BSB'' \cite{BSB} $isothermType=5$ is the improved
version of the famous BET isotherm. The isotherm is expressed by means
of the moisture ratio $u$:
\begin{equation}\label{nlisomoisture:BSB}
u(h) = \frac{C k V_m h}{(1-k h)(1+(C-1)k h)}
\end{equation} 
where $V_m$ is the monolayer capacity, $C$ depends on the absolute
temperature $T$ and the difference between the heat of adsorption and
...
only for the cement paste; reduction of moisture ratio is necessary if the isotherm
should be applied for concrete. 
\end{enumerate}

The present implementation offers three functions for moisture permeability:
\begin{enumerate}

\item
Piecewise linear permeability  $permeabilityType=0$ is defined by two arrays
with the coordinates of the relative humidity $perm\_h$ and the
corresponding moisture content $perm\_c(h)$. The arrays must be of
the same size. 

\item
Permeability function ($permeabilityType=1$) with the same shape as in 
Section \ref{BazantNajjarMoistureMat}. Here, parameter $c1$ is the
moisture permeability at full saturation [kg/ms].

\item
Permeability function proposed by Xi et. al. \cite{Xi}
($permeabilityType=2$) reads
\begin{equation}\label{nlisomoisture:Xi}
c(h) = \alpha_h + \beta_h \left[ 1 - 2^{-10^{\gamma_h(h-1)}} \right]
\end{equation} 
where $\alpha_h$, $\beta_h$ and $\gamma_h$ are parameters, which can be
evaluated using empirical mixture-based formulae presented in the same
reference. However, for certain water-to-cement ratios the
permeability becomes negative. Also the physical units are unclear.
\end{enumerate}

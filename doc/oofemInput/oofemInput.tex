...
by a conversion program or by a FEM pre-processor.
...
\Pmode{When invoking the solver in parallel mode, only base name of input file should be specified, 
...
\textbf{-l int}\\
Sets treshold for log messages (Errors=0, Warnings=1,
...
The order of records in file is compulsory, and is following
\begin{enumerate}
...
The individual records consist of record keyword followed by one or more attributes, where each attribute is identified by
its keyword, which is followed by attribute value(s). Some attributes have no values.
The order of attributes is optional.
...
The type of attribute values is attribute specific. To describe the type of attribute, the following notation is used: 
\field{AttributeKeyword}{type}, where type determines the attribute type and \# is the placeholder for attribute value.
Sometimes, record keyword can be variable, depending on entity type which is
described by particular record. For example, we have element records, where attribute keywords determine particular element types. In this case, the keyword is preceded by star. We call such keyword as entity keyword. The possible
substitutions for entity keyword are typed using \entKeywordInst{Typewriter} font family.

Often, many attributes are specific to particular entity keyword. Then we describe
general format of record without these specific attributes and we describe
entity specific attributes separately.
...
As already mentioned, each attribute value has type. The possible types are:
\begin{itemize}
...
should equal to number of true regions and values should be in a range $\langle 1, nvr \rangle$. When \param{nvr} is zero or negative, this parameter is ignored. The i-th value defines mapping of true region number to virtual region number. Zero or negative values cause a region to be ignored. For example, to consider only elements associated with a cross-section 1 and disregarding other cross sections, use the line with \param{nvr} and \param{vrmap} as

...
The array \param{masters} contains nodes, which create a virtual element-like interpolation. The sequence of
nodes must correspond to element numbering, e.g. anti-clockwise numbering for a linear quadrilateral.
Under normal circumstances, the nodes in the array are the same as a nodal array for some element. However, the array
may have nodes combined from more than one element.

Values of array \param{dofType} can have following values: {0-primary DOF, 2-linked DOF}.
...
If parameter \param{type} is positive, then the value determines the type of interpolation used to
...
defined as a local coordinate that yield node global coordinates for
selected interpolation type (isoparametric interpolation with the same order
for geometry and unknowns is assumed).

Negative value of \param{type} signalizes that the interpolation uses directly
\param{masters} array and calculates natural coordinates from given coordinates
of the hanging node. The negative number is calculated in the same way as a positive one.
For example -413 refers to a linear tetrahedron.

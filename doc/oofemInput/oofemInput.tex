...
\hline
\textbf{-v} & Prints oofem version.\\
\textbf{-f~string} & path to oofem input file name, if not present, program interactively
...
\textbf{-r~int} & Restarts the analysis from given solution step. The corresponding
context file (*.osf) must exist.\\
\textbf{-rn} & Turns on the equation renumbering. Default is off.\\
\textbf{-ar~int} & 
...
\textbf{-l~int} & Sets treshold for log messages (Errors=0, Warnings=1,
Relevant=2, Info=3, Debug=4).\\
\textbf{-qo~string} & Redirect the standard output stream (stdout) to given file.\\
\textbf{-qe~string} & Redirect standard error stream (stderr) to given file.\\
\textbf{-c} & Forces the creation of context file for each solution step.\\
\hline
\end{tabularx}\\[1em]

...
& \param{smtype} & \tiny{Direct (0)} &\tiny{IML (1)}
 &\tiny{Spooles (2)}& \tiny{Petsc (3)}& \tiny{DSS (4)}& \tiny{MKLPardiso (6)}& \tiny{SuperLU\_MT (7)}\\
\hline
...
\item \param{loadType} = 7 specifies radiative boundary condition. It requires to specify emmisivity $\varepsilon\in\langle 0,1\rangle$ and the \param{components} array contains the environmental values (temperature of the environment).
\end{itemize}

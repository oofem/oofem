...
\item ``standard-syntax''\\
  \begin{record}[0.9\textwidth]
    ...
    \recentry{}{\field{attributes}{string}} \recentry{}{\optField{nmodules}{in}} \recentry{}{\optField{nxfemman}{in}}
  \end{record}
\item ``meta step-syntax''\\
  \begin{record}[0.9\textwidth]
    \recentry{\entKeyword{AnalysisType}}{\field{nmsteps}{in}}
    \recentry{}{\optField{nmodules}{in}} \recentry{}{\optField{nxfemman}{in}}
  \end{record}
...
The direct solver corresponds to \param{controllmode}=1 and Newton-Raphson solver is used.
...
allowed.
\item \param{reqIterations} - number of required iterations during the iterative solution.
...
\item \param{rtolv} - determines relative convergence norm (both for displacement
iterative change vector and for residual unbalanced force vector). If the defailt convergence criteria is used,
the \param{rtolv} is real value. If the convergence criteria DOF groups are used (see bellow the description of \param{nccdg} parameter) then \param{rtolv} is an real valued array of \param{nccdg} size, and its values define relative convergence norm for individual criteria.
...
\begin{record}
  ...
  \recentry{}{\optField{activityltf}{in}}

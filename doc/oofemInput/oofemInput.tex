...
\begin{tabular}{|l|l|}                                                      
\hline                                                                          
Solver type & Solver parameters \\
\hline
ST\_Direct & \\
ST\_CG + ST\_GMRES & \field{lstol}{rn} \field{lsiter}{in}
\field{lsprecond}{in}\\
                   &  \optField{precondattributes}{string}\\
ST\_Spooles & \optField{msglvl}{in} \optField{msgfile}{s}\\
ST\_Petsc & see Petsc manual, for details\footnotemark\\
ST\_DSS & \\
\hline
\end{tabular}                                                                   
...
fill-up\\
\hline
IML\_ICPrec   &3& SMT\_SymCompCol&Incoplete Cholesky\\
...
\field{DofIDMask}{ia}]
\optField{bc}{ia}
\optField{ic}{ia} \optField{masterMask}{ia}\\ \PfieldVal{globnum}{in}
...
Parameter \param{masterMask} allows to connect some dof manager's dofs
(so-called ``slave'' dof)
...
In \param{masterMask} array are numbers of master dof managers to
which particular receiver dofs are connected (linked). If master mask array value
is zero, then corresponding dof is assumed to be master (having own
equation number), otherwise slave dof, linked to corresponding master
dof manager dof is created. By default, if \param{masterMask}
parameter is omitted, all dofs are assumed to be masters.
{\bf Note: the meaning of \param{masterMask} parameter is different for \entKeywordInst{RigidArmNode}}
...
possible to map only certain dofs - see \param{masterMask}.} The local coordinate
system in rigid arm node is not supported in current implementation, the global
lcs applies. On the other hand, rigid arm node can be loaded
...
{\em The same number of dofs and same physical dof meaning for rigid arm
node and related master are required. The current implementation
precludes chaining rigid arm nodes.}
The optional parameter \param{masterMask} allows to map by rigid arm
transformation only certain dofs. The size of \param{masterMask} array 
should be equal to number of DOFs. The zero value indicates that
corresponding DOF is a independent DOF (independent of master, with own
equilibrium equation), nonzero value indicates, that corresponding DOF
is mapped on master.
\item
\entKeywordInst{HangingNode}  \field{coords}{ra} \field{type}{in} \field{masters}{ia}
\field{ksi}{rn}\\ \optField{eta}{rn} \optField{dzeta}{rn}.\\
Hanging node connected to other nodes (masters) using interpolation.
Hanging node posses no degrees of freedom	- all values are
...
Parameter \param{type} determines the type of interpolation used to
evalueate receiver unknowns and position. The required value can be
evaluated as 100*(number of master nodes) + 10*(order of polynomial
...
Represents user defined time function. The function must be expressed
using t parameter, for which actual time will be substituted and
expression evaluated. Very general, but relatively slow.
\end{itemize}

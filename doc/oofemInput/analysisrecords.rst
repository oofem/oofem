...
[``minvof #(rn)``] [``skipsm #(in)``]
[``minsteplength #(rn)``] [``maxsteplength #(rn)``] [``reqIterations #(in)``]
[``stepMultiplier #(rn)``]
...
deposition profile defined by the G-code file. 

Initially, the RVOM of all elements are set to zero, making all elements empty without
any deposited material. The elements with zero RVOM are defined as inactive. In
consecutive time steps, individual elements and corresponding nodes are activated when
...
zero RVOM values, exceeding the treshold. Additionally, empty elements inside regions fully bounded by
elements with nonzero RVOM values are added in each layer. This ensured that elements representing
internal voids (air-filled) are also a part of the computational domain. This naturally
allows us to consider, for example, the heat transfer inside a specimen. Active nodes are subsequently
defined as nodes that are shared by at least one active element.

At present, the additive manufacturing problem supports only two sub-problems, with first one being the transient thermal analysis otionally followed by mechanical analysis.

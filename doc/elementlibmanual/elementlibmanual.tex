...
integration points used can be overloaded using \param{NIP} parameter.
...
\descitem{Parameters} \optField{NIP}{in} \optField{NIPRot}{in}\\
\descitem{Unknowns}
...
Integration of all terms using one point formula. \\
\descitem{Features} Layered cross section support. \\
\descitem{Loads} Body loads are supported. Boundary loads are not supported.\\
...
\elemkeyword{quad1mindlin}\\
\descitem{Parameters} \optField{NIP}{in} \\
\descitem{Unknowns} Three dofs (w-displacement, u and v - rotation) are required in each node. \\
\descitem{Integration} Default uses 4 integration points. No reduced integration is used, as it causes numerical problems. \\
\descitem{Features} Layered cross section support. \\
\descitem{Loads} Dead weight loads, and edge loads are supported. \\
\descitem{Status} Experimental. \\
\descitem{Reference} \cite{RobertCook1989}

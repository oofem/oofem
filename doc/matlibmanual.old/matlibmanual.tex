...
\subsubsection{Damage-plastic model for concrete - CDPM2} \label{sec:cdpm2}
...
\end{equation}
Taking into account that $\partial\bar{\sigma}_{\rm V}/\partial\bar{\boldsymbol{\sigma}}=\boldsymbol{\delta}/3$ and $\partial\bar{\rho}/\partial\bar{\boldsymbol{\sigma}}=\bar{\mbf{s}}/\bar\rho$, restricting attention to the onset of the post-peak regime (in which $q_{\rm h1} = q_{\rm h2} = 1$) and differentiating the plastic potential (\ref{eq:plasticPotential}), we rewrite equation (\ref{eq:generalFlow}) as
...
\subsubsection{Damage-plastic model for fibre reinforced concrete - CDPM2F}
CDPM2F is a fibre extension of CDPM2 in \ref{sec:cdpm2}. The theory of CDPM2F is described in \cite{ZhoMarGra24}.
The main idea of CDPM2F is to adjust the damage part of CDPM2 to predict the effect of fibres.
Below in Table~\ref{cdpm2f_table} the input parameters of CDPM2F in addition to those required for CDPM2 are described. 
...
Description & CDPM2F\\
\hline
Record Format & \descitem{cdpm2f} \elemparam{lf}{rn} \elemparam{vf0}{rn} \elemparam{df}{rn} \elemparam{tau0}{rn} \elemparam{beta}{rn} [\elemparam{f}{rn}] \elemparam{ef}{rn} \elemparam{sm}{rn} [\elemparam{alpha}{rn}] [\elemparam{xi}{rn}]\\
...
\subsubsection{LinkSlip - Slip model for trusses and beams}
This is an elasto-plastic model for slip between reinforcement modelled as three-dimensional beam or truss elements and matrix elements. The theory of the model is described in the paper \cite{SciGraLarRun20} for the case of beams. The model parameters are summarised in Tab.~\ref{linkslip_table}.
There are three types of constitutive relations supported which are shown in Figure~\ref{linkslip}, which are chosen by means of the input parameter \param{type}.

\begin{figure}[!htb]
\begin{htmlonly}
  \centerline{\includegraphics[width=1.\textwidth]{linkslip.eps}}
\end{htmlonly}
%begin{latexonly}
 \centerline{\includegraphics[width=0.8\textwidth]{linkslip}}
%end{latexonly}
  \caption{Bond stress ($\tau$) - slip ($s$) diagram for link slip model.}
  \label{linkslip}
\end{figure}
...
\hline
Record Format & \descitem{linkslip} \elemparam{}{in} 
\elemparam{kn}{rn}  \elemparam{kl}{rn} \optelemparam{type}{in} \elemparam{s1}{rn} \elemparam{s2}{rn} \elemparam{s3}{rn} \elemparam{t0}{rn} \optelemparam{tf}{rn}  \optelemparam{alpha}{rn}\\
Parameters &- \param{} material number\\
...
\hline
\end{mmt}
\caption{Slip model for truss and beam elements -- summary.}
\label{linkslip_table}
%\end{center}
\end{table}
...
\section{Material Models for Lattice Analysis}
The lattice material models are part of the lattice module; the solver must be compiled with \texttt{USE\_LM}. The module gathers the material models for lattice analysis.

\subsection{Material models for lattice structural elements}
...
\subsubsection{Elastic material model for lattice based frame elements}
This is an elastic material model used together with 3d lattice frame elements. It has been specifically developed to describe the failure of steel frame member using the lattice elements lattice3d and lattice3dnl. However, it can also been used with any other 3d lattice element.
The stress-strain law has the form
\begin{equation}
  \mathbf{s} = \mathbf{D}_{\rm e} \mathbf{e}
\end{equation}
where $\mathbf{e}$ is a vector of internal forces and moments, $\mathbf{D}_{\rm e}$ is the elastic stiffness matrix, $\mathbf{e}$ is a vector of generalised strains obtained from displacement jumps smeared over the element length and rotational components. The model parameters are summarised in Table \ref{latticeframesteelplastic_table}.
...
\hline
Record Format & \descitem{latticeframeelastic} \elemparam{}{in}
\elemparam{d}{rn} \optelemparam{talpha}{rn} \elemparam{e}{rn} \elemparam{n}{rn}\\
Parameters &- \param{} material number\\
&- \param{d} material density\\
&- \optparam{talpha} Thermal expansion coefficient. Default is 0.\\
...
\hline
\end{mmt}
\caption{Elastic model for 3D lattice based frame elements -- summary.}
\label{latticeframeelastic_table}
%\end{center}                  
  \end{table}
The linear-elastic response of the frame elements is given directly as an relationship between generalised stress (internal forces) and generalised strain. 
%


\subsubsection{Steel Plasticity material model for lattice based frame elements}
This is a plasticity material used together with 3d lattice frame elements. It has been specifically developed to describe the failure process failure of steel frame member using the lattice elements lattice3d and lattice3dnl. 
The generalised stress-strain law has the form
\begin{equation}
  \mathbf{s} = \mathbf{D}_{\rm e} \left(\mathbf{e}-\mathbf{e}_{\rm p} \right)
\end{equation}
where $\mathbf{e}$ is a vector of internal forces and moments, $\mathbf{D}_{\rm e}$ is the elastic stiffness matrix, $\mathbf{e}$ is a vector of generalised strains obtained from displacement jumps smeared over the element length and rotational components and $\mathbf{e}_{\rm p}$ are the plastic strains. The model parameters are summarised in Table \ref{latticeframesteelplastic_table}.
...
Description & Plasticity lattice model for steel \\
\hline
Record Format & \descitem{latticeframesteelplastic} \elemparam{}{in}
\elemparam{d}{rn} \optelemparam{talpha}{rn} \elemparam{e}{rn} \elemparam{n}{rn} \elemparam{nx0}{rn} \elemparam{mx0}{rn} \elemparam{my0}{rn} \elemparam{mz0}{rn} \elemparam{sub}{in} \elemparam{iter}{in} \elemparam{tol}{rn}\\
Parameters &- \param{} material number\\
&- \param{d} material density\\
&- \optparam{talpha} Thermal expansion coefficient. Default is 0.\\
&- \param{e} Young's modulus of the lattice material.\\
...
&- \param{mx0} ultimate capacity under pure moment about $x$.\\
&- \param{my0} ultimate capacity under pure moment about $y$.\\
&- \param{mz0} ultimate capacity under pure moment about $z$.\\
&- \optparam{sub} maximum number of subincrementations.\\
&- \optparam{iter} maximum number of newton iterations.\\
&- \optparam{tol} tolerance for newton method.\\
Supported modes & 3dlattice\\
\hline
\end{mmt}
\caption{Plasticity steel model for 3D lattice frame elements -- summary.}
\label{latticeframesteelplastic_table}
%\end{center}                  
  \end{table}
The nonlinear response of the beam element is given directly as an relationship between internal force and strain. The basic equations include an additive decomposition of total strain into elastic part and plastic part of the form
%
\begin{equation}
...
\begin{equation}
(\dfrac{N_x}{N_0})^2+ (\dfrac{M_x}{M_{x0}})^2+ (\dfrac{M_y}{M_{y0}})^2+ (\dfrac{M_z}{M_{z0}})^2 - 1 = 0
\end{equation}
where $N_x$, $M_x$,$M_y$ and $M$ , are the two components of bending moment, an axial force and
a torsional moment, respectively. The corresponding components with the subscript 0 indicate a fully plastic value under the
condition that each component of resultant forces acts independently.
...
\subsubsection{Reinforced concrete plasticity-damage material model for lattice based frame elements}
This is a plasticity-damage material used together with 3d lattice frame elements. It has been specifically developed to describe the failure process failure of reinforced concrete frame member using the lattice elements lattice3d and lattice3dnl. 
The generalised stress-strain law has the form
\begin{equation}
  \mathbf{s} = \mathbf{D}_{\rm e} \left(\mathbf{e}-\mathbf{e}_{\rm p} \right)
\end{equation}
where $\mathbf{e}$ is a vector of internal forces and moments, $\mathbf{D}_{\rm e}$ is the elastic stiffness matrix, $\mathbf{e}$ is a vector of translation and rotational jumps smeared over the element length and $\mathbf{e}_{\rm p}$ are the plastic smeared jumps. The model parameters are summarised in Table \ref{latticeframesteelplastic_table}.
...
\hline
Record Format & \descitem{latticeframeconcreteplastic} \elemparam{}{in}
\elemparam{d}{rn} \optelemparam{talpha}{rn} \elemparam{e}{rn} \elemparam{n}{rn} \elemparam{nx0}{rn} \elemparam{nx1}{rn} \elemparam{mx0}{rn} \elemparam{mx1}{rn} \elemparam{my0}{rn} \elemparam{my1}{rn} \elemparam{mz0}{rn} \elemparam{mz1}{rn} \elemparam{wu}{rn}  \elemparam{wf}{rn} \elemparam{sub}{in} \elemparam{iter}{in} \elemparam{tol}{rn}\\
Parameters &- \param{} material number\\
&- \param{d} material density\\
&- \optparam{talpha} Thermal expansion coefficient. Default is 0.\\
&- \param{e} Young's modulus of the lattice material.\\
...
&- \param{mx0} ultimate positive capacity under pure moment about $x$.\\
&- \param{my0} ultimate positive capacity under pure moment about $y$.\\
&- \param{mz0} ultimate positive capacity under pure moment about $z$.\\
&- \param{nx1} ultimate negative capacity under pure axialloads.\\
&- \param{mx1} ultimate negative capacity under pure moment about $x$.\\
&- \param{my1} ultimate negative capacity under pure moment about $y$.\\
&- \param{mz1} ultimate negative capacity under pure moment about $z$.\\
&- \param{wu} plastic generalised displacement jump norm (computed from rotational components only) threshold.\\
...
\hline
\end{mmt}
\caption{Damage Plasticity concrete model for 3D lattice frame elements -- summary.}
\label{latticeframesteelplastic_table}
\end{table}


\subsection{Material models for lattice transport elements}

\subsubsection{Material for unsaturated flow in lattice models -- LatticeTransMat}
...
\bibitem{ZhoMarGra24} C. Zhou and A. Marlot and P. Grassl: CDPM2F: A Damage-Plasticity Approach to Modelling the Failure of Engineered Cementitious Composites, Available at SSRN 4819173, 2024.
\end{thebibliography}
\end{document}
